...
### Fixed — ช่องจำนวน (crew/กล้อง/ไมค์) พิมพ์ไม่ได้ + ใช้ยากบนมือถือ

- ช่อง **จำนวน Videographer** ในฟอร์มจองเดิม "พิมพ์ไม่ได้" — เด้งกลับเป็น 1
  ทุกครั้งที่พิมพ์ (โค้ดเดิม `parseInt(...)||1` ทุก keystroke) แก้แล้ว พิมพ์/ลบ
  ได้ตามปกติ
- ทุกช่องจำนวนเปลี่ยนเป็น **NumberStepper** มีปุ่ม **−/+** กดง่ายบนมือถือ
  (ปุ่มใหญ่ ~40px) + ยังพิมพ์เลขเองได้ (แป้นตัวเลข, ไม่ซูมบน iOS)
- ใช้เหมือนกันทุกที่: ฟอร์มจอง (Videographer/กล้อง/ไมค์), Routine, หน้าแก้ไข
  booking (Producer + Admin)
- กล้อง/ไมค์ยังปล่อยว่างได้ (ให้ validation เตือน "กรุณาระบุ") และ 0 = ไม่ใช้
  ยังถูกต้อง · Videographer ว่างแล้วเด้งเป็น 1 ตอนออกจากช่อง

Logic ล้วน (clamp/step/blur) แยกไว้ที่ `src/lib/number-stepper.ts` + เทส 5 เคส
...
- **ช่องค้นหาทุกโมดูล** (equipment/rentals/purchases/repairs/vendors) — ค้นข้าม
  ทุกคอลัมน์ทันที (client-side) + equipment ยังค้น server-side (`?q=`) ทะลุเพดาน
  1000 แถว
- **คลิกหัวคอลัมน์เพื่อเรียง** (asc/desc/ปิด) ทุกคอลัมน์
- **หัวตารางลอย (sticky) + เลื่อนในกรอบ** — รายการยาว (อุปกรณ์ 1,719) เลื่อนดูสบาย
- ปุ่ม **ล้างตัวกรอง** + ตัวนับ "แสดง N (กรองจาก M)" ให้รู้ว่ากำลังกรองอยู่
- (เป็นการอัปเกรด CrudTable ตัวเดียว → ได้ผลทั้ง 5 โมดูลพร้อมกัน)
