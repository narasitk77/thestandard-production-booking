...
- วันนี้ booking หนึ่งตัวถูก list ซ้ำโดย 4 sweep ทุกชั่วโมง จ่ายโควตา Drive ซ้ำเพื่อคำตอบเดียวกัน · `DriveView` จำผลไว้ตลอด 1 pass และอัปเดตตัวเองเมื่อ phase เขียน
- **เส้นแบ่งที่สำคัญกว่าการประหยัดโควตา**: ผลจากแคช **ห้ามใช้เป็นหลักฐานในการลบ** เด็ดขาด — แคชอายุทั้ง pass (เป็นนาที) การ trash เพราะ listing เก่าบอกว่า "ว่าง" คือการขยายช่องระหว่าง "ดูแล้วว่าง" กับ "ลบ" จากวินาทีเดียวเป็นทั้งรอบ และทีมงาน**อัพไฟล์เข้าโฟลเดอร์วันเก่าระหว่าง pass จริงๆ** (พฤติกรรมเดียวกับที่บังคับให้ merge ต้อง lookback −45 วัน)
- `freshFiles`/`freshChildren` ข้ามแคชเสมอ = การอ่านแบบเดียวที่ใช้ก่อนลบได้ · `assertNotForDeletion('cached')` โยน error ทันทีเพื่อให้กฎนี้เห็นได้ใน diff ไม่ใช่ซ่อนในคอมเมนต์
- `createdThisRun` = โฟลเดอร์ที่ pass นี้เพิ่งสร้าง **นับเป็น consumable** (fast path ของ merge trash ทิ้งเพื่อย้ายทั้งก้อนได้) — ดีไซน์ v1 เขียนว่า "ห้าม trash" ซึ่งกลับด้านและจะฆ่า fast path ของทุก booking ทุกชั่วโมง
- +8 เทสต์ (list ครั้งเดียว · fresh อ่านใหม่เสมอและกลายเป็นความจริงใหม่ · guard ลบโยน error · move evict ปลายทางแทนที่จะเดา) รวม 421 ผ่าน

---

## [1.170.0] — 2026-08-07

### Added — `/admin/monitor` ศูนย์ติดตาม (KPI ของคนดูแลระบบ)
หน้าเดิมตอบได้แค่ "ผลลัพธ์เป็นยังไง" แต่คนดูแลระบบต้องการ "วันนี้ระบบทำอะไรไปบ้าง มีอะไรค้าง" — คนละคำถามกัน
- ตอบ 4 คำถามในหน้าเดียว: **งานไหนถึงคิวส่ง · ส่งออกไปหรือยัง · ใครตอบมาแล้ว · มีอะไรค้าง**
- `src/lib/review-ops.ts` — ตรรกะ KPI แยกเป็นฟังก์ชันบริสุทธิ์ + เทสต์ปักทุกเกณฑ์ (KPI ที่ความหมายเลื่อนไปเรื่อยๆ แย่กว่าไม่มี)
- **อัตราตอบกลับนับเฉพาะ invite ที่ส่งออกไปได้จริง** — invite ที่เมลล้มเป็นความผิดของระบบ ไม่ใช่ของทีม เอามารวมจะทำให้ทีมดูไม่ตอบทั้งที่ไม่เคยได้รับ
- **ไฟแดงจับ "ยังไม่มีใครรับเรื่อง" ไม่ใช่ "ยังไม่ปิด"** — ticket ที่มีคนรับแล้วแต่ยังแก้ไม่เสร็จไม่ใช่ความล้มเหลว แต่เรื่องที่ไม่มีใครแตะ 2 วันคือสิ่งที่ระบบนี้สร้างมาเพื่อกำจัด
- เวลาปิดงานใช้ **มัธยฐาน** ไม่ใช่ค่าเฉลี่ย (ticket เก่าตัวเดียวลากค่าเฉลี่ยพัง) · แสดง "ค้างนานสุดกี่วัน" เพื่อไม่ให้คนที่ถูกลืมคนเดียวหายไปในค่าเฉลี่ย
- ตอนระบบประเมินยังปิดอยู่ ตารางจะบอกตรงๆ ว่า "ถ้าเปิดวันนี้จะส่งหาใครบ้าง" — คำนวณจาก `buildInvites` ตัวเดียวกับที่ worker ใช้ หน้าจึงสัญญาในสิ่งที่ worker ทำจริงเท่านั้น
- สิทธิ์: ตัวเลข ticket = console · ตัวเลขประเมิน = 3 คนเท่านั้น (console คนอื่นได้ `reviews: null` ไม่ใช่ 403 ทั้งหน้า)
- +9 เทสต์ · รวม 413 ผ่าน

---

## [1.169.0] — 2026-08-07

### Added — ปุ่ม "สร้างฟอร์มตัวอย่างให้ฉัน" — เห็นแบบประเมินได้โดยไม่ต้องเปิดใช้จริง
ข้อบกพร่องของ v1.166 ที่ operator ทักมาตรงๆ: ฟีเจอร์ปิดอยู่ (ต้องเป็นแบบนั้น เพราะเปิด = เมลถึงทีมทันที) ผลคือ**ไม่มีอะไรให้กดเลย** หน้าเห็นผลว่างเปล่า ไม่มีทางรู้ว่าทีมจะได้รับอะไร
- `POST /api/admin/reviews/preview` — สร้าง invite **จริง** ให้ผู้เรียกเอง บน booking จริง **ไม่ส่งอีเมลถึงใครสักคน** · ฟอร์มที่เปิดคือของจริงไม่ใช่ mockup
- จำกัดเฉพาะ 3 คนที่ดูผลได้ (การ์ดเดียวกับหน้าผลประเมิน) — invite เป็น bearer credential การแจกไม่ใช่สิทธิ์แอดมินทั่วไป
- ใช้ invite เดิมถ้ามีอยู่แล้ว (unique index จะปฏิเสธตัวที่สอง และ preview ต้องกดซ้ำได้)
- ปุ่มอยู่บน `/admin/reviews` พร้อมบอกตรงๆ ว่าคะแนนที่ส่งจากฟอร์มนี้จะขึ้นในรายการจริงและลบไม่ได้ตามกติกา
...
สร้างหน้า feedback/ผลประเมินไว้ครบแล้วแต่ลืมใส่ทางเข้า — ผู้ใช้จริงจะไม่มีวันเจอ เท่ากับฟีเจอร์ไม่มีอยู่
- เพิ่มในเมนู **More**: `เรื่องที่ฉันแจ้งไว้` (ทุกคนที่ล็อกอิน) · `Feedback จากทีม` (console) · `ผลประเมินหลังงาน` (3 คน)
- เมนูผลประเมินซ่อนฝั่งเบราว์เซอร์เป็น **ความสวยงามเท่านั้น** — การ์ดจริงคือ `canReadReviews` ฝั่งเซิร์ฟเวอร์ (พิมพ์ URL เข้ามาก็ 403) เขียนกำกับไว้ในโค้ดกันเข้าใจผิด
- +1 เทสต์ยืนยันว่า tier gate ไม่เตะ crew/producer ออกจากหน้าเหล่านี้ · รวม 404 ผ่าน
...
เริ่มลงมือตัวยุบ ~10 workers เป็น pass เดียว หลังปิด §9 ครบแล้ว — รอบนี้วางฐาน 2 ชิ้นที่ทุก phase ต้องใช้ **โค้ดยังไม่ถูกเรียกจากที่ไหน ไม่กระทบ prod**
- `src/lib/reconciler/lease.ts` — DB lease (SystemHeartbeat) แทน route guard 8 ตัว · claim เป็น **CAS ผ่าน `updateMany` + staleness ใน WHERE** (read-then-write ปล่อยให้ 2 pass เห็น lease หมดอายุพร้อมกันแล้วเดินทั้งคู่) · **staleness วัดจากการ renew ไม่ใช่ความยาว run** เพราะ pass ที่ทำงานถูกต้องกินเวลา 20+ นาทีได้ — guard แบบ "หมดอายุใน 15 นาที" จะยกงานให้ pass ที่สองระหว่างตัวแรกยังเขียนอยู่ · crash แล้วปล่อยเองผ่าน TTL · แยก key ต่อ booking เพื่อให้ปุ่ม merge ไม่บล็อกทั้ง pass
- `src/lib/reconciler/guards.ts` — รวม predicate ที่กระจายซ้ำอยู่ **19 invariant** พร้อมเหตุผลว่าแต่ละข้อเขียนขึ้นหลังเหตุการณ์อะไร
- **เจอของจริงระหว่างรวม**: regex จับไฟล์ marker มี 2 แบบปนกันในระบบ — แบบไม่มี `\b` (4 ที่) กับมี `\b` (3 ที่) **ให้ผลต่างกันจริง**: ไฟล์ชื่อ `_SHOOTLIST.txt` หรือ `_SHOOTING-notes.txt` แบบไม่มี `\b` จะถูกนับเป็น marker ของระบบ → **โฟลเดอร์ที่มีไฟล์ของทีมอยู่จะดู "ว่าง" แล้วโดน cleanup ลบทิ้ง** · มาตรฐานใหม่ใช้แบบมี `\b` (ปลอดภัยทั้งสองทาง: ไฟล์แปลกหน้านับเป็นของจริงเสมอ)
- +18 เทสต์ปักเหตุการณ์จริงไว้กับกฎ (โฟลเดอร์พรุ่งนี้ห้ามลบ · งานหลายวันนับจากวันสุดท้าย · probe อ่านไม่ได้ = ห้ามสร้าง/ห้ามลบ · marker code อ่านดิบ · twin match ด้วย lead ที่แก้ไม่ได้ · ชื่อที่ทีมตั้งเองห้ามแตะ) รวม 403 ผ่าน

---

## [1.166.1] — 2026-08-06

### Fixed — ปิด*ต้นตอ*ของบั๊กที่ทำตัวตนคนประเมินหลุด ไม่ใช่แค่จุดที่เจอ
v1.166.0 แก้ audit row ที่หลุดไปแล้ว แต่ยังไม่ได้แก้เหตุที่ทำให้มันหลุดได้ตั้งแต่แรก: **`GET /api/bookings/:id/history` คืน audit row ทุก action ที่ผูกกับ booking พร้อม payload `changes` แบบดิบๆ ให้ทุกคนที่ล็อกอิน** (canViewBooking เปิดกว้างตั้งใจตั้งแต่ v1.152 เพราะตารางงานเป็นของสาธารณะ) แปลว่า**ฟีเจอร์ไหนก็ตามในอนาคตที่เขียน log ผูก booking จะหลุดซ้ำแบบเดียวกัน** โดยคนเขียนไม่มีทางรู้
- `src/lib/booking-history-visibility.ts` (ใหม่) — กรองแบบ **fail-closed**: คนที่ไม่ใช่ console เห็นเฉพาะ action วงจรชีวิตของ booking เอง (`booking.*`, `approve`, `document.*`) · **action ที่ไม่มีใครใส่ไว้ = มองไม่เห็น** ไม่ใช่เปิดเผยโดยปริยาย · console ยังเห็นครบเหมือนเดิมเพื่อไม่ให้งาน triage พัง
- เทสต์เขียนไว้จับ**ประเภท**ของบั๊ก ไม่ใช่แค่เคสวันนี้: `review.*` ต้องมองไม่เห็น, action สมมติในอนาคตต้องมองไม่เห็น, `approve` ต้อง match แบบเป๊ะไม่ใช่ prefix (กัน `approve.salary` เล็ดลอด), allow-list ห้ามโตเกิน 4 รายการโดยไม่มีเหตุผล
- เทสต์อ่าน**ซอร์สจริง**ของ review route ยืนยันว่า audit ไม่มี `entityType:'Booking'` / `raterRole` / `bookingCode` และ token ดิบไม่เคยเข้า audit payload — ถ้าใครเผลอใส่กลับ เทสต์แดงทันที
- audit ของ worker ส่งเมลเปลี่ยนจาก `entityType:'Booking'` เป็น `ShootReviewInvite` ด้วย (เดิมปลอดภัยเพราะ entityId='batch' แต่ปลอดภัยโดยบังเอิญ ไม่ใช่โดยตั้งใจ)
- 385 เทสต์ผ่าน · tsc สะอาด · lint 6 warnings เท่า baseline

---

## [1.166.0] — 2026-08-06

### Added — Feedback เป็นระบบ ticket (เลข FB-001) แทนอีเมลที่ตายในกล่อง
กล่องลอย "ติชม/แจ้งปัญหา" (v1.133) ส่งเข้าเมลแล้วจบ — คนแจ้งไม่รู้ว่าถึงไหน ไม่มีอะไรตามว่าเรื่องไหนยังค้าง
- ตาราง `FeedbackTicket` + `FeedbackMessage` · ผู้แจ้งเห็นเลขที่ทันทีในกล่องลอย + ลิงก์ไปหน้า **`/feedback`** ("เรื่องที่ฉันแจ้งไว้")
- **`/admin/feedback`** — คิวงาน: ใหม่/กำลังแก้/แก้เสร็จ, เธรดสนทนา, ตอบในเว็บแล้วระบบเมลหาผู้แจ้งให้, ปุ่ม "แก้เสร็จ + แจ้งผู้แจ้ง" พร้อมสรุปสิ่งที่แก้
- **ผู้แจ้งตอบกลับเรื่องที่ปิดแล้ว → เปิดใหม่อัตโนมัติ** (กันเคส "ยังไม่หายครับ" แล้วเงียบหาย); แอดมินตอบเรื่องใหม่ = รับเรื่องเอง
- ขอบเขตตัดสินฝั่ง server: คนทั่วไปเห็น/ตอบได้เฉพาะเรื่องตัวเอง เปลี่ยน parameter ไม่ช่วย · เปลี่ยนสถานะได้เฉพาะ console
- อีเมลเปลี่ยนบทบาทเป็น "ตัวแจ้งเตือน" — DB คือบันทึกจริง เมลล่มแล้วเรื่องไม่หาย

### Added — แบบประเมินหลังจบงาน (ประเมินกันเอง ไม่เปิดเผยต่อเพื่อนร่วมงาน)
ส่งอัตโนมัติหลังงานจบ 1 วัน ให้ทุกคนที่ร่วมงานให้คะแนน**ทีมอื่น** (สองทาง: โปรดิวเซอร์ ↔ กล้อง ↔ เสียง)
- เปิดจากลิงก์ในเมล **ไม่ต้อง login** (ฟรีแลนซ์เปิดจากมือถือได้) · ดาว 1–5 + 3 หัวข้อ + ข้อความ · ส่งได้ครั้งเดียวต่องาน
- **สิทธิ์ดูผล = 3 คนเท่านั้น** (นัท/ปุ๊ก/หวาน) บังคับใน `src/lib/review-access.ts` ฝั่ง server — แอดมินคนอื่นก็ 403 · `REVIEW_OWNER_EMAILS` แทนที่รายชื่อ (ไม่ใช่ต่อท้าย)
- **ลบไม่ได้จริง** — ไม่มี endpoint แก้/ลบ ShootReview และ **ตัด FK cascade ออก** เพื่อให้การลบ booking พาคะแนนหายไปไม่ได้ (snapshot bookingCode/shootDate ไว้ในแถว)
- **Export CSV** ปุ่มรายวัน + ตามช่วง · กัน Excel formula injection · บันทึก audit ว่าใครโหลดข้อมูลดิบ
- ปิดอยู่โดย default (`SHOOT_REVIEW_ENABLED=0`) — ฟีเจอร์นี้เมลหาคนจริง เปิดเมื่อสั่งเท่านั้น

### Fixed — 19 ประเด็นที่รีวิว adversarial จับได้ก่อน deploy (จาก 26 ข้อที่อ้าง ผ่านการหักล้าง)
- **BLOCKER — ตัวตนคนประเมินหลุด**: audit row ตอนส่งแบบประเมินเขียนเป็น `entityType:'Booking'` + `raterRole` ซึ่ง `GET /api/bookings/:id/history` คืน**ดิบๆ ให้ใครก็ตามที่ล็อกอิน** (canViewBooking เปิดกว้างตั้งแต่ v1.152) → โปรดิวเซอร์ที่เพิ่งถูกประเมินเปิดดูแล้วรู้ว่า "ทีมเสียงเป็นคนให้คะแนน" ซึ่งงานปกติมีคนเสียงคนเดียว = รู้ตัวคน · แก้เป็น `entityType:'ShootReviewInvite'` + ตัด raterRole/targets ทิ้ง
- **HIGH — ฟอร์มถามไม่ตรงกับที่เมลสัญญา**: เดิมคำนวณ "ทีมที่ต้องประเมิน" ใหม่ตอนเปิดฟอร์มจาก role ของคนเดียว ทำให้ทีมกล้องไม่มีวันถูกประเมิน · แก้เป็น**เก็บ targets ลง invite ตอนสร้าง** แล้วอ่านค่านั้น + POST ปฏิเสธทีมที่ไม่ได้ถูกถาม
- **HIGH — tier gate เตะทีมงานออก**: `/review` และ `/feedback` ไม่อยู่ใน ALWAYS → crew ที่ได้เมลกดลิงก์แล้วเด้งไป /admin (ลิงก์ตายตั้งแต่วันแรก) · เพิ่มแล้ว + เทสต์ปัก
- **HIGH — worker ไม่มีอยู่จริง**: endpoint ส่งเมลไม่มีอะไรเรียกเลย · เพิ่ม `scripts/shoot-review-worker.js` (10:00 BKK — ไม่ส่งแบบสอบถามตอนตี 3), supervise ใน start.sh, env passthrough ทั้ง prod + staging
- **MEDIUM** — คำนวณ "เมื่อวาน" เป็น UTC ทั้งที่ worker รันตามเวลาไทย (แก้ใช้ `startOfTodayBangkok`) · งานหลายวันถูกส่งฟอร์มทั้งที่ยังถ่ายอยู่ (แก้ใช้ `shootEndDate`) · เมลล้มแล้วคนนั้นไม่มีวันได้รับอีก (เพิ่ม `mailedAt`, รอบหน้าส่งซ้ำด้วย token เดิม) · error เดียวล้มทั้ง batch (try/catch ต่อ booking + รายงาน errors)
- **LOW** — ตอบ ticket ที่ปิดแล้วเขียนทับ resolvedAt/resolvedBy (แก้ให้ stamp เฉพาะตอนเปลี่ยนสถานะจริง + เปิดใหม่ล้าง resolution) · คนที่แค่*กรอกฟอร์มจอง*ให้คนอื่นถูกเชิญประเมินด้วย (ตัดออก)
- 377 เทสต์ผ่าน · tsc สะอาด · lint 6 warnings เท่า baseline

---

## [1.165.0] — 2026-08-06

### Fixed — probe เช็คโฟลเดอร์ตายมองว่า "Drive สะดุด" = "โฟลเดอร์หาย" (ทำให้ footage แตกสองกล่องได้)
`isFolderAlive` เป็น `catch { return false }` ครอบ `files.get` ที่**ไม่มี retry** ทั้งที่ `withDriveRetry` มีอยู่แล้วและ mutation ทุกตัวใช้อยู่ → 429 ครั้งเดียว (เกิดประจำเวลา sweep เร่ง) ทำให้ Drive id ที่ดีอยู่ถูกตัดสินว่า "ตาย" แล้ว caller ไหลไป**เดินหาจากชื่อโฟลเดอร์** ซึ่งหลายจุดเป็น create-on-miss
- `folderLiveness()` ใหม่ — tri-state `alive` / `dead` / `unknown` + retry · **เฉพาะ 404/410 เท่านั้นที่แปลว่าหาย**; 429/5xx/403 = `unknown` (Drive ตอบ 403 ทั้งกรณี rateLimitExceeded และสิทธิ์)
- `isFolderAlive` **คงความหมายเดิม** (จริงเฉพาะ `alive`) เพราะหลายจุดใช้เป็นการ์ด `continue` ก่อน mutate — เปลี่ยนเป็น "ไม่แน่ใจก็ลุย" คือผิดทาง · ที่ได้เพิ่มคือ retry
- `upload/init` (id-first ตั้งแต่ v1.163) เปลี่ยนเป็น **`unknown` = เชื่อ id เดิม** — Drive สะดุดครั้งเดียวต้องไม่ส่งการอัพไปเข้าเส้นทางที่สร้างกล่องใหม่ · id ที่เก่าจริงจะพังแบบเห็นชัด ดีกว่าแตกไฟล์เงียบๆ
- +5 เทสต์ปักตารางแปลงสถานะ · รวม 346 ผ่าน

### Changed — ยกเลิกแผน "ตัด name-fallback" (แก้คำตอบ §9 ข้อ 6 ของเช้าวันเดียวกัน)
รีวิว 11 agents + ตรวจโค้ดยืนยัน: แผนเดิมผิด และ**ตัดแล้วขัดกฎสูงสุด** (ห้ามทำให้งานทีมสะดุด / ห้ามลบเกินต้นแบบ)
- ตัวเลข gate เชื่อไม่ได้ (ปนกับ Drive สะดุด ตามข้อบนนี้) และ `noteResolve` ยิงเฉพาะตอนสำเร็จ — สถานะที่อันตรายจริงไม่เคยถูกนับ
- landing/staging **เป็น id-only ไม่ได้เชิงนโยบาย** — ops/NAS/ทีมเสียงสร้างโฟลเดอร์เองตลอด ไม่มีโค้ดไหนเก็บ id ให้
- `agn-restructure.ts:211-214` ตัด arm กลางออก = สร้างกล่องที่สองแล้วเขียน id กล่องเปล่าทับ (poison) — **อันตรายกว่าไม่ตัด**
- สัญญาใหม่: **id-first, name-repair, never name-authoritative** (บันทึกใน `docs/reconciler-design.md` §9 ข้อ 6 พร้อมหลักฐาน)
- ผลข้างเคียงที่ดี: การตัด fallback **ไม่ใช่ prerequisite ของ reconciler อีกต่อไป** — Stage 1 parallel-run เริ่มได้เลย
...
เดิมถ้า booking ถูกลงผิด outlet แก้ไม่ได้เลย (outlet ฝังอยู่ในเลข Production ID, path Drive, ชีท, ปฏิทิน, `_SHOOT.txt`) ทางเดียวคือยกเลิกแล้วจองใหม่ = เสียเลข ID เสียโฟลเดอร์ เสียนัดในปฏิทินของทีม
- `src/lib/move-outlet.ts` (ใหม่) — planner: คำนวณเลข ID ใหม่ในสาย outlet ปลายทาง (ตัวเองไม่นับในลำดับ), รายการปลายทางต่อ EP (เว้นว่าง = ใช้รายการเดิมถ้าบ้านใหม่มี), path Drive ทั้งสองฝั่ง, และคำเตือน (โปรดิวเซอร์ยังไม่อยู่ทีมปลายทาง / OT เดิม / กล่องมีไฟล์แล้ว)
- **ปฏิเสธชัดเจน** (ทั้งหมดตัดสินก่อนเขียนอะไรเลย): AGN สองทาง · Photo album · งานที่ผูก project · งานที่ส่งงานแล้ว · บ้านเดิม · รายการที่ปลายทางไม่มี
- `regenerateBookingId` รับ `outletMove` — ยังเป็นที่เดียวที่เลข ID เปลี่ยน: **preflight ตรวจ Drive ให้จบก่อนแตะชีท/ปฏิทิน** (ถ้าปฏิเสธ = ไม่มีอะไรถูกเขียน), ย้ายกล่องด้วย `files.update` ครั้งเดียว (**id เดิม ไฟล์ไม่หาย ลิงก์เก่ายังใช้ได้**), ย้าย `_SOUND-STAGING` ตาม, เปลี่ยนชื่อโฟลเดอร์ NAS, เขียน `outletId`+`programId` พร้อมรหัสใน transaction เดียว
- **หากหากล่องไม่เจอ = ปฏิเสธ ไม่ทำครึ่งๆ** — ไม่มี worker ไหนในระบบตรวจว่ากล่องอยู่ใต้ outlet ที่ถูกต้อง ความไม่ตรงแบบนี้จะไม่มีใครเจอและไม่มีใครซ่อม
- UI `/admin/<id>`: การ์ด "🚚 ย้ายสังกัด" แบบ **ดูก่อน → ย้ายจริง** (ปุ่มย้ายปลดล็อกเฉพาะเมื่อ preview ตรงกับตัวเลือกปัจจุบัน), preview บอกครบว่าจะแตะอะไรบ้าง, `expectedOldCode` กันกดซ้ำหลัง 504/แอดมินสองคนชนกัน
- ชีท: `updateBookingRow` เขียนคอลัมน์ **D (Outlet) / E (Program)** ได้แล้ว — เดิมย้ายแล้วชีทจะค้างชื่อบ้านเก่าคู่กับรหัสใหม่ (PMDC join คอลัมน์นี้)
- Route ใหม่ `POST /api/admin/<id>/move-outlet` (dry-run เป็นค่าเริ่มต้น) · kill switch `MOVE_OUTLET_ENABLED=0`, `MOVE_OUTLET_ID_FIRST=0`
- **ไม่แตะ OT โดยตั้งใจ** — `syncBookingOT` เป็น deleteMany+recreate จะลบ OT ที่อนุมัติแล้วพร้อมลายเซ็นทิ้ง (แสดงเป็นคำเตือนแทน)

### Fixed — upload/init สร้างกล่อง Drive ซ้ำได้ (blocker ที่เจอตอนรีวิว v1.163)
`resolveShootFolder` เป็น create-on-miss และ upload/init เดินตาม**ชื่อ**ล้วน — ถ้ากล่องถูกเปลี่ยนชื่อ/ย้าย (ops rename หรือย้ายสังกัด) การอัพครั้งถัดไปจะ**สร้างกล่องที่สองที่ path เดิม** แล้ว footage ของงานเดียวกันแตกเป็นสองที่โดยไม่มี worker ไหนรวมให้
- upload/init เป็น **id-first** แล้ว: ใช้ `driveFolders.box` ที่เก็บไว้ (ตรวจ alive + อยู่ใน footage tree) ไม่งั้นค่อย fallback ไปเดินชื่อแบบเดิม · kill switch `UPLOAD_ID_FIRST=0`
- `moveAndRenameFile` ห่อ `withDriveRetry` แล้ว (เป็นตัวสุดท้ายที่ยังไม่มี — 429 ระหว่างย้ายเคยเด้งเป็น error ทันที)

### กระบวนการ
ดีไซน์ผ่านการแข่ง 3 แนวทาง + ตรวจหาช่องพังแบบ adversarial 3 มุม (11 agents): เจอ 15 ประเด็น รวม blocker 2 (upload/init ข้างบน) — แก้ครบก่อน commit · 341 เทสต์ผ่าน · tsc สะอาด · lint 6 warnings เท่า baseline
...
ที่มา: `AGN-260713-01` (GWM) ใส่ EP id (`PP-26-036-S01`) แทนเลขใบเสนอราคา ทั้งที่ `QU-4480` มีอยู่ในคิวก่อนหน้าของโปรเจกต์เดียวกัน — สแกนพบ AGN 8/28 งานไม่มี QU ที่ถูกต้อง
- `src/lib/agency-ref.ts` — รูปแบบอิงข้อมูลจริง (`QU-4289` / `QU-4426-V1` / `QU-2811/2`): ขึ้นต้น QU ตามด้วยตัวเลข + normalize (uppercase, ตัดช่องว่าง)
- **บังคับ 3 เส้นทางเขียน** (create / PATCH / producer-edit) เฉพาะงาน **AGN + Advertorial** (ตรงกับที่ UI แสดงช่องนี้): พิมพ์มาผิดรูปแบบ → 400 พร้อมข้อความไทย; **เว้นว่าง → ดึงเลขจากคิวก่อนหน้าของโปรเจกต์เดียวกันใน DB อัตโนมัติ**; โปรเจกต์ใหม่ที่ไม่เคยมี → บังคับใส่
- Wizard: label เตือนสำหรับงาน Agency + validation แบบ live ตอนพิมพ์ (ตัวตัดสินจริงอยู่ server)
- Kill switch: `AGENCY_REF_QU_RULE=0` ปิดกฏจาก stack env ได้ทันทีถ้าไปขวางหน้างาน
- +3 เทสต์ (รูปแบบจริงทุกตัวผ่าน / ของหลุดจริง PP-xxx, LIFE2601, TBC ไม่ผ่าน) · รวม 322 · tsc สะอาด
...
บั๊กโฟลเดอร์ทุกตัวที่เจอ 22–23 ก.ค. อยู่ในโค้ดที่เรียก Google Drive — ซึ่ง**ไม่มีเทสต์เลย** (เทสต์เดิม 278 ตัวทดสอบแต่ฟังก์ชันบริสุทธิ์) คนที่เจอก่อนจึงเป็นหน้างาน
- `helpers/fake-drive.ts` — Drive จำลองในหน่วยความจำ (โฟลเดอร์/ไฟล์/parent, move=ย้าย parent, trash=tombstone, "ว่าง"=ไม่มีลูกเลย ตรงกับ `isFolderEmpty` จริง)
- รัน **`mirrorMove` ตัวจริง** ผ่าน `node:test` module-mock สลับ `./google-drive` เป็น fake — ไม่แตะ Google, ไม่แตะโค้ด production
- 4 เทสต์ = บั๊กจริงที่เคยหลุด: (1) โครง landing ว่างต้องไม่ถูกย้าย (v1.150.2) (2) โฟลเดอร์ EP ชื่อต่าง (มีโน้ตรถ) ต้อง merge เข้า EP เดิม ไม่แตกเป็นสองอัน (v1.151.3) (3) ไฟล์ซ้ำทิ้งไว้ที่เดิม ไฟล์ใหม่ย้าย (4) ชื่อซ้ำ+ขนาดต่าง = ไฟล์ใหม่
- **พิสูจน์ว่าเทสต์มีเขี้ยว** ด้วย mutation test: ย้อนโค้ดกลับเป็นบั๊ก → เทสต์ EP-split ล้มทันที
- เป็นเทสต์ล้วน + แก้ `npm test` เพิ่ม flag เท่านั้น — **ไม่มีการเปลี่ยนพฤติกรรม production ไม่ต้อง deploy** (เป็นด่านกันตอน build/CI)

---

## [1.153.0] — 2026-07-23

### Added — สรุปโฟลเดอร์ประจำวันเข้า Discord
- ทุกวัน 09:00 (BKK) worker สรุปให้ว่า **เจออะไร แก้อะไรไปบ้าง** ยิงเข้า Discord — นับยอดรวมทั้งวัน + ยกตัวอย่างจริงที่แก้ + รายการที่ต้องให้คนดูเอง + เตือนถ้าเจอ EP ซ้ำ
- สรุปสร้างจาก **audit log** ไม่ใช่ตัวแปรในหน่วยความจำ — ระบบนี้ deploy วันละหลายรอบ ถ้านับในหน่วยความจำตัวเลขจะรีเซ็ตทุกครั้ง
- กันส่งซ้ำด้วย marker ใน DB (แบบเดียวกับ throttle ของ stale-worker alert) — redeploy กลางวันไม่ทำให้ส่งซ้ำ และถ้า Discord ล่มก็ไม่ retry ทั้งวัน
- วันไหนไม่มีอะไรต้องแก้ ข้อความจะบอกว่า "ทุกอย่างเรียบร้อย" แทนที่จะส่งหัวข้อว่างๆ
- ตั้งเวลาได้ด้วย `FOLDER_INTEGRITY_DIGEST_HOUR` (ค่าเริ่มต้น 9)
- อีเมลสรุปรายชั่วโมงยังเหมือนเดิม (รายละเอียดเต็มอยู่ที่นั่น)

---

## [1.152.2] — 2026-07-23

### Changed — Discord แจ้งเฉพาะเรื่องไฟล์
- Discord เป็นช่องที่ทีมงานเฝ้าดูว่าไฟล์ลงหรือยัง การเอาเรื่องของเช่าเกินกำหนด/ใบแจ้งหนี้/worker ล่ม ไปปนทำให้คนเลื่อนผ่าน — เสียประโยชน์ข้อเดียวที่ช่องนี้มี
- ตอนนี้ Discord ได้เฉพาะ **เรื่องไฟล์**: รวมไฟล์วิดีโอเสร็จ · footage พร้อม · NAS ซิงค์ขึ้น Drive ครบ
- ย้ายไป**อีเมลอย่างเดียว**: digest เตือนงานค้างรายวัน (ของเช่า/ใบแจ้งหนี้/ซ่อม/อุปกรณ์) และแจ้งเตือน worker ไม่ตอบสนอง
- ย้อนกลับได้ด้วย env `DISCORD_NOTIFY_SCOPE=all` ไม่ต้องแก้โค้ด

---

## [1.152.1] — 2026-07-23

### Added — กลไกป้องกัน: ตรวจจับโฟลเดอร์ EP ซ้ำอัตโนมัติ
- v1.151.3 หยุด video-merge ไม่ให้สร้าง EP ซ้ำเพิ่ม แต่**ไม่มีอะไรคอยตรวจว่ามีของเก่าค้างอยู่หรือเกิดใหม่จากทางอื่น** — 8 งานที่สะสมมาถูกพบเพราะหน้างานมาบ่น ไม่ใช่เพราะระบบแจ้ง
- folder-integrity worker ตรวจทุกชั่วโมงว่ามีโฟลเดอร์ที่ขึ้นต้นด้วย EP เดียวกันซ้ำกันไหม — เจอแล้วขึ้น**หัวข้ออีเมล + บรรทัดแรกของ digest** พร้อมลิงก์ทั้งสองอัน ให้คนตัดสินใจรวมเอง
- **เป็น report-only เสมอ** — การรวมคือการย้ายไฟล์ ซึ่ง worker นี้ไม่ทำโดยหลักการ (สร้างกับเปลี่ยนชื่อเท่านั้น)
- ตัวนับแยก “แก้ N” กับ “เจอ N” ออกจากกัน — การเจอของซ้ำไม่นับเป็น “แก้แล้ว”

---

## [1.152.0] — 2026-07-22

### Changed — ตารางโปร่งใส: ทุกคนเห็นทุกงาน รวมถึงงานที่ยัง "ขอจอง"
- เดิม user ทั่วไปเห็นเฉพาะงาน **CONFIRMED** ของคนอื่น + งานตัวเอง → **งานที่ยังขอจองอยู่ (REQUESTED/ASSIGNED) ของคนอื่นถูกซ่อน** บนหน้า `/calendar` ผลคือโปรดิวเซอร์สองคนขอคิว/ทีมงานวันเดียวกันได้โดยไม่เห็นคำขอของอีกฝ่าย — ซึ่งขัดกับกติกา "First Come First Served" ที่ต้องเห็นกันถึงจะทำงาน
- ตอนนี้ทุกคนที่ล็อกอินเห็น **ทุก booking ทุกสถานะ** ทั้งบนปฏิทิน หน้าแรก และหน้ารายละเอียด (เดิมเห็นในปฏิทินแต่กดเข้าไปเจอ "Forbidden" — บั๊กเดียวกับที่ v1.131 เคยแก้ไว้เฉพาะงาน CONFIRMED ตอนนี้ปิดช่องว่างทั้งหมด)
- **ไม่ได้เปลี่ยนสิทธิ์แก้ไข**: อนุมัติ/จัดทีม/แก้ไข/ยกเลิก ยังเป็นของเจ้าของงาน + console เหมือนเดิม, งานที่ถูกลบยังเห็นเฉพาะ ADMIN, และ `scope=mine` (My Bookings, หน้าแรกช่อง "งานของฉัน") ยังกรองเฉพาะงานตัวเองถูกต้อง
- หมายเหตุ: `adminNotes` เดินทางไปกับข้อมูลรายละเอียดด้วย ถือเป็นข้อมูลปฏิบัติงานตามเจตนา "transparent data" — ถ้าภายหลังอยากซ่อนเฉพาะฟิลด์นี้ ให้ตัดฟิลด์ออกสำหรับคนนอก console แทนการปิดทั้งหน้า
