...
### Fixed — โฟลเดอร์ NAS ไม่มีเตรียมไว้ให้งานที่กำลังจะถ่าย (v1.222)

รายงานจากหน้างานบ่ายวันที่ 16 ก.ย.: NAS ไม่มีโฟลเดอร์ของวันที่ 17 เลย ทั้งที่พรุ่งนี้มีถ่าย 4 กอง · **worker ไม่ได้พัง — มันทำถูกตามที่เขียนไว้** ปัญหาคือหน้าต่างการสร้างกว้างแค่วันเดียว และมีสองที่ที่ต้องตรงกันแต่ไม่มีใครผูกไว้ด้วยกัน: `manageLandingFolders` ใช้ `bangkokDayRange(1)` = พรุ่งนี้วันเดียว ส่วน `shootIsImminentBkk` (ตะขอตอนอนุมัติ ที่มีไว้อุดช่องนี้โดยเฉพาะ) ครอบแค่วันนี้+พรุ่งนี้

**ใบที่อนุมัติล่วงหน้าเกิน 1 วันจึงตกช่องว่างทั้งหมด** ตอนอนุมัติยังไม่ imminent · sweep ก็ไม่ถึงจนกว่าจะ 19:00 ของเย็นก่อนถ่าย → เกือบทั้งวันทำงาน ไดรฟ์ drop ไม่มีอะไรเตรียมไว้ และ Synology ก็ไม่มีอะไรให้ซิงก์ลง NAS

- `LANDING_CREATE_DAYS` (ค่าเริ่มต้น 1 = พฤติกรรมเดิม · พรอดตั้ง **3**) ทำให้หน้าต่างเป็นช่วง `[+1 .. +N]` และ **`shootIsImminentBkk` อ่านค่าเดียวกัน** — ขยับพร้อมกันเสมอ ไม่งั้นช่องว่างแค่ย้ายที่ ไม่ได้ปิด
- `?days=N` สั่งเฉพาะครั้งได้ที่ `/api/internal/landing/manage` · ป้ายกำกับ action ใช้วันถ่ายของใบนั้นเอง ไม่ใช่วันแรกของหน้าต่าง · `targetDayEnd` + `createDays` เข้า audit/อีเมลสรุป
- สร้างล่วงหน้ามากขึ้นไม่เพิ่มความเสี่ยง: ครึ่งเก็บกวาดลบเฉพาะโฟลเดอร์ **ว่าง** ที่วันถ่าย **ผ่านไปแล้ว** ของอนาคตจึงไม่เคยถูกแตะ และ `ensureFlatShootFolders` เป็น idempotent

### Fixed — จองห้อง: อ่านไม่ได้ ≠ ห้องหาย · ห้องเต็มไม่ต้องยิงซ้ำทุกชั่วโมง · กฎเวลาเหลือที่เดียว (v1.222)

เจอระหว่างที่ระบบจองห้องของ IT **ล่มจริง** 10:47–10:55 น. (ตอบ 500 `Can't reach database server at db:3306` แล้วเป็น 502)

1. **`room-booking-reconcile.ts` ล้างเลขห้องทิ้งตอนอ่านระบบเขาไม่ได้** — `.catch(() => null)` ยุบ "อ่านไม่ได้" กับ "ไม่มีการจองอยู่" เป็นค่าเดียว แล้ว `stampCleared` ล้าง `roomBookingNo` ทั้งที่ห้องยังถูกยึดฝั่งเขา → ห้องค้างถาวรแบบไม่มีอะไรชี้กลับมา และรอบถัดไปจะจองใหม่ทับของเดิม = **จองซ้ำ** ในระบบที่ไม่มี idempotency · ผิดกฎ *"อ่านไม่ได้ ≠ ว่าง/ไม่มี"* ของทีมเอง (ตรวจ ส.ค.–ต.ค. แล้ว **ยังไม่เกิดความเสียหาย** — 67 ใบ รหัสไม่ซ้ำเลย)
2. **ห้องเต็มแล้วยิงซ้ำไม่รู้จบ** — candidates กรองแค่ `roomBookingNo=null` ใบเดียวจึงยิงได้ถึง ~1,000 ครั้งกว่าจะพ้นหน้าต่าง 45 วัน กินโควตา 20 req/5 นาที ที่ใช้ร่วมกันทั้งบริษัท และเด้งเข้ากลุ่ม LINE แอดมิน IT ทุกรอบ → เว้น 6 ชม. ก่อนลองใหม่ · `classifyRoomBookingResponse` รับ HTTP 409 และเทียบวลีหลายแบบ แทนประโยคไทยประโยคเดียวที่ถ้า IT แก้คำเมื่อไหร่จะกลายเป็น `unknown` แล้ววนยิงเงียบ ๆ
3. **กฎ "กองนี้กินห้องช่วงไหน" เขียนสองที่ ไม่ตรงกัน** — ตรวจว่างใช้ `call+4 ชม.` และข้ามวันเมื่อ `wrap <= call` ส่วนจองจริงใช้ `call+8 ชม.` และ `wrap < call` = **เช็คแคบ จองกว้าง** (ตรวจว่าง 4 ชม.บอกว่าง แล้วไปยึดห้องจริง 8 ชม.ทับคนอื่น) และเคส `wrap == call` ฝั่งตรวจได้ช่วงเวลากลับหัว → `overlaps()` คืน false เสมอ = **"ว่าง 100%"** → รวมเป็น `resolveShootWindow()` ที่เดียว · ช่วงที่ประกอบไม่ได้เป็น skip `'bad-times'` ไม่ใช่ช่วงว่าง ๆ เงียบ ๆ

เทสเดิม *"ไม่มี wrap → บวก 4 ชั่วโมง"* คือเทสที่ล็อกบั๊กข้อ 3 เอาไว้ — เปลี่ยนเป็น 8 พร้อมเทสใหม่ที่บังคับว่า **เช็คกับจองจริงต้องได้ช่วงเวลาเดียวกันเสมอ**

### Fixed — แก้ใบจองแล้วห้องต้องตาม + ปิดรูยกเลิกที่เหลือ (v1.222)

*"การแก้"* เจ็บกว่า *"การยกเลิก"* เพราะเงียบสนิท: PATCH ของแอดมินและ producer-edit เขียน `callTime` / `estimatedWrap` / `shootEndDate` / `locationId` ได้ แล้ว re-sync ปฏิทินและ OT แต่ **ไม่แตะห้องเลย** → ห้องเดิมค้างที่ช่วงเวลาเดิมตลอดไป ช่วงใหม่ไม่มีห้อง และ `syncRoomBooking` ก็ไม่จองใหม่เพราะเห็นว่ามี `roomBookingNo` แล้ว

ตัวกันพลาดตัวสุดท้ายก็จับไม่ได้ เพราะ reconciler **เทียบของเรากับตัวเราเอง**: `roomChanged = want.roomId !== roomIdForLocation(b.locationId)` โดยที่ `want.roomId` ก็มาจาก `roomIdForLocation(b.locationId)` ตัวเดียวกัน → เป็นเท็จเสมอ และไม่มีบรรทัดไหนเทียบ *เวลา* เลย

- `roomScheduleChanges()` — กฎ "ตารางเปลี่ยนไหม" ที่เดียว ใช้ร่วมกันทั้งสองเส้นทางแก้ไข · `resyncRoomForBooking()` คืนห้องเดิมให้สำเร็จ **ก่อน** จองใหม่ · คืนไม่สำเร็จ = **ไม่จองใหม่** (ไม่งั้นใบเดียวยึดห้องสองช่วง)
- reconciler เทียบกับ **ของจริงในระบบเขา** — `listRoomBookings` คืน `roomId`/`startAt`/`endAt` มาด้วยแล้ว (เดิมตัดทิ้ง) จับได้ทั้ง "ย้ายห้อง" และ "เลื่อนเวลา"
- ปิดรูยกเลิกที่เหลือ: `DELETE /api/bookings/:id` · MCP `cancel_booking` · hard delete

**4 ข้อที่รีวิวจับได้ก่อนขึ้นพรอด** (รวมบั๊กที่การแก้รอบแรกสร้างขึ้นเอง) — ทั้งหมดเป็นเรื่อง fail-closed:
1. `cancelRoomBookingFor` **ไม่ throw** เวลาล้มเหลว มันคืน FORBIDDEN/UNKNOWN ⇒ `try/catch` ที่ใส่ไว้ใน hard delete ไม่มีวันทำงาน แถวถูกลบทิ้งพร้อม `auditLog.deleteMany` ที่ลบแถว `booking.room_*` ซึ่งเป็นที่เดียวที่ยังบอกเลข `BK-####` ได้ → ตอบ 409 ไม่ลบ + กัน audit ของห้องไว้
2. เวลาที่อ่านมาเป็น `null` ถูกอ่านว่า "เวลาไม่ตรง" แล้วสั่งยกเลิกห้อง — ถ้าฟีดเขาหยุดส่ง `startAt`/`endAt` เราจะปลดห้องทิ้งทั้งระบบในรอบเดียว
3. `expectedTarget` คืน `null` ให้ skip ทุกเหตุผล ⇒ เหมา "บอกไม่ได้" ปนกับ "ไม่ควรมีห้อง" → เป็นสามสถานะ `none` / `unknown` / `target` · เฉพาะ `external` + `no-room-mapping` เท่านั้นที่ปลดห้องได้
4. resync ไม่มีตัวกันซ้อน → UPDATE เดียวเป็นตัวจองสิทธิ์ พร้อมวันหมดอายุ 10 นาที (ไม่ใช้ advisory lock เพราะ xact lock ปล่อยตอน commit ซึ่งเกิดก่อนงาน HTTP เริ่มด้วยซ้ำ)

### หมายเหตุ — งานเชื่อมระบบจองห้องกลาง (v1.195–v1.208) ไม่เคยถูกบันทึกไว้ที่นี่

ทั้งชุด (จองอัตโนมัติ at-most-once · ยกเลิกคิว = คืนห้อง · reconciler รายชั่วโมง) อยู่ใน git เท่านั้น ทั้งที่ตอนนี้เดินจริงบนพรอดและเป็น **~98% ของการจองห้องทั้งบริษัท** (ก.ย. 2026: 50 ใบในระบบกลาง มาจากโปรบุ๊ค 49) · สิทธิ์ยกเลิกที่ IT เปิดให้เมื่อ 2026-08-25 ทำให้ข้อความเตือนเก่าที่เขียนว่า "รอ IT เปิดสิทธิ์" **ไม่จริงอีกต่อไป**

### Added — คลังข้อมูลรายวันบน Lark: เก็บของที่ระบบจะลบทิ้ง (v1.213)

`BACKUP_WORKER_ENABLED=1` อยู่แล้วและทำงานดี แต่มันไม่ใช่ "คลัง" ด้วยเหตุผลสองข้อ: **มัน prune ตัวเองที่ 30 วัน** และมันเป็น `.sql.gz` ที่กู้ทั้งฐานได้แต่เปิดย้อนดูรายการเดียวไม่ได้ ขณะที่แอปนี้ลบแถวจริงตามนโยบาย — `audit_logs` เกิน 90 วัน **ทุกครั้งที่ container boot** · `ot_records` นอกหน้าต่าง 10 วัน **ทุกครั้งที่มีคนเปิดหน้า `/ot`** · ลบ booking ลาก `audit_logs`/`footage_log`/`ot_records` ของใบนั้นไปด้วย ผลรวมคือ **แถวที่ถูกลบไปเกิน 30 วันไม่เหลือในสำเนาไหนเลย**

- **ไฟล์รายวันขึ้น Lark Drive** `probook-YYYY-MM-DD.json.gz` — ทั้งฐาน วันละไฟล์ **ไม่มีอะไรในโค้ดนี้ลบมัน** · อ่านรายชื่อตารางจาก `information_schema` ไม่ใช่จาก Prisma models เพราะคลังที่รู้จักแค่ตารางที่มีคนจำได้ว่าต้องเพิ่ม ไม่ใช่คลัง (ยืนยันกับ DB จริงแล้ว: 30 ตาราง รวม `nas_sync_state.key` / `notification_seen.email` ที่ primary key ไม่ใช่ `id`)
- **mirror ลง Lark Base** ตาราง `probook_<ชื่อ>` เขียนทับทุกคืน — เปิดดู กรอง ค้นได้โดยไม่ต้องแตกไฟล์ · **เขียนทับ ไม่ใช่เพิ่มทบ** เพราะเพิ่มทบจะโตวันละ ~11,000 แถวและชนเพดานแถวของ Lark ภายในสองสัปดาห์
- **`probook__deleted_rows` — ตารางของที่หายไป** เพิ่มอย่างเดียว ไม่เคยลบ · mirror บอกไม่ได้ว่าอะไรถูกลบ (แถวที่ถูกลบก็แค่ *ไม่อยู่* ซึ่งมองไม่เห็น) ตัวนี้เทียบ key ของรอบนี้กับรอบก่อน แล้วบันทึกทุกแถวที่หายพร้อม **ชื่อไฟล์ snapshot ที่ยังมีเนื้อเต็ม** · ตารางหายทั้งตาราง (มีคนแก้ `LARK_EXPORT_EXCLUDE`) นับเป็น config event ไม่ใช่การลบ 4,129 แถว ไม่งั้นตารางที่บันทึกการลบจริงจะอ่านไม่ได้อีกเลย
- **สามชั้นความอ่อนไหว อยู่ที่เดียวใน `lark-export-policy.ts`** — ชั้น 1 `shoot_reviews` + `shoot_review_invites` **env เปิดไม่ได้ ต้องแก้โค้ด**: `review-access.ts` ล็อกให้อ่านได้ 3 คนและฟอร์มบอกพนักงานไว้แบบนั้น ก็อปลง Base = ย้ายคำสัญญาจากโค้ดที่บังคับได้ไปเป็นช่องติ๊กแชร์ (และ invite มีคอลัมน์ `token` ที่เป็น bearer credential) · ชั้น 2 ปิดไว้ เปิดได้ด้วย `LARK_EXPORT_INCLUDE`: `users` `ot_records` `page_events` `feedback_*` (มติ 2026-08-31) · ชั้น 3 ที่เหลือ 23 ตาราง · ระดับคอลัมน์: ตัดชื่อที่มี `token`/`secret`/`password`/`apikey`/`credential`/`iban`/`swift` เป็นส่วนใดก็ได้ ทุกตาราง — บวก `account` แบบแคบ (ท้ายชื่อ หรือตามด้วยคำว่าเลข) เพื่อดัก `Vendor.bankAccount` โดยไม่ไปโดน `accountManager` · ยืนยันกับ 303 คอลัมน์จริงบน prod: ตัดออกคอลัมน์เดียวคือ `vendors.bankAccount` และไม่แตะ `driveFileId`/`boxFolderId`/`externalKey`
- **`archiveOk` กับ `mirrorOk` แยกกันเสมอ ไม่ยุบเป็น boolean เดียว** — ไฟล์คือคำสัญญา Base คือความสะดวก คืนที่ไฟล์ลงแต่ Base พังคือคืนที่ดีกับของเล่นที่เสีย ส่วนคืนที่รายงาน "ok" ทั้งที่ไฟล์ไม่ลงคือความผิดพลาดที่รีโปนี้โดนซ้ำ ๆ (v1.186) · และ `code 0` จาก Lark แปลว่า "รับคำสั่งแล้ว" เท่านั้น — ไม่มี `file_token` = ไม่ถือว่าไฟล์ลง
- **liveness แยกจาก outcome** — heartbeat `lark-export` เต้นเมื่อ "รอบกลางคืนวิ่ง" (แม้อัปไม่ขึ้น) ส่วน "ไฟล์ลงจริงไหม" อยู่ที่ `/api/internal/lark-export/stats` ที่คำนวณ `alerts[]` ภาษาไทยมาให้ · ผูก tick กับความสำเร็จจะทำให้ stack ที่ตั้งค่า Lark ผิดปลุกคนทุก 6 ชม.เรื่อง worker ตาย ทั้งที่ worker ปกติและ config ต่างหากที่ไม่ปกติ (บทเรียน folder-integrity กลับด้าน)
- **`?dryRun=1` และ `/preflight?write=1`** — dry run อ่าน DB สร้าง snapshot คิด diff แล้วจบ ไม่แตะ Lark · preflight ลองยิงจริงเพราะ scope กับการแชร์โฟลเดอร์ให้แอปเป็นคนละเรื่องกัน และอ่านจากโค้ดไม่มีทางรู้ · ทั้งคู่รายงานปัญหาเป็นภาษาไทยว่าขาดขั้นไหน
- Runbook: `docs/runbook-lark-export.md` (ขั้นตอนใน Lark Developer Console ~10 นาที) · dormant จนกว่าจะตั้ง `LARK_EXPORT_ENABLED=1` · nightly check มีขั้น 3c ใหม่ · 49 เคสทดสอบใหม่ รวม 744 เคส

### Added — ขอมิกซ์จากหน้าใบจองได้ + เห็นคำขอที่มีอยู่ (v1.219)

ปิดข้อสุดท้ายของ journey ที่ไล่กับ operator: **ใบจองกับคิวมิกซ์ไม่เคยรู้จักกัน** — คนที่กำลังเปิดงานอยู่ต้องจำว่ามี `/mix` แล้วไปพิมพ์รหัสใบจองเองอีกที · v1.218 แก้แค่ "หาทางเข้าเจอ" ยังไม่ได้แก้ "เติมรหัสให้"

- **การ์ด "มิกซ์เสียง" บนหน้าใบจอง** วางต่อจากเบิกอุปกรณ์ เพราะเป็นของประเภทเดียวกันคือ *ขออะไรเพิ่มสำหรับงานใบนี้* ไม่ใช่ข้อมูลของตัวใบจองเอง
- **แสดงคำขอที่มีอยู่ — สำคัญกว่าปุ่มด้วยซ้ำ** ถ้าไม่โชว์ คนที่สองจะขอซ้ำเพราะไม่มีทางรู้ว่าคนแรกขอไปแล้ว แล้วทีมเสียงได้งานซ้ำสองใบ · โชว์ **ทุกสถานะรวมที่จบแล้ว** เพราะงานที่ส่งไปแล้วคือคำตอบของ "ไฟล์มิกซ์อยู่ไหน" ซึ่งคนเปิดหน้านี้อยากรู้พอ ๆ กับการขอใหม่ (มีลิงก์ไฟล์ให้กดตรงนั้นเลย)
- **ปุ่มพาไป `/new?mode=mix&booking=<รหัส>`** เติมรหัสไว้ให้แล้ว · เลือกวิธีนี้แทนการฝังฟอร์มลงหน้าใบจอง เพราะฟอร์มมีที่อยู่ที่เดียว (`MixRequestForm`) และหน้าใบจองก็ยาวพออยู่แล้ว · ผลพลอยได้: ลิงก์นี้ส่งต่อในแชทได้ กดแล้วเปิดฟอร์มพร้อมรหัส
- **`GET /api/mix?bookingId=` แยกจาก `scope`** โดยตั้งใจ — ถามว่า "ใบนี้มีคำขออะไรบ้าง" ไม่ใช่ "คิวตอนนี้เป็นไง" จึงคืนทุกสถานะ
- **"โหลดไม่ได้" ไม่ถูกวาดให้เหมือน "ไม่มีคำขอ"** — ขึ้นข้อความสีเหลืองว่าให้ไปเช็คที่คิวก่อนขอใหม่ ไม่งั้นการ์ดที่สร้างมาเพื่อกันคำขอซ้ำจะกลายเป็นตัวทำให้ขอซ้ำเสียเอง (คลาส PP-26-039)
- `bookingCode` ถูกเพิ่มในไทป์ `BookingDetail` — API คืนมาอยู่แล้ว (`findUnique` + `include` = ได้ scalar ครบ) แค่ไม่เคยประกาศฝั่งหน้าเว็บ

### Added — ขอมิกซ์เสียงได้จากหน้า New Booking (v1.218)

คิวมิกซ์เปิดใช้ได้ตั้งแต่ v1.215 แต่คนขอ **ต้องรู้เองว่ามีหน้า `/mix`** ซึ่งเป็นปัญหาการค้นพบแบบเดียวกับที่ทำให้ `/switcher` มี 0 แถว · ส่วน `/new` คือที่ที่คนมาอยู่แล้วเวลา "อยากได้อะไรสักอย่างจากทีมโปรดักชัน" — ปุ่ม **+ New Booking** อยู่บน nav ตลอด ทางเข้าที่คนหาเจอจริงจึงอยู่ที่นี่ ไม่ใช่ URL ที่ต้องจำ

- **ไม่ได้คิดแบบใหม่ — ต่อของเดิม** `/new` มี mode switch อยู่แล้ว (จองครั้งเดียว ↔ Routine) เพิ่มโหมดที่สามในแบบที่พิสูจน์แล้วว่าใช้ได้
- **`single` ยังเป็นค่าเริ่มต้น** คนจองกอง (ทราฟฟิกเกือบทั้งหมด — `bookings` 532 แถว เทียบ `mix_jobs` 0) **ไม่ต้องคลิกเพิ่มแม้แต่ครั้งเดียว** · การเพิ่มทางเลือกต้องไม่เก็บค่าผ่านทางจากเส้นทางหลัก
- **โหมดมิกซ์เห็นได้ทุกคน** ต่างจาก Routine ที่เป็น console-only — คนขอมิกซ์คือโปรดิวเซอร์/คนตัด/ใครก็ได้ที่มีงาน กั้นแล้วก็กลับไปที่ปัญหาเดิมคือคิวว่าง
- **ตั้งชื่อให้อ่านเป็นแกนเดียวกัน** สองปุ่มเดิมเป็นแกน "กี่ครั้ง" ปุ่มใหม่เป็นแกน "งานอะไร" — คนละแกน จึงเปลี่ยนเป็น *จะขออะไร*: `จองคิวถ่าย` / `ขอมิกซ์เสียง` / `จองถ่ายแบบประจำ` แทนที่จะปล่อยให้อ่านว่า "ครั้งเดียว vs รายสัปดาห์ vs เสียง" ซึ่งไม่เข้าพวกกัน
- **ฟอร์มแยกออกมาเป็น `MixRequestForm` ใช้ร่วมกันสองที่** ไม่ก็อป — ฟอร์มที่ถูกก็อปคือฟอร์มที่จะเพี้ยนออกจากกันในอีกหกเดือน แล้วสองทางเข้าจะรับข้อมูลไม่เหมือนกันโดยไม่มีใครรู้
- **ส่งแล้วแสดงผลตรงนั้น ไม่เด้งไป `/mix`** คนที่มาจากหน้านี้ยังไม่ได้เปิดคิวอยู่ การพาไปหน้าอื่นทันทีทำให้ไม่ทันเห็นว่าคำขอได้เลขอะไร · และแสดง **แจ้งใครไปแล้วบ้าง** — ถ้าเมลไม่ออกจะขึ้นเตือนสีเหลืองให้ทักทีมเสียงอีกทาง แทนที่จะบอกว่า "ส่งแล้ว" เฉย ๆ (บทเรียน v1.186)
- ตรวจด้วยการเปิดหน้าจริง: wizard เดิมยัง render ครบ 5 ขั้นไม่กระทบ · สลับโหมดได้ · คนทั่วไปเห็น 2 ปุ่ม (Routine ไม่โผล่)

### Added — ปิดวงจร: ลิงก์ไฟล์ที่มิกซ์เสร็จ + แจ้งคนขอตอนส่งงาน (v1.217)

v1.215–216 มีแต่ `sourceLink` **ขาเข้า** — มิกซ์เสร็จแล้วไฟล์ไปไหนยังต้องบอกกันในแชท และคนขอไม่เคยรู้ว่างานเสร็จเลยจนกว่าจะกลับมาเปิดหน้าเอง **วงจรจึงกลับไปจุดเริ่มต้น** ซึ่งเป็นสิ่งที่คิวนี้มีไว้แก้พอดี · รูนี้ผมพลาดเองตอนออกแบบ v1.215 เจอตอนไล่ journey ทีละขั้นกับ operator

- **`deliveryLink` ขาออก** และ **บังคับต้องมีก่อนเปลี่ยนเป็น `DONE`** (`canCloseMixJob`) — รูปเดียวกับกฎขาเข้า "ต้องมีใบจองหรือ sourceLink อย่างน้อยหนึ่ง" · เหตุผลตรงกัน: ถ้ากด "ส่งแล้ว" ได้โดยไม่บอกที่อยู่ไฟล์ คนขอได้เมลว่าเสร็จแล้วแต่ยังต้องไปถามอยู่ดี
- **ราคาที่รู้ตัวและเขียนกำกับไว้ในโค้ด**: คนที่ไม่มีลิงก์อาจเลี่ยงด้วยการไม่กดปิดงานเลย ซึ่งแย่กว่า — ถ้าวันหนึ่งคิวเต็มไปด้วยงานที่ทำเสร็จแล้วแต่ค้างสถานะ **ให้ผ่อนกฎนี้ ไม่ใช่ไปโทษคนใช้**
- **แจ้งคนขอตอนส่งงาน** พร้อมลิงก์ในเมล (+ coordinator เห็นว่าคิวเดินจบ) — นี่คือจุดที่วงจรปิดจริง คนขอไม่ต้องถามใครอีก
- **แปะลิงก์ไว้ก่อนแล้วค่อยกดปิดทีหลังได้** — API รับ `deliveryLink` แยกจาก `status` ไม่บังคับทำสองอย่างในคลิกเดียว
- หน้าเว็บถามลิงก์ก่อนยิง แทนที่จะปล่อยให้เซิร์ฟเวอร์ปฏิเสธแล้วค่อยบอก — คนกดปุ่มนี้กำลังจะจบงาน ไม่ได้กำลังทำผิด (เซิร์ฟเวอร์ยังบังคับซ้ำอยู่ ตรงนั้นคือด่านจริง ตรงนี้คือความสุภาพ)
- `normalizeHttpLink()` แยกออกมาใช้ร่วมกันทั้งขาเข้า/ขาออก — `javascript:` และ `file://` ตกทั้งคู่ · ค่าที่เคยเก็บไว้แต่ใช้ไม่ได้ก็ไม่ถือว่าผ่าน
- 3 เคสทดสอบใหม่ รวม 783

### Changed — คิวมิกซ์: coordinator เป็นคนแจกงาน + แจ้งเตือนเข้า sound@ (v1.216)

v1.215 ปล่อยคิวออกไปโดย **ไม่มีการแจ้งเตือนเลยสักบรรทัด** — ทีมเสียงต้องเปิดหน้าเองถึงจะรู้ว่ามีงาน ซึ่งเป็นความล้มเหลวแบบเดียวกับ `/switcher` แต่กลับด้าน: ที่นั่นไม่มีคน**กรอก** ที่นี่ไม่มีคน**อ่าน** ผลปลายทางเหมือนกันคือทุกคนกลับไปทักในแชท · และโมเดลก็ผิดจากที่ ops ทำจริง: ผมทำเป็น "ทีมเสียงกดรับงานเอง" แต่ของจริงคือ **coordinator เป็นคนแจก**

กระบวนการที่ operator ระบุ 2026-09-03: คนขอส่งคำขอพร้อมกำหนดส่ง → แจ้ง `sound@thestandard.co` → coordinator ได้รับแจ้งแล้วแจกให้ทีมงาน → บันทึกไว้ในระบบ

- **แจกงาน ≠ หยิบงาน** — `canAssignMixJob` (สั่งให้คนอื่นทำ) จำกัดที่ coordinator + admin · `canClaimMixJob` (หยิบให้ตัวเอง) ยังอยู่เป็น**ทางสำรอง** เพราะ coordinator คนเดียวลาหยุดแล้วคิวค้างทั้งคิวคือ single point of failure ที่แพงกว่าความเรียบร้อยของกระบวนการ · UI ซ่อนปุ่ม "รับงานเอง" จากคนที่แจกได้อยู่แล้ว ไม่ให้มีสองทางทำเรื่องเดียวกันบนหน้าจอเดียว
- **แจกได้เฉพาะคนใน roster จริง** (`TeamMember.role='sound'`) — แจกให้คนนอกทำให้ตัวเลขภาระงานเพี้ยนแบบเดียวกับปล่อยให้คนขอหยิบงานเอง · dropdown บนหน้าเว็บใช้**รายชื่อชุดเดียวกับที่ route ใช้ตรวจ** ไม่งั้นจะโชว์คนที่ฝั่งเซิร์ฟเวอร์ปฏิเสธ
- **`assignedByEmail` แยกจาก `assigneeEmail`** — "ใครทำ" กับ "ใครสั่งให้ทำ" เป็นคนละคำถาม และคำถามหลังคือสิ่งที่ตอบได้ว่างานค้างเพราะไม่มีคนแจก หรือแจกแล้วไม่มีคนทำ · `null` = หยิบเอง
- **แจ้งเตือนสองจังหวะ** — คำขอใหม่ → `SOUND_TEAM_EMAIL` + coordinator · แจกแล้ว → คนที่ถูกแจก **+ คนขอ** (ถ้าแจ้งแต่คนทำ คนขอก็ยังต้องไปถามในแชทอยู่ดี คิวก็แก้ปัญหาได้ครึ่งเดียว)
- **ผลการส่งบันทึกราย recipient ไม่ยุบเป็น boolean** และคืนกลับไปให้คนขอเห็นด้วย — "ส่งคำขอแล้ว" ที่ไม่มีใครได้รับคือคำโกหกที่สุภาพ (บทเรียน v1.186 ที่บันทึกว่าส่งถึง 85/85 คนทั้งที่ไม่มีใครได้รับ)
- **`dropSender()` กันกับดัก Gmail ส่งหาตัวเอง** — prod ส่งผ่านบัญชี `narasit.k@` และ Gmail ไม่ส่งเมลถึงตัวเอง เมลจะหายเงียบโดยไม่มี error (สาเหตุจริงของ footage-ready ที่ "สำเร็จ" 103 ครั้งโดยไม่มีใครได้รับ 5 สัปดาห์) · เคสนี้ปลายทางเป็นคนละที่อยู่จึงผ่าน แต่กันไว้เผื่อวันหนึ่งมีคนตั้ง `SOUND_TEAM_EMAIL` ทับที่อยู่ผู้ส่ง
- `SOUND_COORDINATOR_EMAILS` ว่าง = `krittapon.j@thestandard.co` (role COORDINATOR · position Senior Sound Engineer — ยืนยันจาก prod) · ตั้งคนสำรองเพิ่มได้โดยไม่ต้อง deploy · ประกาศใน compose แล้ว ซึ่ง `compose-env-coverage.test.ts` บังคับอยู่
- 4 เคสทดสอบใหม่ รวม 780 · ครอบว่าวิศวกรเสียงธรรมดา **แจกงานไม่ได้** (หยิบเองได้อย่างเดียว) และแจกซ้ำกลางทางได้แต่แจกงานที่จบแล้วไม่ได้

### Added — คิวงานมิกซ์เสียง `/mix` (v1.215)

ทีมเสียงถูกสั่งงานมิกซ์กันในแชท ไม่มีที่ไหนตอบได้ว่าคิวยาวแค่ไหน ใครกำลังทำอะไร งานไหนเลยกำหนด — คำถามเดียวกับที่ `/switcher` ถูกสร้างมาตอบสำหรับงานไลฟ์

**แต่จงใจกลับด้านกับ `/switcher`** — ที่นั่นเป็น *สมุดบันทึก* ให้คนทำงานมากรอกย้อนหลัง และมี **0 แถว** ตั้งแต่ปล่อย v1.211 (ยัง beta อยู่ก็จริง แต่โครงสร้างแรงจูงใจก็ไม่ช่วย) เทียบกับ `bookings` ที่มี 532 แถว · ความต่างไม่ใช่ UI แต่คือ **ใครเป็นคนกรอก**: ใน log คนกรอกคือคนที่ทำเสร็จแล้ว กรอกไปก็ไม่ได้อะไร · ในคิวจอง คนกรอกคือคนที่อยากได้ของ ไม่กรอกแล้วไม่ได้งาน แล้วบันทึกว่าใครทำอะไรเกิดขึ้นเองเป็นผลพลอยได้

- **คนขอสร้างแถว ทีมเสียงเข้ามารับ** — `QUEUED → IN_PROGRESS → DONE` เป็น allowlist (สถานะใหม่ที่เพิ่มวันหลังจะไปไหนไม่ได้จนกว่าจะมีคนเขียนกฎ) · `DONE → IN_PROGRESS` เปิดไว้เพราะลูกค้าขอแก้หลังส่งเป็นเรื่องปกติของงานมิกซ์ · ข้าม `QUEUED → DONE` ไม่ได้ ไม่งั้นไม่มีใครรู้ว่าใครทำ
- **แนบใบจองได้แต่ไม่บังคับ** — งานส่วนใหญ่ต่อจากกองถ่าย แต่พอดแคสต์/ไฟล์นอก/รีมิกซ์ของเก่าไม่มีกอง · บังคับผูกแล้วงานกลุ่มหลังเข้าระบบไม่ได้ · `bookingId` เป็น plain column **ไม่ใช่ FK** (บทเรียน ShootReview: `onDelete: Cascade` ทำให้ลบใบจองแล้วลบคำขอที่คนขอไว้จริงหายไปด้วย) พร้อม `bookingCode` snapshot ให้รายงานยังอ่านออกหลังย้ายสังกัด · แต่ต้องมี **ใบจองหรือลิงก์อย่างน้อยหนึ่ง** ไม่งั้นทีมเสียงไม่รู้ว่าไฟล์อยู่ไหน
- **ไม่จองช่วงเวลา/ห้อง ไม่มีช่อง "ด่วน"** — มีแค่กำหนดส่ง (มติ operator 2026-09-03) · ช่องด่วนจะถูกติ๊กเหมือนกันหมดแล้วไม่บอกอะไรอีกต่อไป กำหนดส่งจัดลำดับได้จริงกว่า
- **ทุกคนที่ล็อกอินขอได้ · เฉพาะทีมเสียงรับงานได้** — กั้นคนขอด้วย role = ต้องไล่เพิ่มคนทีละคน แล้วคนที่เพิ่มไม่ทันก็กลับไปทักในแชท = คิวว่าง · ส่วนการรับงานกั้นไว้เพราะถ้าใครตั้งตัวเองเป็นคนมิกซ์ได้ ตัวเลขภาระงานทีมเสียงก็เชื่อไม่ได้ · คนขอแก้ของตัวเองได้เฉพาะตอนยังไม่มีใครรับ — เริ่มทำแล้วแก้โจทย์กลางคันคือเปลี่ยนงานที่คนอื่นลงแรงไปแล้วโดยเขาไม่รู้ตัว
- **เห็นคิวทั้งหมด ไม่ใช่เฉพาะของตัวเอง** — คนขอต้องเห็นว่าคิวยาวแค่ไหนก่อนไปรับปากลูกค้า และกฎ "เห็นเฉพาะของตัวเอง" คือคลาสบั๊กที่ทำให้โปรดิวเซอร์มองไม่เห็นงานตัวเอง 59 ใบใน v1.196 — เลี่ยงทั้งคลาส
- `/mix` เข้า `ALWAYS` ใน tiers.ts: ทีมเสียงส่วนใหญ่อยู่ tier `crew` ที่เปิดได้แค่ `/upload` ไม่ใส่ = คนที่หน้านี้สร้างมาเพื่อเขาเข้าไม่ได้เลย (กับดักเดียวกับ `/switcher`) · nav มีทั้งแท็บหลัก (ทีมเสียง) และใน More (คนอื่น) — **หน้าที่ไม่มีทางเข้าคือหน้าที่ยังไม่ได้ปล่อย**
- 21 เคสทดสอบใหม่เขียนจากมุมของแต่ละบทบาท (คนขอ/ทีมเสียง/คนนอก/แอดมิน) รวม 776 เคส · **และเจอบั๊กจากการเปิดหน้าจริง ไม่ใช่จากเทส**: error กับ empty state ขึ้นพร้อมกัน = "โหลดไม่สำเร็จ" อ่านเหมือน "คิวว่าง" ซึ่งคือคลาสบั๊ก PP-26-039 เป๊ะ (งานมี 15 ตอนแต่ขึ้นว่าไม่มี) — แก้ให้ error กินที่ empty state ไปเลย

### Fixed — `restart: always` แทน `unless-stopped`: พรอดดับค้างเพราะบรรทัดเดียว (v1.214)

2026-09-02 ~22:57 มีการสั่งหยุดทั้งโฮสต์ Docker — **21 จาก 23 คอนเทนเนอร์ดับพร้อมกัน** (probook, staging, vaultwarden, helpdesk, tsd-360, thestandard-*, mimir, hermes) กลับมาเองแค่ `portainer` กับ `management-npm` และ **ความต่างเดียวระหว่างสองตัวนั้นกับที่เหลือคือบรรทัด `restart:`**

`unless-stopped` = "restart ตอน daemon ขึ้น **เว้นแต่ถูกสั่งหยุด**" และ docker **จำเจตนานั้นข้ามการรีสตาร์ท daemon** — ฉะนั้นการกดหยุดครั้งเดียวทำให้พรอดค้างดับจนกว่าจะมีคนสังเกตแล้วไปกด Start เอง ซึ่งรอบนี้คือหลายชั่วโมง กลางดึก ไม่มีใครรู้

ลายเซ็นที่ยืนยันว่าเป็นการสั่งหยุด ไม่ใช่เครื่องแครช: แอปทุกตัว exit 0 (ปิดสวย ๆ ตาม SIGTERM) ส่วน DB ทุกตัว exit 137 (ปิดไม่ทันใน grace period เลยโดน SIGKILL) — รูปแบบของ `docker stop` เป๊ะ · ข้อมูลไม่กระทบเลย โวลุ่มไม่ถูกแตะ และนับแถวหลังกลับมาแล้วเท่าเดิมหรือเพิ่มขึ้นทุกตาราง

เปลี่ยนทั้ง `db` / `app` / `worker` เป็น `always` · **นโยบาย restart ตั้งตอนสร้างคอนเทนเนอร์** การแก้ compose จึงมีผลตอน redeploy ครั้งหน้า — คอนเทนเนอร์ที่วิ่งอยู่ถูกสั่งเปลี่ยนแยกด้วย `docker update --restart=always` (ไม่มี downtime) ให้ได้รับการป้องกันทันทีโดยไม่ต้องดับซ้ำ

ราคาที่ต้องรู้: ถ้าจะปิดคอนเทนเนอร์ไว้เพื่อซ่อมบำรุง ห้ามให้ daemon รีสตาร์ทระหว่างนั้น เพราะมันจะปลุกกลับมาเอง

### Fixed — ตัวแปร `MCP_*` ไม่เคยอยู่ใน compose เลย: `/api/mcp` เปิดไม่ได้มาตั้งแต่ v1.49 (v1.213)

PR #19 เพิ่ม `MCP_API_KEYS_READONLY` (key อ่านอย่างเดียวสำหรับบอท) และ CI เขียว แต่ `docker-compose.portainer.yml` **ไม่มีตัวแปร `MCP_` สักตัว** — ทั้ง `MCP_API_KEY`, `MCP_API_KEYS`, `MCP_ACTOR_EMAIL` ที่มีมาตั้งแต่ v1.49 ก็ไม่เคยถูกประกาศ ผลคือ endpoint นี้ **เปิดใช้ไม่ได้เลยมาตลอด**: ตั้งค่าใน Portainer ได้ กด Update ได้ แล้ว `/api/mcp` ยังตอบ 503 พร้อมข้อความที่บอกให้ไปตั้งตัวแปรที่เพิ่งตั้งไปแล้ว

คลาสเดียวกับ `AUTH_SECRET` ข้างบน และเจอด้วยวิธีเดียวกัน คือไปอ่าน compose แทนที่จะเชื่อว่า "PR ผ่าน CI แล้วแปลว่าเปิดใช้ได้" — CI รันเทสของโค้ด ไม่ได้รันเส้นทางที่ค่าเดินทางจาก stack เข้าไปถึง `process.env`

เพิ่มทั้งสี่ตัวใน service `app` ค่าเริ่มต้นว่างทั้งหมด = MCP ยังปิดอยู่เหมือนเดิม (route ตอบ 503 เมื่อไม่มี key) การประกาศเฉย ๆ ไม่เปลี่ยนพฤติกรรมอะไรจนกว่าจะมีคนตั้ง key จริง · คอมเมนต์ในไฟล์ระบุชัดว่า `MCP_API_KEYS` = สิทธิ์เต็ม (จอง/ยกเลิก/ติ๊กจ่ายได้) ส่วน `MCP_API_KEYS_READONLY` = อ่านอย่างเดียว เพราะจากภายนอกสองอันนี้หน้าตาเหมือนกันจนถึงวันที่บอทเขียนอะไรลงไป

### Fixed — `AUTH_SECRET` ถูกตั้งบน stack แต่ compose ไม่เคยส่งเข้า container (v1.213)

ทุก route ใน `/api/internal/*` ประกาศว่ารับ `AUTH_SECRET` เป็น shared secret ได้ (`internal-auth.ts` v1.123 ตั้งใจให้จับคู่กับ **ทุกค่าที่ตั้งไว้** พอดีเพื่อกันปัญหาแบบนี้) แต่ `docker-compose.portainer.yml` ไม่เคยประกาศ `AUTH_SECRET` ใน `environment:` ของ service `app` — ค่าที่โปรเซสอ่านไม่เห็น ไม่ใช่ค่าที่ถูกตั้ง ผลคือ **ทุกคนที่ถือค่านั้นได้ 401 ที่หน้าตาเหมือน "secret ถูกหมุน"**

วัดเมื่อ 2026-08-31: cron `probook-landing-cleanup` ของ Hermes 401 ติดกันหลายวัน (ข้อความเตือนของมันเองเดาว่า "prod หมุน NEXTAUTH_SECRET?") และขั้น 3b ของ nightly check — ขั้นที่เพิ่มมาเพราะเมลแจ้ง footage เคยหายไป 5 สัปดาห์โดยไม่มีใครรู้ — ก็ 401 ด้วยเหตุเดียวกัน คือรูปแบบเดียวกับบั๊ก kill switch v1.164 เป๊ะ ๆ: โค้ดรองรับ operator ตั้งค่าแล้ว compose ทำหล่น

แก้ด้วยการเพิ่ม `AUTH_SECRET: ${AUTH_SECRET:-}` ทั้ง service `app` และ `worker` · **มีผลตอน redeploy stack 125 เท่านั้น** · ถ้าอยากเลิกใช้ค่าเก่าแทน ให้ลบบรรทัดนั้นแล้วเอา `NEXTAUTH_SECRET` ปัจจุบันไปใส่ `~/.hermes/scripts/probook.env`

### Added — แท็บของสวิตเชอร์: บันทึกงานไลฟ์ที่ระบบไม่เคยรู้ว่ามีอยู่ (v1.211)

งานสวิตช์ไลฟ์ลงโซเชียลถูกสั่งกันในกลุ่มไลน์ ไม่เคยผ่าน booking flow เลย ผลคือคำถามพื้นฐานที่สุดตอบไม่ได้สักข้อ — เดือนนี้ทีมสวิตเชอร์คุมไปกี่หมาย กี่ชั่วโมง ใครคุมอันไหน ลิงก์ที่ออกอากาศอยู่ที่ไหน ทุกอย่างอยู่ในหัวคนกับสกรอลล์ของแชท

- **หน้าใหม่ `/switcher`** (เมนู 🎛 สวิตเชอร์) — ฟอร์มให้สวิตเชอร์มาลงเองหลังงานจบ: สังกัด · ชื่อหมาย · วันเวลาทำงาน · ลิงก์ที่ออกอากาศ · ผู้สั่งงาน · หมายเหตุ และ **Production ID ที่ระบบออกให้ในช่องสุดท้าย**
- **Production ID `[OUT]-LIV-[YYMMDD]-[NN]`** เช่น `NWS-LIV-260829-01` — รูปเดียวกับของงานถ่ายจึงแปะในชีท/ชื่อโฟลเดอร์ได้เลย · `LIV` **ไม่ใช่รหัสรายการของ outlet ใดเลย** สายเลขจึงแยกขาดจากงานถ่ายตามนิยาม (มีเทสต์ยืนกฎนี้ไว้ เผื่อวันหนึ่งมีคนเพิ่มรายการรหัส LIV ลง `data.ts`) · ออกเลขใน `pg_advisory_xact_lock` แบบเดียวกับ `create-booking` (v1.146) และ **นับเลขของแถวที่ลบไปแล้วด้วย** เพราะเลขที่ออกไปแล้วอาจถูกก็อปไปแปะที่อื่นแล้ว
- **สังกัด/วันที่ล็อกทันทีที่ออกเลข** — ปล่อยให้แก้ = แถวที่ "เลขบอกวันหนึ่ง ข้อมูลบอกอีกวัน" ซึ่งคือ drift แบบเดียวกับที่โฟลเดอร์ไดรฟ์เคยเป็นก่อน id-first · กรอกผิดให้ลบแล้วเพิ่มใหม่
- **ลิงก์ไม่บังคับตอนบันทึก** เพราะมันมาช้ากว่าตัวงานเสมอ — บังคับกรอกจะได้ลิงก์มั่ว ปล่อยเงียบจะได้ศูนย์ · แทนที่ด้วยกล่อง **"ต้องตามต่อ"** ที่ค้างไว้จนกว่าจะเติม และ **ไม่ผูกกับเดือนที่เลือก** (ของค้างเดือนก่อนคือของที่ต้องตามที่สุด ถ้าซ่อนตามตัวกรองก็ไม่มีใครเห็นมันอีกเลย) · เหตุผลแยกเป็น 3 แบบ ไม่ยุบเป็น boolean เดียว: ยังไม่มีคนรับ / ยังไม่ใส่เวลา / ยังไม่ใส่ลิงก์ — คนละเรื่อง ตามคนละคน
- **สิทธิ์**: สวิตเชอร์เห็นรายการทั้งหมด (จะได้ไม่ลงซ้ำกันเอง) แต่แก้/ลบได้เฉพาะของตัวเอง · console เปิดดู + โหลด CSV · ADMIN/MANAGER แก้ได้ทุกแถว · `/switcher` เข้า `ALWAYS` ใน `tiers.ts` เพราะสวิตเชอร์อยู่ tier `crew` (ไม่งั้น middleware เด้งคนที่ฟีเจอร์นี้สร้างมาเพื่อเขาออกทุกครั้ง) ประตูตัวจริงคือ `switcher/layout.tsx` ที่รู้จัก roster — แพตเทิร์นเดียวกับ `/ot`
- **ตะเข็บของปลายทาง (ดูดหมายจากกลุ่มไลน์)** — `POST /api/internal/switcher/prefill` ตั้งแถว `DRAFT` ไว้ให้สวิตเชอร์กด "รับงาน" แล้วเติมข้อมูล · แถว DRAFT **ยังไม่มี Production ID** โดยตั้งใจ (หมายที่ไม่ได้เกิดจริงไม่ควรกินเลข) · กันซ้ำด้วย `externalKey` (เช่น message id) — ตัวอ่านข้อความยิงซ้ำได้โดยไม่งอกแถว ไม่งั้นรายการ "รอกรอก" จะบวมจนคนเลิกดูแล้วฟีเจอร์ตามงานตายไปด้วย · **ตัวอ่านไลน์ยังไม่ได้ทำ** — endpoint นี้คือฝั่งรับที่ทดสอบด้วย curl ได้เดี๋ยวนี้
- **PATCH ไม่แตะช่องที่ไม่ได้ส่งมา** — เจอตอนทดสอบจริงว่าการยิงแก้เวลาผ่าน curl โดยไม่แนบ `links` ลบลิงก์ทิ้งเงียบ ๆ · ตอนนี้ "ไม่ส่ง = ไม่แตะ, ส่ง `[]` = ตั้งใจล้าง" (ฟอร์มบนเว็บส่งครบทุกช่องอยู่แล้ว จึงยังล้างได้ตามปกติ)
- ลิงก์รับเฉพาะ `http/https` — ค่าพวกนี้ถูกเรนเดอร์เป็น `<a href>` ให้คนอื่นกด · soft delete (ทั้งรีโปไม่มีการลบถาวร) · CSV ต่อเดือนพร้อมคอลัมน์ชั่วโมงทศนิยมสำหรับรวมในชีท
- 26 เคสทดสอบใหม่ (`switcher-jobs.test.ts` 25 + `tiers.test.ts` 1) · รวม 695 เคส · ตรวจจริงบน Postgres: ลำดับเลขต่อ outlet ต่อวัน, ไลฟ์ข้ามคืน, ลบแล้วไม่ใช้เลขซ้ำ, prefill ยิงซ้ำไม่งอกแถว, สวิตเชอร์แก้ของคนอื่นได้ 403, คนที่ไม่ใช่สวิตเชอร์เห็นหน้าอธิบายแทนการถูกเด้งเงียบ ๆ


### Added — 3 สคริปต์เฝ้าระบบย้ายจาก Claude Code มาอยู่ใน repo (`scripts/hermes/`)

งานตรวจตามเวลา 3 ตัวย้ายไป Hermes cron (2026-08-18) แล้วเขียนใหม่เป็น Python stdlib ล้วน — งานคือ "อ่านเลข → ใช้กฎ → รายงาน" ไม่ต้องใช้ LLM และได้ความนิ่งกว่า (เงียบเมื่อปกติกลายเป็นคุณสมบัติของโค้ด ไม่ใช่คำสั่งที่โมเดลจะ drift ได้)

- **`probook-worker-check.py`** — `/api/health-summary` + `/api/version` เตือน stale / app ล่ม / `neverTicked` ซ้ำ
- **`probook-idfirst-monitor.py`** — snapshot 14 วัน เตือน fallback พุ่ง / hit ตก / endpoint ล่ม 2 วันติด
- **`probook-landing-cleanup.py`** — `prune=today` dry-run → apply → verify พร้อมการ์ดห้ามลบโฟลเดอร์ของคิวถ่ายวันหลัง (เคสจริง: รันหลัง 19:00 แล้ว prune ลบ drop folder ของพรุ่งนี้ที่ worker เพิ่งสร้าง)
- `~/.hermes` ไม่มีระบบสำรองและ `pre_update_backup=false` จึงเก็บ**ต้นฉบับ**ไว้ที่นี่ ตัวที่รันจริงคือสำเนาใน `~/.hermes/scripts/` · secret อยู่ที่ `~/.hermes/scripts/probook.env` (600) ไม่ commit เข้า repo

### Fixed — เน็ตเครื่องตัวเองล่ม แล้ว alert ที่บอกว่าล่มก็ส่งไม่ออก

2026-08-19 DNS ของเครื่องนี้ล่ม ~11:03–12:00 รอบ landing-cleanup 12:00 จึงล้ม (`urlopen` Errno 8) และ**ข้อความแจ้งเตือนก็ส่งไม่ออกด้วยเหตุเดียวกัน** เพราะ Discord ก็ resolve ไม่ได้ — alert หายไปพร้อมกับเหตุที่ทำให้ล้ม รอบนั้นจึงไม่มีใครรู้จนมาไล่ดูเอง

- retry ก่อนสรุป 20/60/120 วิ ทั้ง 3 สคริปต์ (dry-run/probe เป็น read-only จึง retry ปลอดภัย) — เน็ตสะดุด 3 นาทีไม่ควรทำให้งานทั้งวันหาย
- `net_down()` resolve cloudflare/google/github ก่อนตัดสิน **แยก "เน็ตเครื่องนี้ล่ม" ออกจาก "prod ล่ม"** → worker-check ไม่ตะโกนว่า prod ตายเวลา wifi งอแง และไม่นับ `appDownStreak` · idfirst ไม่นับ `failStreak` (ไม่งั้นเน็ตงอแง 2 วัน = เตือนผิด) · landing ข้ามรอบแบบบอกเหตุ
- ฝั่งผู้เฝ้า: `probook-nightly-check` (Claude Code) เพิ่มหน้าที่อ่านไฟล์ผลล่าสุดของทั้ง 3 job และ grep log หา `delivery error` ย้อน 24 ชม. → alert ที่ส่งไม่ออกจะไม่หายเงียบอีก
- 22 เคสทดสอบ (regression 13 + ใหม่ 9) รวมเคสแยกเน็ตเรา/prod ทั้งสองทิศ

### Added — log scan ของ Portainer กลับมา (headless ผ่าน API token)

step 2 ของ routine เดิมหายไปตอนย้ายมา Hermes เพราะมันยืม session Chrome ที่ล็อกอินไว้ Hermes ขับบราวเซอร์ของตัวเองจึงยืมไม่ได้ → เปลี่ยนไปใช้ Portainer access token (`X-API-Key`) ยิง `GET /api/endpoints/<id>/docker/containers/<name>/logs` ตรง ๆ ไม่ต้องมีบราวเซอร์เลย

- จับเคสที่ `health-summary` มองไม่เห็น: worker ที่ยิง HTTP ล้มทุกรอบแต่งานข้างล่างยังเสร็จ (แบบ `[sound-merge] run failed: fetch failed` 48/48 รอบก่อน v1.172) + worker ที่ enabled แต่ 24 ชม. ไม่มีร่องรอยใน log เลย = supervisor ไม่ได้รันสคริปต์
- ไม่มี token = ข้าม step เงียบ ๆ · token เสีย/403/404 = รายงานให้รู้ ไม่เงียบหาย · ยิงแค่ GET ไม่เขียนอะไร
- **false positive รอบแรกที่มี token**: กล่าวหา backup / calendar-reconcile / shoot-marker ว่าตาย ทั้งที่ log มีบรรทัดของพวกมันชัด ๆ — `OK_RE` นับแต่รูปแบบสรุป (`bookings=|checked=|scanned=`) ส่วน worker รายวันพิมพ์บรรทัดเดียวและใช้คำคนละแบบ · เลิกใช้ `OK_RE` ทิ้งไปเลย → **บรรทัดใดก็ได้ของ tag นั้นที่ไม่ใช่ SKIP = ร่องรอยว่ายังมีชีวิต** และ "ไม่มีร่องรอยใน log" เตือนเฉพาะเมื่อ health-summary ก็บอกว่าไม่ tick เกิน 24 ชม./ไม่เคย tick (tick ปกติแต่เงียบใน log = worker รายวันยังไม่ถึงคิว ไม่ใช่ปัญหา)
- ลอก byte framing ของ docker (`\x00-\x1f` + `U+FFFD` จาก decode) ออกจากหัวบรรทัดตัวอย่าง · เทสต์ใช้ corpus จาก log prod จริง 21 เคส (รวม false positive ทั้งสองทิศ)

### Added — เฝ้า launchd NAS agent, เตือนก่อนงวดไดรฟ์ฟุตเทจ, และบอกว่า error ยังเกิดอยู่ไหม

ทำตามรายงานระบบข้างเคียงที่ปุ๊กส่งมา (19 ส.ค.) — 4 ใน 6 เรื่องเป็นโรคเดียวกับเรา: "ระบบบอกว่าเรียบร้อยทั้งที่ไม่เรียบร้อย หรือบอกว่าพังทั้งที่ไม่พัง"

- **launchd NAS agent** — กลุ่มงานตั้งเวลาที่ไม่มีใครตรวจเลย พบว่ามันเด้งทุก 10 นาทีแล้วขึ้น "not mounted — skip" ติดกัน **205 รอบ (~56 ชม.)** เพราะ NAS ไม่ได้ mount ซึ่งเป็นพฤติกรรมที่ออกแบบไว้ แต่ไม่มีใครรู้ว่ามันเงียบมานานแค่ไหน → แจ้งเมื่อ agent ไม่เด้ง/ไม่ mount เกิน 48 ชม. + ก็อป log ออกจาก `/tmp` (หายทุกครั้งที่รีบูต) มาเก็บถาวร
- **งวดไดรฟ์ฟุตเทจ** — `DRIVE_FOOTAGE_ROOT` ชี้ "VIDEO 2026 [JUL–DEC]" ที่หมุนด้วยมือ ไม่มีโค้ดเทียบกับปฏิทิน ขึ้นงวดใหม่แล้วไม่หมุน = เขียนกล่องลงไดรฟ์เก่าเงียบ ๆ → เตือนล่วงหน้าเป็นช่วง (≤14/7/3/1 วัน ช่วงละครั้ง) ยืนยันด้วย `--ack-footage-root`
- **สถานะ error** — ทุก error ที่รายงานบอกว่า 🔴 ยังเกิดอยู่ หรือ ✅ หายแล้ว (+นับรอบที่ปกติตามหลัง) — เตือนเรื่องที่จบไปแล้วคือการรบกวนคนด้วยปัญหาที่ไม่มีอยู่
- แก้ 2 บั๊กที่เจอตอนรันจริง: framing byte ของ docker บางตัว decode เป็นตัวอักษรที่มองเห็นได้ (W/C/?) ทำให้ parse timestamp พลาด → ใช้ `search` แทน `match` · และ `route error` ไม่ถูกนับเป็น error
- 60 เคสทดสอบ (regression 22 + log scan 21 + ใหม่ 17) · nightly-check ฝั่ง Claude Code อัปเดตให้ตรวจครบทั้ง **5 กลุ่มงานตั้งเวลา** (เพิ่ม launchd + GitHub Actions) และเช็คว่า `/api/projects` ยังผูกกับแท็บ "All Projects" ได้อยู่

---

## [1.192.0] — 2026-08-24

### Fixed — ร่าง OT ของเดือนอนาคตถูกลบทิ้งทุกครั้งที่มีใครเปิดหน้า /ot

นโยบายเก็บ OT เขียนไว้ชัดว่า *"Older than that: deleted"* แต่โค้ดเขียนเป็น `month: { notIn: keep }` ซึ่ง **ลบเดือนอนาคตไปด้วย** — และ `cleanupOTRecords()` รันแบบ lazy ที่ `GET /api/ot` คือ **ทุกครั้งที่มีใครเปิดหน้า OT**

ร่าง OT ถูกสร้างอัตโนมัติจากคิวถ่าย ซึ่ง**ส่วนใหญ่เป็นวันในอนาคต** ผลคือร่างของเดือนถัดไปถูกลบทิ้งเงียบ ๆ ตลอดมา และไม่มีใครรู้เพราะไม่เคยมีใครเปิดหน้า OT (ทั้งเดือน ส.ค. เปิด 0 ครั้ง)

**เจอเพราะทำข้อมูลหายเอง**: 2026-08-24 เปิดหน้า `/ot` 3 ครั้งเพื่อทดสอบตัวเก็บสถิติ (v1.190) แล้ว **ร่างเดือน ก.ย. 33 ใบหายทันที** — ซึ่งคือเดือนที่ pilot จะใช้พอดี · ตัวเลขไม่ตรง (551 → 513 ก่อนการล้างกล่องกลาง 203 ใบ) คือร่องรอยที่ทำให้ไล่เจอ

- แยก `otCleanupCutoff(todayBangkok)` ออกมาเป็นฟังก์ชันบริสุทธิ์ — เดือนที่เก่าที่สุดที่ยังเก็บ ทุกอย่างที่ **เก่ากว่านี้** เท่านั้นที่ถูกลบ · `deleteMany({ month: { lt: cutoff } })`
- **กฏนี้ไม่เคยถูกเทสมาก่อน** — เพิ่ม 5 เคส รวมเคสที่ทำข้อมูลหายจริง (เดือนอนาคตต้องรอด), ข้ามปี, และ zero-pad (ถ้าไม่ pad `'2026-9' > '2026-10'` ตามลำดับสตริง = ลบ ต.ค. ทิ้ง)
- รวม **609 ผ่าน**

> บทเรียนเดียวกับที่เจอซ้ำ ๆ ในรอบนี้: **โค้ดกับคอมเมนต์ข้าง ๆ พูดคนละเรื่อง แล้วไม่มีเทสมาตัดสินว่าอันไหนถูก** — คอมเมนต์บอก "ลบของเก่า" โค้ดทำ "ลบทุกอย่างที่ไม่ใช่เดือนนี้"

## [1.191.0] — 2026-08-24

### Fixed — ร่าง OT 40% ตกอยู่กับกล่องอีเมลกลางที่ไม่มีเจ้าของ

พบตอนอ่าน funnel ที่ v1.190 เพิ่งเปิดใช้: สองอันดับแรกของ "คนที่มีร่าง OT มากที่สุด" **ไม่ใช่คน** — `video@thestandard.co` 113 ใบ และ `sound@thestandard.co` 90 ใบ รวม **203 จาก 513 ใบ (40%)**

`Booking.assignedEmails` ใส่ได้ทั้งคนจริงและ "ทีม" ซึ่งสะดวกตอนจัดคิว (หมายถึงทีมวิดีโอไปกันเอง) แต่ `syncBookingOT` ตีความว่าเป็นบุคคลแล้วสร้างร่างให้ — ร่างที่**ไม่มีใครเป็นเจ้าของ ไม่มีใครกดส่ง ไม่มีใครเซ็น** และจะดึงตัวเลข funnel ของ pilot สิ้นเดือนให้ดูแย่กว่าความจริง

- `src/lib/shared-mailboxes.ts` — รายชื่อกล่องกลาง (`video@`, `sound@`, `photo@`, `podcast@`, `event@`, `webmaster@`) ยืนยันจากข้อมูลจริงบนพรอด ทุกตัวชื่อ "<ทีม> THE STANDARD" และไม่มี position
- **ใช้รายชื่อชัดเจน ไม่ใช้ heuristic** — เคยจะเดาจาก "local-part ไม่มีจุด" (คนใช้ firstname.lastname) แต่ตรวจข้อมูลจริงก่อนแล้วเจอ `bickboon@thestandard.co` ซึ่ง**เป็นคนจริง**และไม่มีจุด → เดาผิดแล้วตัดคนจริงออกจาก OT เงียบ ๆ
- `syncBookingOT` กรองกล่องกลางออกก่อนสร้างร่าง · ฟังก์ชันนี้ลบ-แล้ว-สร้างใหม่อยู่แล้ว คิวที่ถูก sync อีกครั้งจึงสะอาดเอง
- **จงใจไม่ตัดออกจากการแจ้งข่าว** — กล่องกลางคือช่องที่ทีมอ่านจริง ถ้าตัดจากเมลแจ้งฟุตเทจด้วยทีมจะไม่ได้ยิน (footage-ready จงใจเก็บ `video@`/`sound@` ไว้) คอมเมนต์ในไฟล์เขียนกำกับไว้ไม่ให้ใครเผลอเอาไปใช้ผิดที่
- ปรับได้ไม่ต้อง deploy: `SHARED_MAILBOXES="a@x.co,b@x.co"` · ค่าพิการตกกลับเป็น default ไม่ใช่ตัดทุกคนออก
- 7 เคสทดสอบใหม่ รวม **604 ผ่าน**

## [1.190.0] — 2026-08-24

### Added — เก็บสถิติการใช้งานแบบแคบ + รายงาน funnel ของ OT

operator: *"ทำ analytics ก่อนเลย ทันสิ้นเดือน"* — pilot OT เริ่มสิ้นเดือน ส.ค. และ **ข้อมูลที่ไม่ได้เก็บตอน pilot คือข้อมูลที่หายถาวร**

**ปัญหาที่แก้**: ระบบมีฟีเจอร์ที่ข้อมูลพร้อมแต่ไม่มีใครใช้หลายตัว (OT 551 ใบ/ส่ง 0 · ปุ่มส่งงานไม่เคยถูกกด · อัปโหลดมีคนใช้คนเดียวแล้วหยุด 14 ก.ค.) และทุกครั้งที่ถามว่า "ทำไมไม่มีใครใช้" เราตอบไม่ได้ เพราะ `audit_logs` บอกได้แค่ *"ลงมือทำอะไรบ้าง"* ไม่ได้บอก *"เปิดดูแล้วเงียบไป"* — ซึ่ง **ไม่รู้ว่ามี** กับ **เข้าไปแล้วยอมแพ้** แก้คนละทางกันสิ้นเชิง

**จงใจไม่ใช้ GA/PostHog**: ระบบมีผู้ใช้ 86 คน อยู่หลังล็อกอิน คำถามที่ analytics เชิงพาณิชย์ตอบเก่ง (traffic source / bounce / device) ไม่ใช่คำถามของเรา แลกมาด้วย tracker ของบุคคลที่สาม + เรื่อง privacy ของพนักงาน + ของอีกชิ้นที่ต้องดูแล

- **`PageEvent`** — เก็บแค่ `email + path + at` · **ไม่เก็บ** IP / user-agent / referrer / query string · path ถูกตัด id ออก (`/admin/<id>` → `/admin`) เพราะคำถามคือ "คนรู้จักหน้านี้ไหม" ไม่ใช่ "ใครดูงานใบไหน" (อย่างหลังอยู่ใน audit อยู่แล้วและมีกฏการมองเห็นคุม)
- **allowlist 6 หน้า** (`/ot`, `/ot/admin`, `/new`, `/upload`, `/my-bookings`, `/admin`) — เก็บเฉพาะหน้าที่มีคำถามค้างอยู่จริง ไม่ใช่เก็บหมดแล้วค่อยกรอง · **ข้อมูลที่ไม่มีใครตั้งใจจะอ่านคือภาระ ไม่ใช่สินทรัพย์**
- **แยกจาก `audit_logs` โดยเจตนา** — audit คือหลักฐาน "ใครลงมือทำอะไร" และมีกฏการมองเห็น (`booking-history-visibility`) เอา page view ไปปนจะทำให้ตารางบวมและความหมายเบลอ
- ตัวตนมาจาก session เท่านั้น — **ไม่เชื่อค่าที่ client ส่ง** (ส่งได้แค่ path) และ allowlist ถูกตรวจซ้ำฝั่ง server เสมอ
- กันแถวเฟ้อ: กด refresh/re-render ซ้ำภายใน **30 นาที** นับเป็นการเข้าครั้งเดียว
- ล้มเงียบโดยเจตนา — สถิติพังต้องไม่ทำให้หน้าเว็บพัง · kill switch `PAGE_EVENTS_ENABLED=0`

**`GET /api/internal/usage?month=YYYY-MM`** (read-only, secret หรือ admin session) — คืน:
- `pages` — หน้าไหนถูกเปิดกี่ครั้ง กี่คน
- `ot.funnel` — **นับเป็นจำนวนคน** ไม่ใช่จำนวนใบ เพราะคำถามคือคนติดตรงไหน: `hasDraft` → `openedPage` → `edited` → `submitted` → `approved`
- `ot.perPerson` — ไล่ตามได้จริงว่าใครติดขั้นไหน · `openedButNoDraft` — คนที่สนใจแต่ไม่มีอะไรให้ทำ
- 9 เคสทดสอบใหม่ (`page-events`) รวม **597 ผ่าน**
...
คำสั่ง operator: *"งาน AD Required Agency ref ทุกครั้ง ทุกบ้าน เขียนไว้ใต้ช่องว่า หากยังไม่ทราบให้ใส่ QU-1234TBC · เวลาตรวจจับให้จับตามเงื่อนไข แต่ต้องเผื่อเวลาเขาใส่ผิดด้วย"*

- **เลิกจำกัดเฉพาะ AGN** — กฏ QU เดิม (v1.161) บังคับแค่ `outlet=AGN` ตอนนี้ทุก outlet ที่ booking เป็น `ADVERTORIAL` ต้องมี Agency Ref ทั้ง create / PATCH ของแอดมิน / producer-edit · ของจริงตอนเปลี่ยน: 23 ใบใน 10 บ้านที่ยังไม่มีเลข (TSS 6, NWS 3, EVT 5, POD 2, …) แต่บ้านใหญ่ใส่เลขจริงอยู่แล้ว (TSS 27/33) จึงไม่กระทบคนส่วนใหญ่
- **ตัวยึดที่ประกาศเปลี่ยนจาก `1234` → `QU-1234TBC`** — ประกาศสิ่งที่ทีมคิดขึ้นเองและใช้กันอยู่แล้วให้ถูกกฏ ดีกว่าบังคับให้จำค่าใหม่ · `1234` เดิมยังใช้ได้ (มีบนพรอดแล้ว)
- **ตัวจับทนการพิมพ์ผิด** (`isQuPending`): ตัดขีด/ช่องว่าง/จุด แล้ว uppercase → นับเป็นตัวยึดเมื่อ **มี `TBC` อยู่ที่ไหนก็ได้** (`QU-1234TBC`, `1234TBC`, `TBC`, `QU-4480TBC`, `qu 1234 tbc`) **หรือ** เป็น `1234`/`QU1234` (ตก TBC)
  > **การแลกที่รู้ตัว**: v1.183 เคยกัน `QU-1234` ไว้เพราะอาจเป็นเลขจริง ตอนนี้เหมาเป็นตัวยึด — พลาดจับของจริงคือเตือนเกินแล้วแก้กลับ (เสียงรบกวน) ส่วนพลาดไม่จับตัวยึดคืองานไม่ถูกตั้งเบิกโดยไม่มีใครรู้ (เงียบและแพงกว่า)
- **ข้อความใต้ช่องกรอก** (`QU_FIELD_HINT`) ใช้ตัวเดียวกันทั้งฟอร์มจอง หน้าแก้ของเจ้าของงาน และหน้าโหมด Confirmed/Completed · ฟอร์มจองเปลี่ยนป้ายจาก `(optional)` เป็น **required** พร้อมตรวจตั้งแต่ในฟอร์ม (predicate ตัวเดียวกับ server)
- ผลพลอยได้: `TBC` เปล่า ๆ บน `AGN-260721-01` เดิม**แก้ไขไม่ได้เลย**เพราะตกกฏ ตอนนี้รับเป็นตัวยึด → เจ้าของงานแก้ต่อได้

### Added — เจ้าของงานแก้ Agency Ref ได้หลังอนุมัติและหลังถ่ายจบ

คำสั่ง operator: *"ให้เจ้าของงานเข้าแก้ไขรายละเอียด Agency ref ได้ภายหลังด้วย"* — เลข QU มักมาจากฝ่ายจัดซื้อ/ลูกค้า **หลังถ่ายเสร็จ** ถ้าล็อกไว้ เจ้าของงานก็ไม่มีทางเติมเลขจริงได้เลย แล้วบอทเตือน (v1.187) จะจี้ไปตลอดกาลโดยไม่มีทางออก

| สถานะ | เดิม | v1.188 |
|---|---|---|
| REQUESTED | ทุกฟิลด์ | ทุกฟิลด์ (เหมือนเดิม) |
| CONFIRMED | สถานที่อย่างเดียว | สถานที่ **+ Agency Ref** |
| COMPLETED | แก้ไม่ได้เลย | **Agency Ref อย่างเดียว** (หน้าเล็กเฉพาะกิจ) |
| CANCELLED | แก้ไม่ได้ | แก้ไม่ได้ (งานเลิกแล้วไม่ต้องตั้งเบิก) |

- ป้ายในเมลแจ้งแอดมินเปลี่ยนตามโหมดจริง — เดิมเขียน "แก้ไขสถานที่" ทั้งที่คนแก้เลข QU
- บอทเตือน (v1.187) ขยายขอบเขตตามกฏใหม่: ADVERTORIAL ทุก outlet
- แก้เทส 5 เคสที่ล็อกกฏเดิมไว้ให้ล็อกกฏใหม่แทน รวม **586 ผ่าน**

## [1.187.0] — 2026-08-23

### Added — บอทเตือน Producer ให้กลับมาใส่เลข QU จริง

คำสั่ง operator: *"ให้บอทส่งแจ้งเตือน Producer ที่ใส่เลข QU-1234, 1234TBC หรือเว้นว่างไว้ ให้กลับมาใส่ QU ที่ถูกต้องด้วย"* — ช่องที่พูดถึงคือ `Booking.agencyRef` (Agency Ref / Product Code ตอนจอง) ไม่ใช่ `quoteNo` ของงานเช่า

**ขอบเขต**: AGN + ADVERTORIAL ที่ยังไม่ยกเลิก และ `agencyRef` เว้นว่าง หรือเป็นตัวยึด (`1234` / `QU-xxxxTBC` — ใช้ `isQuPending` ตัวเดียวกับกฏตอนจอง) · **รวมงานที่ถ่ายจบไปแล้ว** เพราะเลขนี้ใช้ตั้งเบิก ถ่ายเสร็จแล้วก็ยังต้องได้ · ของจริงตอนทำ: 4 ใบ 2 Producer

**จังหวะ** (ตามที่ operator เลือก): สัปดาห์ละครั้ง · **เร่งเป็นทุกวันเมื่อวันถ่ายเหลือ ≤ 3 วัน** · งานที่ถ่ายผ่านไปแล้วไม่นับว่าเร่ง (ยังต้องได้เลข แต่ไม่มีเส้นตายกองถ่าย) · หนึ่ง Producer ได้ **ฉบับเดียวรวมทุกใบที่ค้าง** ไม่ใช่ฉบับละใบ — ใบไหนใบหนึ่งถึงกำหนดก็ส่งทั้งชุด กันคนเดียวได้หลายฉบับกระจายคนละวัน
> เลข QU มาจากฝ่ายจัดซื้อ/ลูกค้า ไม่ได้อยู่ในมือ Producer การเตือนทุกวันตั้งแต่วันแรกคือสแปมที่คนจะเลิกอ่าน แล้วพอถึงใบที่เร่งจริงจะไม่มีใครสนใจ

- **เกาะรอบ reminders worker ที่มีอยู่** (รันทุกวัน มี heartbeat/dead-man แล้ว) แทนการตั้ง worker ตัวที่ 13 · sweep ล้มไม่ทำให้ reminders ทั้งรอบพัง และขึ้น log พร้อม `qu: pending= emailed= failed=` ไม่ทำงานเงียบ
- เพิ่ม `ReminderType.AGENCY_REF_MISSING` → เงื่อนไขนี้โผล่ใน `/admin/reminders` และ **auto-resolve เองเมื่อ Producer ใส่เลขจริง** ไม่ต้องมีใครไปกดปิด
- **ประทับ `quRemindedAt` เฉพาะตอนเมลส่งผ่านจริง** — ส่งไม่ผ่าน = รอบหน้าหยิบมาใหม่ ไม่ใช่ทำเครื่องหมายว่าเตือนแล้วทั้งที่ไม่มีใครได้อ่าน (บทเรียน v1.186 ที่ audit อ้างว่าส่งถึง 85/85 ครั้ง) · audit `booking.qu_reminder_sent` เก็บ `emailOk` + `emailError` จริง
- ใบที่ **ไม่มีอีเมล producer** เตือนไม่ได้ → รายงานแยกใน `noProducerEmail` และติดไปกับสรุป Discord ของ operator ไม่ปล่อยหายเงียบ
- `dryRun` พรีวิวได้ว่าจะส่งถึงใคร หัวเรื่องอะไร ใบไหนบ้าง โดยไม่ส่งและไม่ประทับเวลา — **ทางเดียวที่จะรู้ก่อนหน้านี้คือ "ส่งจริงแล้วดู" ซึ่งย้อนไม่ได้**
- Kill switch `QU_REMINDER_ENABLED=0` · ปรับจังหวะ `QU_REMINDER_INTERVAL_DAYS` (7) / `QU_REMINDER_URGENT_DAYS` (3)
- 12 เคสทดสอบใหม่ (`qu-reminder`) รวม **584 ผ่าน**

**หมายเหตุออกแบบ — ทำไม `1234` ปลอดภัยกว่าเว้นว่าง**: เว้นว่างแยกไม่ออกจาก "งานนี้ไม่ต้องใช้เลข" หรือ "ข้อมูลหาย" ส่วน `1234` คือเจตนาที่เครื่องอ่านได้ (badge ⏳ / audit / ค้นเจอ) · และตัวที่แย่กว่าทั้งคู่คือ**เลขที่แต่งขึ้นให้ดูเหมือนจริง** ซึ่งจับไม่ได้เลย — ร่องรอยบนพรอดคือ `QU-1234TBC` 2 ใบ กับ `TBC` 1 ใบ ที่เกิดตอนกฏบล็อกโดยไม่มีทางออกที่ถูกต้องให้
...
`reconcileCalendarGuests` ใช้ `orderBy shootDate asc` + `take: limit` และ worker route ส่ง `limit=50` — **ลำดับคงที่ทุกรอบ** ฉะนั้นถ้าคิว CONFIRMED (ที่มีครู) ยาวเกิน 50 ใบ งานที่ถ่ายไกลกว่านั้น**ไม่ใช่ "รอบหน้าค่อยถึง" แต่ไม่มีวันถึง** ปฏิทินเพี้ยนก็ค้างไปเรื่อย ๆ เงียบ ๆ

เจอตอนไล่ v1.185: prod มี CONFIRMED+มีครู **62 ใบ** → แถวที่ 51-62 (งาน 22 ก.ย. – 11 ธ.ค.) ไม่ถูกแตะเลย ต้องยิงมือ `?limit=200` ถึงจะได้ Co-Producer เข้าปฏิทิน

- worker default `limit` 50 → **200** (เท่าเพดานที่ `reconcileCalendarGuests` บังคับอยู่แล้ว)
- **NO SILENT CAPS**: ชนเพดานแล้วตั้ง `truncated: true` ในผลลัพธ์ + `console.warn` บอกตรง ๆ ว่า "งานที่ไกลกว่านี้ยังไม่ถูกตรวจ และลำดับคงที่จึงไม่ใช่ว่ารอบหน้าจะถึง" — บทเรียนเดิมของรีโปนี้: การตัดจำนวนแบบเงียบ ๆ อ่านเหมือน "ตรวจครบแล้ว"
- ถ้าคิวโตเกิน 200 ในอนาคต ตอนนี้จะมีเสียงเตือนใน log แทนที่จะเงียบหาย
...
คำสั่ง operator: แอดมินต้องรู้เมื่อมีงานขอแก้ไข/ขอยกเลิกเข้ามา เป็น activity เกี่ยวกับคิว โดยเรียงความสำคัญ **ขอยกเลิก → ขอแก้ไข → error** ส่วนเจ้าของงานได้ noti ผลของงานตัวเองกลับไป

**ไม่มีตาราง notification** — feed derive จากสถานะจริงทุกครั้ง: คิวยกเลิกอ่าน `Booking.cancelRequestedAt` (คอลัมน์เดียวกับแท็บ "ขอยกเลิก"), คิวแก้ไขอ่าน audit log, error อ่าน `evaluateWorkers()` ตัวเดียวกับ `/admin/health` · ตัวเลขบนกระดิ่งจึงไม่มีทางไม่ตรงกับคิว สิ่งเดียวที่เก็บเพิ่มคือตาราง `notification_seen` (email → เปิดล่าสุดเมื่อไหร่) แยกจาก `User` เพราะคอลัมน์ใน User มีความหมายว่า "ใครเป็นสตาฟ/อยู่ dropdown" ไม่ควรผสม state ของ UI

- **ปิดช่องที่มองไม่เห็นมาตลอด**: คำขอแก้เวลา/อัปเดตจาก Producer (`/producer` → `booking.time_change_request` / `booking.producer_update`) เข้าแค่ audit log + เมลกล่องเดียว (`PRODUCER_UPDATE_NOTIFY_EMAIL`, default คนเดียว) — **ไม่มีที่ไหนบนหน้าจอเห็นเลย** ใครไม่เปิดเมลก็ไม่รู้ ตอนนี้ขึ้นกระดิ่ง
- ขอบเขตตัดสินฝั่ง server (`notificationScopes`, ฟังก์ชันบริสุทธิ์ + เทส): คิวยกเลิก/คิวแก้ไข = ทุก role ที่มี console (ของที่เขาเปิด /admin เห็นอยู่แล้ว) · error ระบบ = tier admin เท่านั้น (เรื่อง infra ไม่ใช่คิวงาน) · role พิการ/ปลอมได้สิทธิ์แคบสุด
- ฝั่งเจ้าของงาน **fail-closed แบบเดียวกับ `booking-history-visibility`**: เห็นเฉพาะ booking ที่ตัวเองเป็นเจ้าของ (createdByEmail หรือ producerEmail, เทียบ case-insensitive) × action ที่อยู่ใน allowlist × **ไม่ใช่สิ่งที่ตัวเองทำ** · action ของ peer review ไม่อยู่ใน allowlist และมีเทสยืนยันว่าไม่มี `review.*` ปนเข้ามา (ฟีเจอร์นั้นตั้งใจไม่ระบุตัวตน)
- `booking.update` โชว์แค่ **ชื่อฟิลด์** ที่ถูกแก้ ("แก้: เวลาเริ่ม, สถานที่") ไม่โชว์ค่า · เคสพิเศษ `cancelRequestedAt → null` อ่านเป็น "คำขอยกเลิกถูกปฏิเสธ — เก็บงานไว้" ไม่ใช่ชื่อคอลัมน์ดิบ
- **worker ที่ไม่เคย tick ไม่นับเป็น "ยังไม่ได้ดู"** (โชว์ในลิสต์ แต่ไม่ดันตัวเลข): มันเป็นสภาพค้าง ไม่ใช่ของใหม่ ถ้านับด้วย badge จะไม่มีวันเป็น 0 แล้วคนจะเลิกมองกระดิ่ง — โรคเดียวกับ alert ที่เขียวอยู่ 5 สัปดาห์แล้วไม่มีใครอ่าน · ส่วน worker ที่ "ค้าง" ใช้เวลาที่มันเริ่มค้าง (tick ล่าสุด + เกณฑ์) เป็น timestamp ไม่ใช่ `now` ไม่งั้นทุกครั้งที่โหลดหน้าจะนับเป็นของใหม่
- "โหลดไม่ได้" กับ "ไม่มีข่าว" แยกกัน: fetch ล้มโชว์จุดเทา + ปุ่มลองใหม่ ไม่ใช่โชว์ 0 เงียบ ๆ

### Fixed — บั๊ก 2 ตัวที่เจอตอนทดสอบกระดิ่งบนจอจริง

- **Nav render กระดิ่ง 2 ตัว** (cluster desktop + cluster mobile ซ่อนด้วย CSS) ทั้งคู่ mount จริงและ poll คนละรอบ → ผู้ใช้ 1 คนยิง `GET /api/notifications` สองครั้งทุก 90 วิ (ครั้งละ 4-5 query) แก้ด้วยสโตร์ที่แชร์กัน: 2 กระดิ่ง = **1 request** (วัดจาก `performance.getEntriesByType`) และเลขสองที่ตรงกันเสมอ · poll เดินเฉพาะตอนแท็บถูกมองอยู่
- **panel ถูกตัดหายบนมือถือ**: `absolute right-0` ผูกขอบขวาไว้กับกระดิ่ง ซึ่งบนมือถือ**ไม่ได้อยู่ริมขวา** (ปุ่ม + และแฮมเบอร์เกอร์อยู่ขวากว่า) → ขอบซ้ายไหลออกนอกจอที่ **-55px** ตัวหนังสือด้านซ้ายหายไปเลยที่ 375px แก้เป็นวางตำแหน่งจาก rect ของปุ่มแล้ว clamp เข้ากรอบจอ (ไม่ hardcode ความสูง nav) — วัดหลังแก้: 375px ได้ [12, 352] · 1280px ขอบขวาชนปุ่มพอดี ทั้งสองอย่างอยู่ในจอครบ

- 14 เคสทดสอบใหม่ (`notification-kinds`) รวม **561 ผ่าน**

## [1.183.0] — 2026-08-20

### Added — งานที่ยังไม่มีเลข QU ใส่ "1234" ไว้ก่อนได้

กฎ QU ของ v1.161 ตีกลับทุกอย่างที่ไม่ใช่ `QU-xxxx` ซึ่งถูกสำหรับงานที่มีใบเสนอราคาแล้ว แต่งานจริงบางใบ *ยังไม่มีเลข* ตอนจอง — ทางออกที่คนจะทำเองคือพิมพ์ `QU-1234` มั่ว ๆ ให้ผ่าน ซึ่งแยกไม่ออกจากเลขจริงและไม่มีใครกลับมาแก้

- ตัวยึด = **`1234` เปล่า ๆ เท่านั้น** — จงใจไม่นับ `QU-1234` ด้วย เพราะรูปแบบนั้นเป็นเลขใบเสนอราคาที่ถูกต้อง ถ้าเหมาเป็น placeholder แล้ววันหนึ่งมีใบจริงเลขนี้ เราจะทับของจริงโดยไม่มีใครรู้ · `1234` ตกกฎ QU อยู่แล้วจึงชนกับเลขจริงไม่ได้ และเก็บลง DB ตามที่พิมพ์ (ค้น Agency Ref = เจอ)
- ผ่านได้ทั้ง create, PATCH ของแอดมิน และ producer-edit — ไม่งั้นใบที่ค้างตัวยึดจะแก้ไม่ได้เลย · ข้อความตีกลับตอนนี้**บอกทางออก** ไม่ใช่แค่บอกว่าผิด
- ใส่ `1234` มาแต่โปรเจกต์เดียวกันเคยมีเลข QU จริงในระบบ → ใช้เลขจริงแทน (เจตนาของคนพิมพ์คือ "ฉันไม่มีเลข" จึงเป็นพฤติกรรมเดียวกับเว้นว่าง)
- มองเห็นว่ายังค้าง: badge `⏳ ยังไม่มีเลข QU` ที่หน้า `/admin/<id>` และหน้าแก้ไขของโปรดิวเซอร์ · audit `booking.create` ติด `agencyRefPending` ไว้ · `AGENCY_REF_QU_RULE=0` ยังปิดกฎทั้งหมดได้เหมือนเดิม
- predicate ย้ายมาไฟล์ `qu-ref.ts` ที่ไม่ import prisma → **ฟอร์มจองใช้กฎตัวเดียวกับ server** ไม่มีวันที่ฟอร์มบอกผ่านแต่ server ตีกลับ (เดิมฟอร์มมี regex ของตัวเอง)
- **ปิดรูเก่าไปด้วย**: `QU-xxxxTBC` (ตัวยึดที่ทีมคิดเองก่อนจะมีข้อตกลงนี้ — มีจริงบนพรอด `PP-26-047` / AGN-260824-01,-02) ผ่าน regex จึงเคยถูกนับเป็น "เลขจริง" แล้ว `pullQuRefFromProject()` **ลากไปใส่คิวถัดไปของโปรเจกต์เดียวกันเงียบ ๆ** · ตอนนี้จัดเป็นตัวยึด → หยุดแพร่เอง และขึ้น badge ให้เห็นว่ายังค้าง (ค่าที่เก็บไว้ไม่ถูกแตะ แก้ไขต่อได้ปกติ) · `TBC` เปล่า ๆ ยังไม่ผ่านเหมือนเดิม — รับรู้ของที่มีอยู่ ไม่ได้เปิดรับรูปแบบใหม่

### Added — งาน TSS ทุกใบมี "แก้ว" เป็น Co-Producer ในคิวให้เอง

คำสั่ง operator: งานของ TSS หลังจากนี้ต้องมีแก้ว (Co-Producer TSS) ติดอยู่ในคิวทุกใบ โดยคนจองไม่ต้องเลือกเอง

- เติมฝั่ง server ตอน create แบบเดียวกับ VP auto-assign — **ไม่ preselect ใน dropdown** เพราะ validation "เลือก Producer หรือ Co-Producer อย่างน้อย 1 คน" จะผ่านเองโดยไม่มี Producer เลย
- **เคารพคนกรอกก่อนเสมอ**: เติมเฉพาะตอนช่อง Co-Producer ว่างจริง ๆ · ถ้าแก้วเป็น Producer ของใบนั้นอยู่แล้ว ไม่ใส่ซ้ำ · outlet อื่นไม่กระทบ
- ปิดสวิตช์ `AUTO_COPRODUCER=0` · เปลี่ยนคน/เพิ่ม outlet ด้วย `AUTO_COPRODUCER_<CODE>="email|ชื่อเล่น"` ไม่ต้อง deploy
- audit `booking.create` ติด `coProducerAuto` เมื่อระบบเป็นคนใส่ — เวลามีคนถามว่า "ทำไมชื่อแก้วโผล่มา" ตอบได้จาก audit ไม่ต้องเดา

### Fixed — `coProducerEmail` มีคอลัมน์มาตั้งแต่ v1.59 แต่ว่างทั้งตาราง

ฟอร์มจองคำนวณ `effCoProducerEmail` ไว้แล้ว**ไม่เคยส่งมากับ payload** และฝั่ง create ก็ destructure ทิ้งไว้เฉย ๆ → คิวเก็บได้แค่ชื่อเล่นของ Co-Producer ตามตัวคนไม่ได้ (ยิงเมล/เช็คสิทธิ์ไม่ได้) ตอนนี้เก็บอีเมลจริงทั้งเคสที่คนเลือกเองและเคสที่ระบบเติมให้ · ไม่กระทบสิทธิ์การมองเห็น — ไม่มี code path ไหน authorize จากคอลัมน์นี้

### Added — Co-Producer โผล่ในคิวและหน้ารายละเอียด

เก็บลง DB มาตั้งแต่ v1.59 แต่ไม่เคยแสดงที่ไหนเลยนอกจาก workspace export — งาน TSS ที่ระบบเติมแก้วให้จึงจะมองไม่เห็นถ้าไม่โชว์: การ์ดคิว `/admin` ต่อท้าย `· Co-Pro: <ชื่อ>` และหน้า `/admin/<id>` มีช่อง Co-Producer

- 22 เคสทดสอบใหม่ (`qu-ref` 9 + `outlet-coproducer` 11 + regression) รวม **547 ผ่าน**

## [1.182.0] — 2026-08-20

### Added — สั่งแจ้งใบที่ "หลุดหน้าต่าง" ได้ (`?codes=`) — คำแนะนำ "กด 📣 เอง" ที่ไม่มีปุ่มให้กด

v1.181 เพิ่ม alert ที่บอกว่า *"ใบนี้มีฟุตเทจแล้วแต่หลุดหน้าต่างแจ้งอัตโนมัติ ต้องกด 📣 เอง"* — แล้วพบทันทีว่าคำแนะนำนั้น**ทำตามไม่ได้จากที่ที่คนอ่านมันอยู่**: ปุ่ม 📣 ต้องมี session ในบราวเซอร์ และ POST endpoint นั้นผ่าน console ก็ถูก permission classifier บล็อกมาตั้งแต่ ก.ค. · sweep เองก็มองไม่เห็นเพราะมันดูแค่ในหน้าต่าง `LOOKBACK_DAYS`

- `GET /api/internal/footage-ready/run?codes=A,B` — สลับ**เฉพาะหน้าต่างวันที่** ออกไป กฎอื่นยืนเหมือนเดิมทุกข้อ: ลบ/ยกเลิก/ส่งงานแล้ว/แจ้งแล้ว ยังกันอยู่ (ส่งซ้ำต้องเคลียร์ `readyNotifiedAt` อย่างตั้งใจ ไม่ใช่แค่ใส่ flag) · **ยังต้องมีไฟล์จริงบน Drive** (เมลลิงก์โฟลเดอร์ว่างแย่กว่าไม่ส่ง) · ข้าม settle 4 ชม. เพราะคนที่ระบุ code คือคนที่ตัดสินว่าพร้อมแล้ว · ข้าม walk budget เพราะคนกำลังรออยู่ · การ์ด photo-album ปิดเมื่อระบุ code เอง (คนรู้ดีกว่าการเดาจากประเภทงาน)
- **ไม่ประทับ heartbeat** — ถ้านับการสั่งมือเป็น tick ของ worker dead-man switch จะเงียบทั้งที่ sweep ตามเวลาไม่ได้เดินเลย
- `parseForcedCodes()` เป็นฟังก์ชันบริสุทธิ์ + `FORCED_MAX=25`: input พิการต้อง**แคบลง ไม่ใช่กว้างขึ้น** — ว่าง/ขยะ/`DROP TABLE` ได้ `[]` แล้ว fallback เป็น sweep ปกติ ไม่ใช่ "ทุกใบ" · +4 เทสต์ (รวม **528 ผ่าน**)
- ใช้จริงทันที: `POD-8MH-260814-01` (406 ไฟล์) + `AGN-260814-01` ที่ทีมไม่เคยได้รับแจ้งเลย

## [1.181.0] — 2026-08-20

### Fixed — เมล "footage พร้อม" ไม่ถึงคนในงานเลยตั้งแต่ 14 ก.ค. และไม่มีอะไรมองเห็น

operator ทักว่า *"ไม่มีคนใน job ได้รับเมลแจ้งฟุตเทจเลย"* · ตรวจแล้วจริงและกว้างกว่าที่คิด: ปุ่ม 📣 แบบกดเองครั้งสุดท้าย **14 ก.ค.** (37 ครั้ง) หลังจากนั้นมีแต่ auto **103 ครั้ง ซึ่งเข้า digest ของ operator ที่เดียวทั้งหมด** → ทีมงานไม่ได้รับเมลแจ้งฟุตเทจมา ~5 สัปดาห์

- **v1.178 แก้โค้ดไว้แล้วแต่ยังไม่พอ**: `FOOTAGE_READY_AUDIENCE` เพิ่งถูกตั้งเป็น `team` บน stack เช้าวันนี้ · ที่ยังพลาดต่อคือ **`LOOKBACK_DAYS=3` นับจาก "นาทีนี้"** ไม่ใช่รายวัน — งานที่ถ่าย 17 ส.ค. หลุดหน้าต่างตอนบ่ายวันที่ 20 ทั้งที่ยังไม่เคยถูกแจ้ง · prod ปรับเป็น `LOOKBACK_DAYS=4` + `MAX_PER_RUN=12` (eligible จริงแตะ 10–18 ใบ เกิน budget เดิม 5)
- **backfill 8 ใบ (17–20 ส.ค.)**: เคลียร์ `readyNotifiedAt` ที่เคยประทับตอนโหมด `admin` แล้วให้ worker ยิงใหม่ → ส่งถึงจริง 7 ใบทันที (producer + `video@`/`sound@`/`podcast@` + คนกล้อง/เสียงรายคน 6–9 รายชื่อ/ใบ `errors: []`) · เขียน audit `booking.ready_stamp_cleared` กำกับทุกใบว่าลบตราอะไรไปเพราะอะไร

### Added — `/api/internal/footage-ready/stats` วัด "มีคนได้รับไหม" ไม่ใช่ "worker เต้นไหม"

รากของเรื่องนี้ไม่ใช่บั๊กใน sender แต่คือ **ไม่มีใครวัดผลลัพธ์**: `/api/health-summary` เขียว 5 สัปดาห์เพราะมันถามว่า worker tick ไหม ซึ่ง tick ครบจริง

- นับ audit `booking.auto_notified_ready` แล้วแยก **ส่งถึงคนจริง vs เข้า digest เท่านั้น** (`admin-digest` เป็น marker ไม่ใช่ที่อยู่) + `peopleReached` · จัดใบที่ยังไม่แจ้งเป็น 3 ถัง: `waiting` (มีไฟล์ ยังอยู่ในหน้าต่าง) · **`agedOut`** (มีไฟล์แต่หลุดหน้าต่าง = auto จะไม่แจ้งอีกตลอดไป ต้องกด 📣 เอง) · `neverWalked` (ถ่ายจบแล้วแต่ยังไม่เคยเดินดู = อาการอดคิว)
- alert เป็นภาษาไทยคิดฝั่ง server จากกฎเดียวกับตัวส่ง — เคสที่เพิ่งเจอ (`total>0` แต่ `toTeam=0`) เป็น 🔴 พร้อมบอกชื่อ env ที่ต้องแก้ · read-only ล้วน (ไม่เขียน ไม่เดิน Drive ไม่ส่งเมล) ปลอดภัยที่จะ poll
- `lookbackDays` ที่ใช้แบ่ง `agedOut` อ่านจาก env ของ worker เอง — hardcode ไว้จะรายงานใบผิดทันทีที่ค่าบน stack เปลี่ยน
- **บทเรียนจาก alert แรกของมันเอง**: alert แรกที่ endpoint นี้ผลิตออกมา**ผิด** — `WLT-OTH-260819-01` (Photo Album จัดไฟถ่ายรูปติดบัตร) ถูกฟ้องว่า "worker ไม่เคยเดินดู" ทั้งที่ sweep **ข้ามงานประเภทนี้ตั้งใจ** จึงไม่มีวันมี `readyCheckedAt` = เตือนซ้ำทุกวันโดยไม่มีทางแก้ตก · เพิ่ม `skippedByDesign` (การ์ด photo-album สองด้านเหมือน sweep) + `COLD_AFTER_MS` 18 ชม. (งานที่เพิ่งเก็บกองยังไม่ใช่งานที่ถูกอดคิว — ทีมอาจกำลังอัปโหลด) · แต่ถ้าใบที่ถูกข้าม**มีไฟล์จริง** ยังต้องแจ้งตามปกติ
- Hermes `probook-worker-check.py` เพิ่ม STEP 5 อ่าน endpoint นี้ (ไม่มี secret = ข้ามเงียบ · 404 = prod ยังไม่ deploy ไม่ใช่ความผิดปกติ · 401 = บอกว่า secret ไม่ตรงแล้ว)
- +25 เทสต์ (รวม **524 ผ่าน**) ทุกเคสมาจากสภาพจริง: 5 สัปดาห์ที่ digest-only · ทีมที่ถูกกรองโดเมนจนว่าง · ใบที่หลุดหน้าต่าง · เดินแล้วไม่มีไฟล์จริง (ต้องไม่เตือน)

## [1.180.0] — 2026-08-20

### Added — ปุ่มลบงานเช่า

`/admin/rentals` มีปุ่มดินสอให้แก้ แต่ไม่มีทางลบงานที่สร้างผิดหรือไม่ได้ใช้ออกเลย · endpoint `DELETE /api/admin/rentals/[id]` **มีอยู่แล้วตั้งแต่แรก แค่ไม่เคยมีปุ่มเรียก** — รอบนี้จึงเป็นการต่อ UI เข้ากับของที่มี ไม่ใช่เขียน API ใหม่

- ปุ่มถังขยะข้างปุ่มดินสอบนการ์ด · สีเทาจาง ๆ จนกว่าจะ hover แล้วค่อยเป็นแดง — ให้กดโดนยากกว่าปุ่มแก้ไข
- confirm บอกสิ่งที่จะเกิดจริง: ชื่องาน · vendor · **จำนวนเอกสารที่จะหลุดออกจากระบบ** · และย้ำว่า**ไฟล์ใน Drive ไม่ได้ถูกลบ** (`DocumentRef` cascade แต่ไฟล์จริงอยู่ต่อ — ตั้งใจ เพราะเอกสารการเงินควรอยู่นานกว่าแถวใน tracker)
- confirm **เสนอทางที่เบากว่าให้ก่อน**: ถ้าแค่อยากให้หายจากหน้านี้ เปลี่ยนสถานะเป็น "เก็บแล้ว" ก็พอ (ตัวกรอง default ซ่อนให้อยู่แล้ว และกู้กลับได้) — คนไม่ควรเอื้อมไปหยิบตัวที่กู้ไม่ได้เป็นค่าเริ่มต้น
- ลบแบบ optimistic แล้วดึงกลับ + โชว์ error ถ้าเซิร์ฟเวอร์ไม่เห็นด้วย

### Fixed — audit ของการลบงานเช่า เคยบันทึกแค่ id ซึ่งหลังลบแล้วสืบอะไรไม่ได้

`rental.delete` เขียน `entityId` ไว้อย่างเดียว · พอแถวถูกลบถาวร id นั้นก็ไม่ผูกกับอะไรอีก — เปิด audit ย้อนหลังจะรู้แค่ว่า "มีคนลบงานเช่าใบหนึ่ง" โดยไม่มีทางรู้ว่าใบไหน

- snapshot ก่อนลบลง `changes`: ชื่องาน · quoteNo · items · เลขใบแจ้งหนี้ · ยอด · สถานะจ่าย · วันเช่า/วันคืน · vendor · booking code · **จำนวนเอกสารที่หลุด** และ **`driveFolderId`** เพื่อให้ยังตามไปหาโฟลเดอร์เอกสารเจอหลังแถวหายไปแล้ว
- ลบใบที่ไม่มีอยู่คืน **404 พร้อมข้อความไทย** แทนที่จะเงียบ ๆ 500

---

## [1.179.0] — 2026-08-20

### Fixed — footage-ready เดินดูใบเดิม 5 ใบทุกรอบ ที่เหลือ **ไม่เคยถูกเดินดูเลย**

operator ถามว่า *"ทำไมไม่มีเมลแจ้ง worker มันใช้ได้ไหม"* · worker ใช้ได้ — `errors: []` สแกน 61 เดิน Drive 5 · แต่ dry-run ซ้ำ 3 รอบกับ prod ได้ **5 ใบเดิมเป๊ะทุกรอบ และ deferred 4 ใบเดิมทุกรอบ**

- **ต้นเหตุ**: `prisma.booking.findMany()` ใน `runFootageReadyScan` **ไม่มี `orderBy` เลย** แล้วตัดด้วย `eligible.slice(0, FOOTAGE_READY_MAX_PER_RUN)` → Postgres คืนลำดับเดิม จึงได้ 5 ใบแรกเดิมตลอดกาล
- **ที่ทำให้มันไม่ใช่แค่ "ช้า" แต่เป็น "ไม่ได้เลย"**: 5 ใบที่ยึดคิวคือใบที่ไม่มีทางมี footage — THE STANDARD NOW ×2 (ข่าวรายวัน), งานอีเวนต์, งานจัดไฟถ่ายรูปติดบัตร · ใบพวกนี้ไม่เคยถูก notify จึง**ไม่เคยถูกประทับ** `readyNotifiedAt` แล้วค้างเป็น candidate ตลอดหน้าต่าง lookback 3 วัน · ของที่อยู่ข้างหลังจึงหลุดหน้าต่างไปทั้งที่**ยังไม่เคยถูกเดินดูแม้ครั้งเดียว**
- `readyCheckedAt` คอลัมน์ใหม่ (nullable, additive) — ประทับ **ทุกครั้งที่เดิน รวมตอนที่ไม่เจอไฟล์** ซึ่งเป็นหัวใจของการแก้: การเดินที่ไม่เจออะไรก็ต้องดันใบนั้นไปท้ายคิว
- `orderForWalk()` เป็นฟังก์ชันบริสุทธิ์: **ยังไม่เคยเดินมาก่อนอยู่หน้าสุด** (ใบที่เพิ่งเข้าเงื่อนไขได้คิวก่อน ซึ่งคือใบที่คนกำลังรอ) → เดินนานสุดก่อน → tie-break ด้วยวันถ่ายใหม่สุด
- การ์ด photo-album เดิมอ่าน program code ของ **ตอน** เท่านั้น `WLT-OTH-260819-01` (Photo Album, จัดไฟถ่ายรูปติดบัตร) จึงลอดทุกรอบเพราะ code ของตอนเป็น `OTH` — เผาสล็อตกับงานที่ไม่มีวิดีโอโดยนิยาม · ตอนนี้ดู program ของ **booking** ด้วย
- dry-run คืน `deferredCodes` แล้ว — ตัวเลขเปล่า ๆ ทำให้ต้องกลับไปเดาจากปฏิทินว่าใครถูกดอง
- +8 เทสต์ รวม **499 ผ่าน** — มีเทสต์ regression ที่จำลอง 2 sweep: squatter ถูกประทับแล้วต้องเสียคิวให้งานจริง และห้ามมีใบไหนถูกเดินซ้ำก่อนที่ทุกใบจะได้คิว

### Added — audience `team` — ทีมงานได้ แต่ไม่ส่งเข้าอีเมลส่วนตัวของฟรีแลนซ์

- `FOOTAGE_READY_AUDIENCE=team` = producer + คนสร้าง + ทีมที่ assign **เฉพาะ @thestandard.co** (ทับ list ได้ด้วย `FOOTAGE_READY_INTERNAL_DOMAINS`)
- **กล่องรวม `video@` / `sound@` ยังอยู่** — ต่างจาก invite แบบประเมิน ที่กล่องรวมเป็นปัญหาเรื่อง credential · ที่นี่ลิงก์ไม่ใช่ credential สิทธิ์ Drive ยังเป็นตัวตัดสินว่าใครเปิดได้ และกล่องรวมคือวิธีที่ใช้ติดต่อ "ทีมกล้อง" จริง ๆ
- `isInternalEmail()` บังคับว่าต้องมี `@` เดียว — ที่อยู่ผิดรูปต้อง**ตกกฎโดเมน ไม่ใช่ลอดผ่าน** (กับดักเดิมที่จ่ายไปแล้วใน `isInvitableEmail` v1.173.1)

---

## [1.178.0] — 2026-08-20

### Fixed — เมล "footage พร้อม" ส่งถึงคนที่รอไฟล์ ไม่ใช่ถึงคนดูแลระบบคนเดียว

operator ทักมาว่า *"เมลทุกงานไม่ควรส่งให้ผมคนเดียว ควรส่งให้ทีมงานด้วย"* · ตรวจ Gmail แล้วจริง — 12 ฉบับใน 9 วัน `from` และ `to` เป็น `narasit.k@` ทั้งคู่ ไม่มีโปรดิวเซอร์หรือทีมถ่ายคนไหนได้รับเลย

- **สาเหตุคือ config ไม่ใช่บั๊ก**: prod ตั้ง `FOOTAGE_READY_AUDIENCE=admin` ซึ่งส่งผ่าน `notifyEmailDigest()` ไปที่ `REMINDER_ADMIN_EMAIL` ที่เดียว · ปุ่ม 📣 แบบกดเองส่งให้ทุกคนอยู่แล้ว มีแต่ตัว auto ที่ถูกจำกัด
- **แต่กับดักอยู่ในทางแก้ที่ชัดที่สุด**: สลับเป็น `everyone` เฉย ๆ จะ**ตัดสำเนาของ operator ทิ้ง** เพราะ branch `producer`/`everyone` ไม่เคยเรียก digest เลย — การแก้ที่ตอบคำร้องเรียนพอดี จะทำให้คนเดียวที่เฝ้าฟีเจอร์นี้อยู่มองไม่เห็นมันอีกต่อไป · คำสั่งคือ "**ด้วย**" ไม่ใช่ "แทน"
- digest จึงถูก **เพิ่ม** เข้าไปทุกโหมด และ**ตัดออกเมื่อ admin เป็นผู้รับที่มีชื่ออยู่แล้ว** จะได้ไม่ได้เมลซ้ำสองฉบับ
- แยกการหาผู้รับออกมาเป็นฟังก์ชันบริสุทธิ์ `footageReadyRecipients()` — เดิมตรรกะนี้ฝังอยู่ในฟังก์ชันที่ยิงเมลจริง จึงไม่มีเทสต์สักตัว ทั้งที่เป็นจุดที่ตัดสินว่า "ใครได้รู้"
- digest **ไม่นับเป็น delivered** — ถ้ามีแต่ digest ที่ผ่าน แปลว่าทีมยังไม่ได้ยินอยู่ดี การรายงานว่าสำเร็จคือสิ่งที่ทำให้สภาพ "ส่งให้ผมคนเดียว" อยู่มาได้โดยไม่มีใครเห็น
- +8 เทสต์ (admin-only · producer+digest · everyone ครบทีม · admin อยู่ในลิสต์แล้วต้องไม่ซ้ำ · เทียบอีเมลแบบไม่สนตัวพิมพ์/ช่องว่าง · ไม่มี admin ก็ห้ามเสก · em-dash "—" ที่ UI เขียนแทน "ไม่มี" ต้องไม่กลายเป็นผู้รับ · booking ที่ไม่มีใครเลย) รวม **491 ผ่าน**

**ต้องตั้ง env ตอน deploy**: `FOOTAGE_READY_AUDIENCE=everyone` บน stack 125 — โค้ดอย่างเดียวไม่เปลี่ยนพฤติกรรม เพราะโหมดยังเป็น `admin` อยู่

---

## [1.177.1] — 2026-08-18

### Fixed — headcount ที่ default เป็น 1 ไม่ใช่การขอคน

เรียกจริงกับ prod ครั้งแรกได้ **9 ช่างวิดีโอ** จองในเช้าเดียว — มากกว่าที่ roster มี — บนเช้าที่ไม่ได้มีงานอะไรขนาดนั้น

- ต้นเหตุ: `videographerCount` และ `switcherCount` **default เป็น 1 ใน schema** และ wizard โพสต์ 1 มาแม้เป็นตำแหน่งที่โปรดิวเซอร์ไม่ได้ติ๊ก → การรวมคอลัมน์ที่เก็บไว้ตรง ๆ จึงนับ**ทุก booking ในระบบ**ว่าขอช่างวิดีโอ
- `crewRequired` เป็นสัญญาณเดียวที่ซื่อสัตย์ — DB layer จึง select มันมาด้วย และ headcount จะถูกนับ**เฉพาะเมื่อ role นั้นอยู่ในลิสต์**
- ไม่มี `crewRequired` ให้ผ่านไปเหมือนเดิม — นั่นคือ wizard ตัวจริง ซึ่งเซ็ตเลขเป็น 0 เองตามกล่องที่ติ๊ก
- จับได้เพราะเอาเลขไปเทียบกับความจริง ไม่ใช่เพราะ deploy ขึ้นเขียว: **เลขคณิตถูก แต่ input ผิด**

---

## [1.177.0] — 2026-08-17

### Added — "กล้องเต็ม / คนเต็ม" เตือนตั้งแต่ตอนจอง

โปรดิวเซอร์ได้ยินตั้งแต่ตอนจองว่า pool ไหนที่ช่วงเวลาของเขาใช้หมดแล้ว ในคำที่ทีมขอมา: "กล้องเต็ม อาจมีการเช่าอุปกรณ์" สำหรับของ · "คนเต็มต้องจ้างฟรีแลนซ์" สำหรับคน ปิดท้ายด้วยว่าใครเป็นคนแก้เสมอ — **admin เป็นคนหา งานยังจองผ่านได้** ไม่มีอะไรบล็อก เพราะโปรดิวเซอร์ไม่ได้เป็นเจ้าของ inventory การบล็อกแข็ง ๆ จะสอนให้เขาจองหลบคำเตือนเท่านั้น

กติกาการนับ มาจากการที่ของเคลื่อนจริง:

- งาน **off-site ไม่คืนกล้องตอน wrap** — จบ 12:00 นอกออฟฟิศ งานถัดไปเริ่ม 12:00 = ยังไม่ว่าง ("นับเป็นไม่ทัน") ค่าคงที่ตัวเดียว `TRAVEL_BUFFER_MIN` = 60 ไว้ปรับจากคำบ่นจริง
- **งานหลายวันถือของข้ามคืน** จึงกินทั้งวันของทุกวันที่มันแตะ ไม่ใช่แค่ช่วง call→wrap
- กล้องเป็น pool คงที่ **9 ตัว** · pool คนนับสดจาก roster `TeamMember` จ้างเพิ่มหรือปิดใช้งานแล้วคำเตือนเปลี่ยนตามโดยไม่ต้อง deploy

แก้ 2 บั๊กของ camera check เดิม (v1.61) ไปด้วย:

- **`ASSIGNED` ไม่อยู่ใน status filter** → งานที่เพิ่งจัดคนเสร็จนับกล้องเป็น 0 · ช่วงที่ยุ่งที่สุดคือช่วงที่ถูกนับขาดพอดี
- pool ที่ booking ไม่ได้ขอก็เตือนได้ ("สวิตเชอร์ 2/1, ของคุณ 0") — จริง แต่ไม่ใช่เรื่องของโปรดิวเซอร์คนนั้น

เลือก Studio 1 pre-fill 3 กล้อง + บอกชื่อ rig (Sony FX30 × 3) เติมเฉพาะช่องว่างจึงไม่ทับค่าที่พิมพ์หรือกู้คืนมา · คณิตศาสตร์ของ window อยู่ใน pure lib (14 เทสต์ตั้งชื่อตามกฎแต่ละข้อ) และข้อความไทยสร้างฝั่งเซิร์ฟเวอร์ wizard กับหน้า admin จึงบรรยายของขาดตัวเดียวกันต่างกันไม่ได้

---

## [1.176.0] — 2026-08-17

### Fixed — `REVIEW_EXCLUDE_EMAILS` ที่ไม่ได้ตั้ง ต้องยังกันกล่องอีเมลรวมอยู่

compose ส่ง `REVIEW_EXCLUDE_EMAILS: ${REVIEW_EXCLUDE_EMAILS:-}` → ตัวแปรที่ stack ไม่เคยตั้งมาถึงแอปเป็น **empty string ไม่ใช่ undefined** · `reviewExcludedEmails()` อ่าน `""` ว่า "operator ตั้งใจไม่ยกเว้นใครเลย" ซึ่งคือหน้าตาของ prod ที่**ยังไม่ได้ตั้งค่า**พอดี → ผลคือกล่องรวมของทีม (video@, sound@, event@) ที่ฟีเจอร์สร้างมาเพื่อข้าม กลับเข้าลิสต์ส่งอีกครั้ง

- จับได้ตอน dry run กับ prod ระหว่างเตรียมเปิดระบบประเมิน — ไม่มีเมลออกไป
- ว่าง/ไม่ตั้ง = ใช้ค่า default · จะไม่ยกเว้นใครเลยต้องใส่ sentinel ชัด ๆ `REVIEW_EXCLUDE_EMAILS=none`
- **โรคเดียวกับ SHOOT_MARKER_WORKER**: `${VAR:-default}` ใน compose ไม่ใช่ค่าของ stack และ `""` ไม่เท่ากับ "unset" เมื่อข้ามเส้นนั้นมาแล้ว

---

## [1.175.0] — 2026-08-17

### Added — export ตารางทั้งสัปดาห์เป็นข้อความ

operator กรอก อุปกรณ์/เช่า ทั้งสัปดาห์แล้วต้องไปบอกคนอื่นว่าจัดอะไรไว้ · แคปหน้าจอทำให้ข้อความหาย ค้นไม่ได้ และแปะลงแชทไม่ได้

- **📄 Export ข้อความ** เรนเดอร์สัปดาห์ที่เห็นเป็น plain text พร้อมปุ่มก๊อป ดาวน์โหลด `.txt` และกล่อง preview ให้ลากเลือกเองเมื่อ clipboard API ถูกบล็อก
- "เฉพาะงานที่กรอกแล้ว" ซ่อนงานที่ยังไม่แตะ **โดยไม่แตะตัวนับของวัน** — หัวยังบอก 2/3 ช่องว่างจึงยังมองเห็น
- ช่องว่างพิมพ์ `—` แทนที่จะตัดบรรทัดทิ้ง: บรรทัดที่หายอ่านว่า "ไม่ต้องใช้" ส่วนขีดอ่านว่า "ยังไม่มีใครกรอก" ซึ่งคือสิ่งที่ operator กำลังตาม
- โน้ตหลายบรรทัดยุบเป็น "a / b / c" เพื่อไม่ให้รายการยาว ๆ รายการเดียวพัง layout ในหน้าต่างแชท
- การจัดรูปแบบอยู่ใน pure module พร้อมเทสต์ ไม่ใช่ในคอมโพเนนต์ — ข้อความนี้ถูกอ่านโดยคนใน LINE ที่บรรทัดว่างเกินหรือขีดที่หายไปคือเส้นแบ่งระหว่างอ่านรู้เรื่องกับ noise · แถวสร้างจาก helper ชุดเดียวกับที่หน้าจอใช้ (`windowOf`, `bookingDisplayName`) ข้อความจึง drift จากจอไม่ได้

---

## [1.174.1] — 2026-08-17

### Fixed — LIF/HDL ชื่อจริงคือ "How Do You Live ?"

DB เขียน "How do you live?" โฟลเดอร์เขียน "How do you live" — สองสะกด และไม่ตรงชื่อจริงสักอัน

- rename ผ่าน `/api/admin/programs/rename` + แก้ seed list กัน DB ใหม่พาของเก่ากลับมา
- **Drive rename ของ endpoint ข้ามไป (`driveRenamed: false`) เพราะชื่อโฟลเดอร์กับ DB ไม่ตรงกันอยู่ก่อนแล้ว** จึงต้องเปลี่ยนชื่อโฟลเดอร์แยกด้วย id — ปล่อยให้ไม่ตรงกันจะแย่กว่าเดิม เพราะการหาโฟลเดอร์แมตช์ด้วย**ชื่อเป๊ะ ๆ** รอบ prep-folder ถัดไปจะสร้างโฟลเดอร์**ใบที่สอง** แล้วฟุตเทจของรายการจะแตกเป็นสองที่

---

## [1.174.0] — 2026-08-17

### Fixed — งานนอกสถานที่มี "สถานที่" ไม่ใช่ "ห้อง"

รายงานจากหน้างาน: แก้ booking แบบ On Location แล้วเห็น "Tiffany & Co. สาขาสยามพารากอน https://maps…" แต่พิมพ์อันใหม่ไม่ได้ · ฟอร์มแก้ไขเรนเดอร์ `<select>` ห้องประชุมให้**ทุก** shoot type แล้วเก็บข้อความเดิมไว้เป็น `<option>` เพิ่มมาอีกอันหนึ่ง → ค่าอ่านได้แต่แก้ไม่ได้ และวิธี "แก้" อย่างเดียวคือเลือกห้องประชุม ซึ่งผิดสำหรับงานที่ไม่ได้อยู่ในตึก

- `ON_LOCATION` ได้ text input และไม่มีลิสต์ห้องเลย · type อื่นคงตัวเลือกห้องไว้ — เป็นการแยกที่ **wizard ตอนสร้างมีมาตั้งแต่ v1.58** ฟอร์มแก้ไขแค่ไม่เคยเรียนรู้
- สลับ Shoot Type ข้ามเส้นในตึก/นอกตึกแล้ว**ล้างค่า** เพราะชื่อห้องไม่ใช่สถานที่ และสถานที่ไม่ใช่ชื่อห้อง · เก็บไว้เมื่อยังเข้ากับฝั่งที่สลับไป การกดพลาดจึงไม่ทำลายสิ่งที่พิมพ์ไว้
- รายการ Podcast seed ไว้ว่า "8 Minutes History" ชื่อจริงคือ **"8 Minute History"** — rename ใน DB ผ่าน `/api/admin/programs/rename` (ซึ่ง rename โฟลเดอร์ Drive ให้ด้วย) และแก้ seed list เพราะ program upsert ด้วย `update: {}` จึงจะพาคำผิดกลับมาบน DB ใหม่
---

## [1.173.9] — 2026-08-11

### Changed — กลับไปให้ทุกทีมให้คะแนนทุกทีมเหมือนเดิม

revert เฉพาะครึ่ง `targetsFor` ของ v1.173.8 · คำสั่ง operator: ประสบการณ์ของทุกทีมที่มีต่อทุกทีมคือข้อมูลที่เขาต้องการ และกล้องกับเสียงทำงานใกล้กันพอที่จะมีมุมมองที่ควรบันทึกไว้

```
producer / other → ทีมกล้อง · ทีมเสียง
camera           → ทีมโปรดิวเซอร์ · ทีมเสียง
sound            → ทีมโปรดิวเซอร์ · ทีมกล้อง
```

- กลับมาแค่การจับคู่ · **ยังคงไว้ในฐานะการตัดสินใจแยกกัน**: ไม่มีดาว "ภาพรวม" ต่อทีม (v1.173.6 — คะแนน derive จากเกณฑ์) และไม่ถาม "คุณภาพงานที่ส่งมอบ" กับ rater ฝั่ง crew (v1.173.7 — พวกเขาไม่ได้รับ deliverable) · ฟอร์มของ crew จึงเป็น 2 ทีม × 2 เกณฑ์ + คำถามระดับงาน
- คำใบ้ในกล่องข้อความอิสระของ crew ยังอยู่ — เขียนขึ้นตอนที่ทางออกเดียวของเขาคือกล่องนั้น และกล่องนั้นยังเป็นที่เดียวที่เรื่องคาใจเป็น**ประโยค**ได้แทนที่จะเป็นตัวเลข ซึ่งคือสิ่งที่ช่องทางนี้มีไว้เพื่อสิ่งนั้น

---

## [1.173.8] — 2026-08-11

### Changed — crew เลิกให้ดาวกันเอง ย้ายไปกล่องข้อความ _(การจับคู่ถูก revert ใน v1.173.9)_

กล้องกับเสียงขึ้นรถตู้คันเดียวกันพรุ่งนี้ · ดาวระหว่างกันเปลี่ยนแบบสำรวจบริการให้กลายเป็นกระดานคะแนนระหว่างเพื่อนร่วมงาน และทั้งสองทีมก็ไม่ได้รับอะไรจากกันให้ตัดสินตั้งแต่แรก

- กล่องข้อความบอกกับ crew ตรง ๆ ว่า "มีเรื่องของทีมอื่นในกองก็เขียนมาได้เลยครับ" — พอแถวดาวหายไป กล่องที่เงียบอ่านว่า "ไม่มีใครถาม" แล้วสิ่งที่เขาอยากพูดจะไม่ถูกพูด · ประโยคยังเป็นสิ่งที่ผู้จัดการเอาไปทำอะไรต่อได้ ต่างจาก 2/5 จากเพื่อนร่วมงานที่ไม่รู้ว่าใคร
- แก้ที่ `targetsFor` ไม่ใช่ที่ฟอร์ม — เมลเชิญ, target ที่เก็บ, จำนวน invite ที่ monitor คาดไว้ และ anti-tamper check ของ API จึงขยับพร้อมกัน
- เทสต์ล้ม 4 ตัวและทั้ง 4 ยืนยันการจับคู่แบบเก่า — **กฎถูกปักไว้** การสลับจึงเป็นการแก้จุดเดียว ไม่ใช่การไล่ล่า
- งานที่มีแต่ crew ไม่มีฝั่งโปรดิวเซอร์จะได้ target ว่าง ซึ่งคือสิ่งที่กัน `buildInvites` ไม่ให้ส่งฟอร์มที่ไม่มีอะไรอยู่บนนั้น

---

## [1.173.7] — 2026-08-11

### Fixed — เลิกถาม crew เรื่องงานที่ไม่มีใครส่งมอบให้เขา

"คุณภาพงานที่ส่งมอบ" มีความหมายเฉพาะกับฝั่งที่**ได้รับ**งาน · กล้องกับเสียงไม่ได้ส่งอะไรให้กัน และไม่ได้รับ deliverable จากโปรดิวเซอร์ แถวดาวนั้นจึงเก็บตัวเลขของสิ่งที่ไม่มีอยู่ — และเพราะคะแนนของทีมคือค่าเฉลี่ยของเกณฑ์ มันจึงลากคำตอบจริงลงไปด้วย

```
producer / other → สื่อสาร · ตรงเวลา · คุณภาพงานที่ส่งมอบ
camera / sound   → สื่อสาร · ตรงเวลา
```

- กฎยึดที่ **rater ไม่ใช่ทีมที่ถูกให้คะแนน** ซึ่งเป็นจุดที่กลับด้านง่าย: ทีมกล้องทีมเดียวกันถูกถามเรื่องการส่งมอบโดยโปรดิวเซอร์ และไม่ถูกถามโดยช่างเสียง
- บังคับฝั่งเซิร์ฟเวอร์ด้วย ไม่ใช่แค่ไม่ใส่ในฟอร์ม — หน้าที่ cache ค้างหรือ POST ที่ประดิษฐ์ขึ้นจะยัดคะแนนการส่งมอบเข้า record ถาวรไม่ได้
- เกณฑ์ที่ไม่ได้ถูกถามจะถูก**ตัดทิ้ง ไม่ใช่ 400** — แท็บเก่าไม่คุ้มกับการทำให้การส่งของใครสักคนล้มเหลว และคะแนนที่ derive ก็แค่เฉลี่ยจากที่เหลือ

---

## [1.173.6] — 2026-08-11

### Added — simulator ของฟอร์ม และตัดคำถามออกทีมละหนึ่ง

**1. ดาว "ภาพรวม" ต่อทีมหายไป** — มันอยู่เหนือเกณฑ์สามข้อเดียวกันและขอให้คนสรุปสิ่งที่ตัวเองเพิ่งสรุป · คะแนนต่อทีมที่เก็บตอนนี้ **derive จากเกณฑ์ ฝั่งเซิร์ฟเวอร์** คอลัมน์ยังอยู่เพราะทุกรายงานยึดกับมัน และการคำนวณที่เดียวแปลว่าทุก client ที่โพสต์เข้ามาเห็นตรงกันว่ามันแปลว่าอะไร

**2. `/review/demo-*` เรนเดอร์ฟอร์มจริงบน booking ที่ไม่มีอยู่ และไม่เคย POST**

- ก่อนหน้านี้วิธีเดียวที่จะดูฟอร์มคือ mint invite บน**งานจริง** ซึ่งการกดส่งเขียนคะแนนถาวรลบไม่ได้ของเพื่อนร่วมงานจริง · ผมส่งลิงก์แบบนั้นให้ operator ไปตรวจคำ **เขากดส่ง และไม่มี undo** — ตามดีไซน์ เพราะ append-only คือสัญญาที่ทำให้ฟีเจอร์นี้ตอบตรง ๆ ได้อย่างปลอดภัย · การทดลองต้องไม่มีวันทำแบบนั้นได้ตั้งแต่แรก
- simulator จึงไม่อ่านไม่เขียนอะไรเลย: ไม่มี invite row ไม่มี booking ไม่มี DB call ทั้งขาเข้าและขาออก · ยืนยันกับฐานข้อมูลเปล่า — ทั้งสองฝั่งเรนเดอร์ได้ กดส่งถึงหน้าขอบคุณ และหลังจากนั้น `shoot_reviews` กับ `shoot_review_invites` ยังเป็น 0 แถว ไม่มี POST ใน network log
- **หนึ่งลิงก์ต่อหนึ่งที่นั่ง**: `/review/demo-producer` · `demo-camera` · `demo-sound` — "ฝั่งทีมงาน" ลิงก์เดียวเดิมเรนเดอร์ที่นั่งกล้อง ซึ่งบอกเป็นนัยผิด ๆ ว่า crew เห็นเหมือนกันหมด แต่การประเมินเป็นแบบต่างตอบแทน ทุกที่นั่งจึงถูกถามถึงทีมคนละคู่ และ**ฟอร์มของช่างเสียงเป็นอันเดียวที่ให้คะแนนทีมกล้อง**
- ชื่อเดิมที่ส่งไปก่อนหน้าในวันเดียวกัน (`demo-client`, `demo-crew`) ยัง resolve ไปที่ producer/camera ของที่ส่งออกไปแล้วจึงยังใช้ได้ · ไม่มีอะไรเปลี่ยนในสิ่งที่ฟอร์มจริงถาม — `targetsFor` จับคู่แบบนี้มาตลอด สิ่งที่ขาดคือ**ทางที่จะมองเห็น**ของช่างเสียง

---

## [1.173.5] — 2026-08-11

### Added — ส่ง invite จริงเข้าอินบ็อกซ์ตัวเองก่อนเปิดใช้

"ลองส่งให้ผม" · การอ่านคำใน diff ไม่เหมือนการเจอมันแทรกอยู่ระหว่างเมลอีกสองฉบับบนมือถือ และ operator กำลังจะส่งให้ 67 คนด้วยคำชุดนี้

- endpoint preview ที่ mint invite จริงแล้วไม่ส่งหาใคร ตอนนี้รับ `sendMail: true` แล้วส่ง invite นั้นไปที่**อีเมลของผู้เรียกเอง** — `session.email` เท่านั้น ไม่เคยเอาที่อยู่จาก request body มันจึงกลายเป็นช่องส่งเมลหาคนที่สามไม่ได้
- `voice` เลือกว่าจะอ่านจดหมายฉบับไหนโดยไม่แตะ role ที่เก็บใน invite เขาจึงอ่านจดหมายฝั่ง crew ได้แม้บทบาทของตัวเองในงานนั้นไม่ใช่ crew
- ปุ่มอยู่บน `/admin/monitor` **ไม่ใช่แค่บน `/admin/reviews`** — หลัง v1.173.4 หน้านั้นเป็นของผู้จัดการ operator ไม่มีเมนูเข้าถึงแล้ว · เครื่องมือที่เข้าไม่ถึงคือเครื่องมือที่ยังไม่ได้ ship บทเรียนเดียวกับที่ v1.168.1 เคยจ่ายไปแล้วหนหนึ่ง
- เจอ 2 บั๊กระหว่างต่อสาย: query booking ของ preview เป็น `findFirst` ธรรมดา program/outlet/episodes จึงกลับมาเป็น undefined แล้วเมลจะพาดหัวว่า "งานถ่าย" เฉย ๆ · และการส่งหาตัวเองตอนนี้ stamp `mailedAt` เพราะเมลออกไปจริง monitor จึงควรเลิกเรียก invite นั้นว่าส่งไม่สำเร็จ

---

## [1.173.4] — 2026-08-10

### Changed — ผู้จัดการอ่านข้อความได้ ผู้ดูแลระบบไม่ได้

ช่องทางนี้กลายเป็นที่สำหรับสิ่งที่คนไม่พูดต่อหน้าเพื่อนร่วมงาน — คำร้องเรียน เรื่องคาใจ เรื่องที่ยกขึ้นมาโดยไม่ติดชื่อ — และมันป้อนเข้าการประเมินผลงาน นั่นเปลี่ยนว่าใครควรอ่านได้ · คำสั่ง operator: **ผู้จัดการ (ปุ๊ก, หวาน) เห็นทุกข้อความและเห็นว่าใครเขียน เขาเห็นแค่ว่ามีงานที่ผลิต feedback ออกมา**

เหตุผลเป็นเชิงโครงสร้าง: เขาเป็นคนดูแลระบบ เขาอยู่ในลิสต์ crew และเขามักเป็นโปรดิวเซอร์ที่ถูกให้คะแนน — เจ้าของระบบที่อ่านคำร้องเรียนเกี่ยวกับตัวเองได้ทุกฉบับ คือระบบที่ไม่มีใครร้องเรียน

- `canReadReviews` หายไป แทนด้วย 2 predicate เพื่อบังคับให้ทุก call site ถูก**ตัดสินใจใหม่** ไม่ใช่สืบทอดมาเฉย ๆ:
  - `canReadReviewContent` → ข้อความ คะแนน ชื่อผู้ให้คะแนน · **ผู้จัดการ**
  - `canSeeReviewActivity` → ส่งออกไปไหม มีใครตอบไหม อะไรค้าง · **ผู้จัดการ + operator**
- `/api/admin/reviews` และ `/export` → 403 สำหรับ operator · `/api/admin/monitor` → pipeline ได้ แต่ค่าเฉลี่ยคะแนนเป็น null และ**ไม่ถูก query ด้วยซ้ำ**ถ้าผู้เรียกอ่านไม่ได้ · `/preview` ได้ เพราะ mint ฟอร์มเปล่าของผู้เรียกเอง ซึ่งไม่เปิดเผยคำตอบของใคร · Nav เป็นแค่ความสวยงาม ตัวจริงคือ 403 ฝั่งเซิร์ฟเวอร์
- ทั้งสองลิสต์ **fail closed**: env มั่ว = ตกกลับไปที่ default ไม่เคยตกไปที่ "ทุกคน" และไม่เคยตกไปที่ "ไม่มีใคร" · การเพิ่ม operator กลับเข้าฝั่ง content ต้องแก้ env อย่างตั้งใจ ซึ่งคือหลักประกันที่คำชี้แจงบนฟอร์มพึ่งพาอยู่
- **คำชี้แจงต้องขยับตามการ์ด** ไม่งั้นแอปจะโกหกพนักงานที่มันเพิ่งขอให้พูดตรง ๆ · เดิมเขียนว่า "ผู้ดูแลระบบ 3 คน (นัท · ปุ๊ก · หวาน)" ซึ่งหลังการเปลี่ยนนี้ระบุชื่อคนที่ไม่ได้อ่านแล้ว และเรียกผู้จัดการว่าผู้ดูแลระบบ · 6 ที่ที่เขียนเรื่องนี้แยกกันตอนนี้อ้างค่าคงที่ตัวเดียว และคำมั่นของ operator เอง — ผู้ดูแลระบบเห็นเพียงว่ามีการส่ง ไม่เห็นข้อความ — เขียนไว้ในที่ที่พนักงานถือเขาไว้กับมันได้
- **เป็นเส้นแบ่งเชิงนโยบายในแอป ไม่ใช่เชิงวิทยาการเข้ารหัส** — ใครถือ credential ของ DB หรือ Portainer ยังอ่านตารางได้ คำที่เลือกใช้จึงเลือกให้จริงภายใต้ข้อจำกัดนั้น ไม่ใช่เลือกให้สัญญาเกินจริง

---

## [1.173.3] — 2026-08-10

### Added — ใบเสร็จ เพื่อไม่ต้องมีใครเดินมาถามว่า "เห็นที่ผมเขียนไหม"

ก่อนหน้านี้ตอบฟอร์มเสร็จได้หน้าขอบคุณแล้วจบ · ใครที่เขียนอะไรที่เขาแคร์มีทางเดียวที่จะรู้ว่ามันถึงไหม คือเดินไปถาม operator เอง

- กดส่งแล้วได้เมลถึงตัวเอง สะท้อนคะแนนของเขาและคำของเขากลับมาพร้อม timestamp — หลักฐานของเขา ในอินบ็อกซ์ของเขา ก่อนที่ความสงสัยจะเริ่ม · เปิดลิงก์ซ้ำเห็นว่าส่งเมื่อไร
- **best-effort โดยตั้งใจ**: คะแนน commit ก่อน เมลล่มจึงกลายเป็นการส่งที่ล้มเหลวไม่ได้ (ไม่งั้นคนจะตอบฟอร์มเดิมซ้ำ แล้ว unique index จะทิ้งครั้งที่สองเงียบ ๆ) · ส่งไม่ว่า `SHOOT_REVIEW_ENABLED` จะเปิดหรือไม่ — flag คุมการเชิญ ส่วนคนที่เพิ่งตอบสมควรได้ใบยืนยันอยู่ดี
- **2 อย่างที่ตั้งใจไม่ทำ** — ไม่อ่านคำตอบกลับผ่านลิงก์ฟอร์ม (token เป็น bearer credential เมลที่ถูก forward จะยื่นคะแนนของคนอื่นให้ใครก็ตามที่เปิด · หลักฐานเนื้อหาไปที่กล่องจดหมาย หลักฐานในแอปเหลือแค่ timestamp) และไม่สัญญาว่าจะตอบทุกข้อความ (สัญญาที่ทีมรักษาไม่ได้จะพาคำถามเดิมที่ตั้งใจจะจบกลับมา — ข้อความจึงบอกว่าทีมจะติดต่อกลับถ้ามีอะไรต้องแก้ และเมลฉบับนี้เองคือหลักฐานว่ามันถึงแล้ว)
- หน้าจอหลังส่งอ้างว่าส่งใบเสร็จแล้ว**เฉพาะเมื่อส่งจริง** (`receiptSent` กลับมาจาก route) ส่วนหน้าที่เปิดซ้ำทีหลังแสดงแค่ timestamp และไม่อ้างอะไร เพราะหน้านั้นรู้ไม่ได้ · ยืนยันกับ Postgres ชั่วคราวโดยปิดเมล: คำตอบถูกเก็บ ไม่มีคำอ้างเท็จขึ้นจอ

---

## [1.173.2] — 2026-08-10

### Added — จดหมายสองฉบับ ฉบับหนึ่งถึงฝั่งที่ใช้ Probook อีกฉบับถึงทีมงาน

คำของ operator · ย่อหน้าเดียวที่เขียนถึงทุกคนแปลว่าครึ่งหนึ่งของผู้รับกำลังอ่านจดหมายที่เขียนถึงคนอื่น: ฝั่งโปรดิวเซอร์ใช้ Probook เพื่อให้ได้ทีมมา และกำลังให้คะแนน**บริการ** ส่วนทีมงานเป็นคนทำงาน และถูกถามว่าวันนั้นราบรื่นไหม

```
producer / other → ขอบคุณสำหรับการใช้งาน Probook · ขอรบกวนให้คะแนน<teams>ครับ
camera / sound   → ขอบคุณสำหรับการทำงานหนัก · การทำงานกับทีมและโปรดิวเซอร์…ราบรื่นไหมครับ?
```

- `other` อ่านจดหมายฝั่งโปรดิวเซอร์โดยตั้งใจ — `classifyRater` โยนคนที่จัดที่ไม่ได้ (coordinator, แขกในกอง) มาไว้ตรงนี้ และพวกเขาก็อยู่ปลายทางของบริการเหมือนกัน
- คำถามความพอใจระดับงานแยกตามเดียวกัน (`ต่อการให้บริการงานนี้` vs `ต่อการทำงานนี้`) และ**ฟอร์มเสิร์ฟคำชุดที่ invite ของคนนั้นใช้** ประโยคจึงไม่เปลี่ยนระหว่างเมลกับหน้าเว็บ · เก็บเป็นแถวเดียวกันทั้งสองทาง คะแนนจึงเทียบกันได้ทั้งทีม
- ทั้งสองฉบับย้ายออกจาก route มาเป็น `buildInviteMail()` บริสุทธิ์ตัวเดียว พร้อมเทสต์ปักน้ำเสียงแต่ละแบบ — รวมถึงว่า**ห้ามฉบับไหนพูดคำว่า "เมื่อวาน" หรือ "วันนี้"** เพราะ window ตามเก็บส่งช้ากว่าวันถ่ายได้หลายวัน และคำขอต้องระบุเฉพาะทีมที่ทำงานในงานนั้นจริง
- แก้คำที่ได้รับมา 3 จุด แต่ละจุดมีเหตุ: "วันนี้" → "ในงานนี้" (ปัญหาผิดวันเดียวกัน) · "โปรดิวซ์เซอร์" → "โปรดิวเซอร์" (สะกดแบบที่ใช้ทั้งแอป) · "ต่อการให้ทำงานนี้" → "ต่อการทำงานนี้"
- บรรทัดรักษาความลับแบบสั้นอยู่ใน**เมล** ส่วนฟอร์มเก็บคำชี้แจงแบบยาวที่ระบุชื่อคนที่อ่านคำตอบได้ไว้ — สัญญาแบบนั้นต้องกางให้เห็นตรงที่คนกำลังจะตอบ ไม่ใช่ย่อลงตรงนั้น

---

## [1.173.1] — 2026-08-10

### Fixed — เฉพาะคนจริงที่ @thestandard.co เท่านั้นที่ถือ invite

dry run ที่ operator ขอแสดงว่า **29 จาก 97 invite** วิ่งไปหากล่องรวม 2 ใบ — video@ 18 ครั้ง, sound@ 11 — และอีก 1 ใบไป gmail ข้างนอก · ใน 300 booking หลังสุด video@ โผล่ 264 ครั้ง sound@ 205 ครั้ง: มันนั่งอยู่ใน `assignedEmails` แทนคำว่า "ทีมกล้อง" และไม่มีใบไหนอยู่ใน roster 25 คน

ควรบล็อกมากกว่าปล่อย เพราะ **invite link คือ bearer credential** — ใครเปิดกล่องรวมได้ก็ยื่นคะแนนแทนทั้งทีมได้ ซึ่งทำให้ตัวเลขไร้ความหมาย และทำให้ "ไม่มีใครเห็นว่าใครพูดอะไร" กลายเป็นสัญญาที่แอปรักษาไม่ได้ · แถมเมลสำรวจ 18 ฉบับเข้ากล่องเดียวในสัปดาห์เดียวคือทางที่เร็วที่สุดที่จะสอนคนว่านี่คือสแปม

- 2 กฎใน predicate เดียวที่มีเทสต์: exclude list ที่ default เป็นกล่องรวมที่มีอยู่จริงในข้อมูล (ถูกต้องโดยไม่ต้องมีใครตั้ง env) + กฎโดเมนภายใน
- ที่อยู่ผิดรูปตกกฎโดเมนแทนที่จะลอดผ่าน — เทสต์จับได้ว่าการ split ที่ `@` **ตัวสุดท้าย** อ่าน `a@b@thestandard.co` ว่าเป็นคนใน
- การยกเว้นตัดคนออกจากลิสต์ที่**ถูกถาม**เท่านั้น · ทีมไหน**ถูกให้คะแนนได้**ยังตัดสินจากทุกคนบน booking — งานที่มีแต่ video@ ในช่องกล้องจึงยังคง ทีมกล้อง ไว้ในคำถามที่คนอื่นตอบ แทนที่จะไม่ถูกสำรวจ
- ที่อยู่ที่ถูกข้ามถูกนับและส่งกลับจาก run (และ log ไว้) เพราะ invite 29 ใบหายจาก batch เงียบ ๆ อ่านเหมือน crew ที่ไม่ได้มาทำงาน

---

## [1.173.0] — 2026-08-10

### Added — สำรวจเมื่อ COMPLETED, ตามเก็บรอบที่หลุด, และถามถึงตัวบริการ

operator ขอเมลความพอใจ + feedback เมื่องานจบ เพื่อเก็บข้อมูลว่าบริการงานผลิตส่งมอบได้ดีแค่ไหน · นั่นคือสิ่งที่ peer review ที่หลับอยู่ตั้งแต่ v1.166 ทำอยู่แล้ว จึง**ต่อยอดแทนที่จะตั้งแบบสำรวจใบที่สอง** — ผู้ตอบชุดเดียวกัน (ทุกคนที่อยู่ในกองจริง) สัญญาไม่ระบุชื่อชุดเดียวกัน ที่เก็บ append-only เดียวกัน

- **ทริกเกอร์เป็นสถานะของ booking เอง** — `status: 'COMPLETED'` แทนการคำนวณวันที่ และรอบการรันเรียก `autoCompleteBookings()` เอง แบบสำรวจจึงไม่ขึ้นกับว่าเช้านั้นมีใครเปิดแอปให้สถานะพลิกแบบ lazy หรือเปล่า
- **ตามเก็บรอบที่หลุด** — ตัวส่งเดิมแมตช์วันปฏิทินเดียวเป๊ะ ๆ เช้าไหนไม่ได้รันคืองานนั้นถูกข้าม**ถาวร** ไม่มี retry และไม่มีอะไรบอกว่ามีวันที่หลุดไป · ตอนนี้ทำงานเป็น window (delay 1 + `SHOOT_REVIEW_LOOKBACK_DAYS` 7 รวมปลายทั้งสองข้าง) เก่าก่อน จำกัดที่ `SHOOT_REVIEW_MAX_BOOKINGS` ต่อรอบ การเปิดฟีเจอร์จึงกลายเป็นการส่งเมลก้อนใหญ่ครั้งเดียวไม่ได้ · อะไรที่ cap เลื่อนไว้ถูก log และส่งกลับ ไม่เคยหายเงียบ
- **คำถามระดับงาน** — คะแนนต่อทีมตอบว่า "ทีมกล้องทำได้ยังไง" ไม่ได้ตอบว่า "งานนี้ได้รับบริการดีไหม" · คำถามใหม่ถามข้อหลังตรง ๆ เก็บเป็นแถว `ShootReview` ที่ `targetRole` = `'overall'` ไม่ต้อง migrate และไหลเข้ารายงาน + CSV เดิม · **จงใจไม่ใส่ใน `REVIEW_TARGET_ROLES`** เพราะถ้าอยู่ในลิสต์นั้น `targetsFor` จะแจกหรือหักมันตามบทบาทของผู้ตอบเอง
- dry run ทำได้ขณะฟีเจอร์ยังปิดอยู่ (จะตัดสินใจว่าจะเปิดไหมต้องเห็น batch ก่อน — การส่งจริงยังถูกกั้น) และคืนตัวอย่างเมลจริงที่สร้างด้วยฟังก์ชันเดียวกับที่ใช้ส่ง
- monitor เลิกรายงาน PREVIEW invite ที่ยังไม่ได้ส่งว่าเป็นความล้มเหลวในการส่ง ซึ่งทำให้ไฟค้างแดงโดยที่ operator ทำอะไรกับมันไม่ได้
- ยืนยัน end-to-end บน Postgres ชั่วคราว: ฟอร์มเขียน camera 4 / sound 5 / overall 4 พร้อมคอมเมนต์ · dry run เก็บงาน COMPLETED ที่อยู่ใน window และงาน CONFIRMED ที่มันปิดให้เอง ข้ามงานอายุ 20 วันและงานที่ยกเลิก · cap 2 เลื่อนงานใหม่สุดพร้อม log บอกว่าเลื่อน
