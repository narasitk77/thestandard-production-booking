...
6. **Name-fallback removal** → **ยกเลิกการตัด** (แก้คำตอบเดิมของวันเดียวกัน หลังรีวิว 11-agent + ตรวจโค้ดยืนยัน 2026-08-06). เดิมตอบว่า "ตัดก่อน cutover" ซึ่ง**ผิด** — หลักฐาน:
   - **ตัวเลขที่ใช้เป็น gate เชื่อไม่ได้**: `isFolderAlive` (google-drive.ts) เป็น `catch { return false }` ครอบ `files.get` ที่**ไม่มี retry** ทั้งที่ `withDriveRetry` มีอยู่แล้ว → 429 หนึ่งครั้งทำให้ id ที่ดีอยู่ดูเหมือน "ตาย" แล้วไหลไปเดินชื่อ และถูกนับเป็น fallback · ตัวเลข ~1/วัน จึงปน "ลิงก์หาย" กับ "Drive สะดุด" · แถม `noteResolve` ยิงเฉพาะตอน**สำเร็จ** สถานะที่อันตรายจริง (มี id + id ตาย + ชื่อก็หาไม่เจอ) ไม่เคยถูกนับเลย
   - **2 key เป็น id-only ไม่ได้เชิงนโยบาย**: landing (v1.140 = drop zone ถาวร ops/NAS สร้างมือ) และ staging (ทีมเสียงสร้างมือ) — ไม่มีโค้ดไหนเก็บ id ของโฟลเดอร์ที่คนสร้าง กลุ่มนี้เกิดใหม่เรื่อยๆ ตามพฤติกรรมทีม
   - **1 จุดตัดแล้วอันตรายกว่าเดิม**: `agn-restructure.ts:211-214` คือ `stored ?? name ?? ensureFolderPath` — ตัดตัวกลางออก booking layer ที่มีอยู่แต่ยังไม่มี id จะถูกมองไม่เห็น → **สร้างกล่องที่สอง** แล้ว `rememberDriveLinks` เขียน id กล่องเปล่าทับ = poison ทุก reader ที่อ่าน id · ขัดกฎ §0 ตรงๆ
   **สัญญาที่ใช้แทน**: *id-first, name-repair, never name-authoritative* — เชื่อเฉพาะ stored id ที่ probe แล้ว alive; ชื่อใช้ได้แค่ "ซ่อม" (ต้อง unique + เขียน id ทันที + รายงานเป็น defect); **ห้ามตัด arm ที่กัน `create`**; ปลายทางกำกวม = ปฏิเสธ, ต้นทางกำกวม = เลือกตัวแรกแล้วรายงาน; `unknown` ≠ `not found`
   **ทำไปแล้ว v1.165**: `folderLiveness()` tri-state + retry (alive/dead/unknown), `isFolderAlive` คงความหมาย fail-closed ไว้ (หลาย call site ใช้เป็นการ์ดก่อน mutate) แต่ได้ retry แล้ว, และ `upload/init` เปลี่ยนเป็น "unknown = เชื่อ id เดิม" เพื่อไม่ให้ Drive สะดุดครั้งเดียวไปเข้า create-on-miss จนกล่องแตกสองใบ
   **ที่เหลือ**: defect reporting + ambiguity refusal + backfill แบบ `findFoldersByCode` เป็นงานก้อนถัดไป (รีวิวเจอ 18 ประเด็น รวม blocker 4 บนแผนฉบับเต็ม) — **ไม่ใช่ prerequisite ของ reconciler อีกต่อไป** parallel-run เทียบกับต้นแบบที่มี fallback ได้ตามปกติ
