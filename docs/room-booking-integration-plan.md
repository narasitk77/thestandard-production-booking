...
> **ทบทวน + ทดสอบซ้ำกับของจริง 2026-08-25 — §12 คือข้อสรุปล่าสุด อ่านก่อน §1**
...
## 12. ทบทวน 2026-08-25 — สิ่งที่เปลี่ยนไปจากแผนเดิม (ยิงจริงกับ API แล้ว)

### 12.1 สามเรื่องที่ทำให้แผนเดิมต้องแก้

**(1) read API เปิดหมด ไม่ต้อง auth เลย** — ทดสอบ 2026-08-25 ไม่แนบ token:

| endpoint | ผล |
|---|---|
| `GET /api/liff/rooms` | **200** — 15 ห้อง ตรงกับตาราง §4 ทุกตัว ไม่มีห้องเพิ่ม/หาย |
| `GET /api/liff/room-slots?roomId=&date=` | **200** — คืนช่วงที่ถูกจองจริง |
| `GET /api/liff/bookings-calendar?year=&month=` | **200** — คืน booking ทั้งระบบ |
| `GET /api/liff/check-conflict` | **200** |
| `GET /api/liff/me` | **401** `{"error":"No auth token"}` |

→ **ครึ่ง "อ่าน" ของงานนี้ทำได้ทันที ไม่ต้องรอ IT** เหลือเฉพาะ `POST /booking` ที่ต้องมี credential

**(2) ⚠️ `check-conflict` ใช้ `startAt`/`endAt` เป็น UTC ISO ไม่ใช่ `startDate`+`startTime`**
— และถ้าส่งพารามิเตอร์ผิด **มันตอบ `null` แปลว่า "ว่าง"** ไม่ได้ตอบ error

ทดสอบกับช่องที่มีคนจองจริง (ห้อง 2 · 27 ส.ค. 13:00–14:00 BKK):

```
?roomId=2&startDate=2026-08-27&startTime=13:00&endDate=…&endTime=14:00
  → null                                    ❌ false negative (แผนเดิมเขียนแบบนี้)
?roomId=2&startAt=2026-08-27T06:00:00.000Z&endAt=2026-08-27T07:00:00.000Z
  → {"startAt":…,"endAt":…,"title":"Digital Vaultwarden"}   ✅ เจอ conflict
```

**นี่คือกับดักที่อันตรายที่สุดในงานนี้**: ถ้า implement ตามแผนเดิม ระบบจะ "ตรวจแล้วว่าง" ทุกครั้ง
แล้วจองทับของคนอื่นโดยที่ log บอกว่าเช็กแล้ว (รูปเดียวกับบทเรียน *a record is not delivery*)
→ **ต้อง assert ด้วยว่าเคสที่รู้ว่าชน ต้องคืนค่าไม่ใช่ null** ไม่งั้นเทสผ่านแบบหลอก ๆ

**(3) ระบบจองห้องแทบไม่มีคนใช้** — นับจาก `bookings-calendar` ทั้งปี 2026:

| เดือน | 01 | 02 | 03 | 04 | 05 | 06 | 07 | 08 | 09 |
|---|--:|--:|--:|--:|--:|--:|--:|--:|--:|
| จำนวน | 0 | 0 | 0 | 9 | 1 | 3 | 0 | 1 | 0 |

**14 การจอง ใน 9 เดือน ทั้ง 15 ห้อง** (ล่าสุด `BK-0047`) เทียบกับโปรบุ๊คตั้งแต่ 1 มิ.ย.:
**311 คิวถ่าย — ในสตูดิโอ 143 · ห้องอื่นในตึก 57 · นอกสถานที่ 78**

นัยสองข้อ:
- เป้าหมายเดิม *"โปรดิวเซอร์จะได้ไม่ต้องจอง 2 ที่"* **ไม่ตรงกับความจริง** — ทุกวันนี้เขาไม่ได้จองที่สอง
  อยู่แล้ว. ประโยชน์จริงของงานนี้คือ **ทำให้แผนกอื่นเห็นว่าห้องไม่ว่าง** ไม่ใช่การลดภาระโปรดิวเซอร์
- แต่นี่เป็น **เหตุผลที่ดีที่จะขอ IT**: โปรบุ๊คจะเติมข้อมูลให้พอร์ทัลเขา ~200 รายการ/ไตรมาส
  จากปัจจุบัน 14 รายการ/9 เดือน — พอร์ทัลถึงจะมีประโยชน์จริง

### 12.2 อุปสรรคตัวจริงอยู่ฝั่งเรา ไม่ใช่ฝั่ง IT

`Booking` **ไม่มีคอลัมน์ `locationId`** — เก็บแค่ `locationName` ที่เป็น "ข้อความที่เรนเดอร์แล้ว"

ทั้งที่ฟอร์มจองรู้ id อยู่แล้ว: `BookingWizard.tsx` ให้เลือกจาก `LOCATIONS` และ validate
`locationId` (บรรทัด 662) แต่ตอนส่งขึ้น server บรรทัด 814 ส่ง `locationName: resolvedLocationName`
แล้ว **ทิ้ง id**

ผลบนพรอด — ห้องเดียวกันมีได้หลายสะกด:

| ข้อความใน DB | จำนวน | ห้องจริง | มาจาก |
|---|--:|---|---|
| **`สตูดิโอ 1`** | **129** | Studio 1 | RoutinePlanner (128 ใบ) |
| `Studio 1 (TSD)` | 65 | Studio 1 | BookingWizard |
| `Studio 1` | 11 | Studio 1 | — |
| `Studio 2 (TSD, 1/F)` | 56 | Studio 2 | BookingWizard |
| `war room ชั้น 4` · `War Room ชั้น 4` · `War Room` | 6 | War Room | — |
| `สตู 1` · `สตู2` · `สตูดิโอ 2` | 4 | Studio 1/2 | — |

*(ตัวเลขทั้งหมด = ทุกใบที่ยังไม่ถูกลบ ตรวจ 2026-08-25)*

**ค่าที่พบมากที่สุดในทั้งระบบคือ `สตูดิโอ 1`** — มากกว่าชื่ออังกฤษที่ฟอร์มสร้างเสียอีก
และ 128 ใบมาจาก **`RoutinePlanner.tsx` ซึ่ง Location เป็นช่องพิมพ์อิสระ** (placeholder
"เช่น Studio 1") ไม่ใช่ picker — คนพิมพ์ไทยครั้งเดียว แล้วมันสร้างไป 128 ใบรวด
84 ใบยังเป็น REQUESTED มีวันถ่ายไกลถึง 31 ธ.ค. 2026

→ **ถ้าแก้แต่ BookingWizard จะพลาดแหล่งที่ใหญ่ที่สุด** — ต้องแก้ RoutinePlanner ด้วย

**ถ้าแมป `locationName` → `roomId` ด้วยการจับคู่ข้อความ มันจะพังเงียบ ๆ แน่นอน** — `สตูดิโอ 1`
38 ใบจะไม่เข้าเงื่อนไขไหนเลยถ้าเขียนกฎจากชื่ออังกฤษ และ *"แมปด้วย id ตายตัว ห้ามแมปด้วยชื่อ"*
ที่ §4 เขียนไว้ **ทำไม่ได้จริง** ตราบใดที่ยังไม่มี id ใน DB

โรคเดียวกับที่โปรบุ๊คเคยเป็นแล้วแก้ไปแล้ว: **v1.114 id-first Drive links** (เก็บ folder id
แทนชื่อ เพราะการเปลี่ยนชื่อทำให้ทุกอย่างพัง) — ใช้ยาตัวเดิม

### 12.3 ลำดับงานที่แนะนำ (แก้จาก §10)

| เฟส | งาน | ต้องพึ่ง IT | ได้อะไร |
|---|---|:--:|---|
| **1** | `Booking.locationId` (เพิ่มคอลัมน์อย่างเดียว) + ฟอร์มส่ง id + backfill ข้อความเดิม → id | ❌ | ฐานที่ทำให้ทุกเฟสหลังจากนี้แมปห้องได้แน่นอน |
| **2** | `src/lib/room-booking.ts` ฝั่งอ่าน: `roomBusySlots()` / `checkRoomConflict()` (ใช้ `startAt`/`endAt`) + เตือนตอน approve ว่า "ห้องนี้มีคนจองในระบบกลางแล้ว" | ❌ | กันจองชนได้จริง + ทดสอบ contract ทั้งหมดก่อนแตะการเขียน |
| **3** | `POST /booking` จริง + `roomBookingId/Status` + reconciler + อ่านกลับมายืนยัน + แจ้ง Discord เมื่อล้ม | ✅ **credential** | จองออโต้เต็มตัว |
| **4** | lifecycle: แก้ห้อง/เวลา → cancel+จองใหม่, ยกเลิกงาน → `PATCH cancel` | ✅ | ไม่มีการจองค้าง |

เฟส 1–2 ทำได้เลยวันนี้และเป็นของที่ต้องมีอยู่ดีไม่ว่าจะจบที่ Option ไหน

### 12.4 กฎความเสถียรที่ต้องบังคับในเฟส 3

- **อ่านกลับมายืนยันเสมอ** — หลัง POST ต้อง `room-slots` ซ้ำแล้วเห็นการจองของเรา ถึงจะบันทึกว่า `OK`
  (ห้ามเชื่อ 200 จาก POST — บทเรียน v1.186)
- **idempotency**: มี `roomBookingId` แล้วห้ามจองซ้ำ (CAS เหมือน `calendarEventId`)
- **flag `ROOM_BOOKING_ENABLED` + เปิดทีละห้อง** เริ่มที่ Studio 1/2 ซึ่งเป็น 143 จาก 200 ใบ
- **ข้ามแทนเดา**: EXTERNAL / Lounge (2/F) / นอก 08:00–21:00 / ข้ามวัน → `SKIPPED` + บอกเหตุผล
  ห้าม clamp เวลาเงียบ ๆ
- **การจองห้องล้ม ห้ามทำให้ approve ล้ม** (เส้นเดียวกับ calendar/Drive)

### 12.5 คำถามถึง IT — เหลือน้อยลงมาก

ตอบเองได้แล้วจากการทดสอบ: Q3 (บางส่วน), Q4, Q6 (5A มีจริง ยังไม่รู้ว่าห้องอะไร)

**เหลือที่ต้องถามจริง ๆ:**
- **Q2 (ตัวบล็อกเดียว):** ขอ credential ให้โปรบุ๊คเรียก `POST /api/liff/booking`
  แบบ server-to-server ได้ไหม — รับได้ทั้ง Google ID token จาก service account ที่มีอยู่แล้ว,
  API key, หรือ service user + `register-email`
- **Q3′:** required fields ของ `POST /booking` และรูปแบบเวลา (`startAt`/`endAt` เหมือน
  check-conflict หรือ `startDate`+`startTime` เหมือนฟอร์ม?) — สองที่ในระบบเขาใช้คนละแบบ
- **Q5:** รับจองข้ามวัน / นอก 08:00–21:00 ไหม
- **Q7:** แจ้งก่อนเปลี่ยน API ได้ไหม (API ภายในไม่มีเอกสาร)
