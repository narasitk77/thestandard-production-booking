# สถานะปัจจุบัน — 2026-09-23 · v1.234.2
...
## 1. ของที่เพิ่ง ship (v1.228 → v1.234.2)
...
| **1.234.0** | `a11c709` | ตารางวันหยุดรู้ตัวว่า "ไม่มีข้อมูลปีนี้" — `isThaiHoliday()` คืน false ทั้งกับ *ไม่ใช่วันหยุด* และ *ไม่มีข้อมูลทั้งปี* สองอย่างนี้หน้าตาเหมือนกันแต่แก้คนละทาง · สร้าง routine ข้ามปีด้วย `skipHolidays:true` จะดู "ทำงานอยู่" ทั้งที่ไม่ได้ข้ามอะไรเลย · เพิ่ม `holidayYearCovered()` / `uncoveredHolidayYears()` + คำเตือนสีเหลืองในฟอร์ม |
| **1.234.1** | `d630b19` | **คำเตือนข้างบนเงียบเอง** — `COVERED_YEARS` derive จาก `THAI_HOLIDAYS.map(...)` ปีที่มีวันหยุดอยู่วันเดียว (2027 มีแต่ 1 ม.ค.) เลยถูกนับว่าครบทั้งปี · เปลี่ยนเป็นลิสต์เขียนมือ `new Set(['2026'])` เติมปีใหม่ **หลัง** ลงวันครบเท่านั้น · เทสเดิมล็อกพฤติกรรมผิดไว้ด้วย (คาด `['2028']`) |
| **1.234.2** | `c917d93` | ช่อง Producer email ในฟอร์มสร้าง routine — ฟอร์มไม่เคยส่ง `producerEmail` และ `create-booking.ts:389` ไม่มี fallback → ทุกใบจากหน้านี้อีเมลเป็น null แล้วหลุด `scope=mine` ของ `/my-bookings` (v1.196) = โปรดิวเซอร์มองไม่เห็นงานตัวเอง |
...
### 2.0 🟠 `approve` ไม่ทิ้งร่องรอยว่า **ใคร** อนุมัติ (พบ 2026-09-24)

ตาราง `bookings` มีแค่ `approvedAt` — **ไม่มี `approvedByEmail`** และเส้นสำเร็จของ
`src/app/api/admin/[id]/approve/route.ts` **ไม่เรียก `logAudit` เลย** (เขียน audit
เฉพาะตอนพัง: `calendar.approve_failed`)

พิสูจน์แล้วกับพรอด: อนุมัติ 59 ใบรวดเดียว แล้ว `audit_logs` ใน 90 นาทีนั้นมีแค่
`booking.room_reserved` / `audit.auto_email_sent` / `drive.folder_integrity`
— ไม่มีแถวไหนบอกว่าใครกดอนุมัติสักใบ

ทำไมถึงสำคัญ: approve คือคำสั่งที่แพงที่สุดในระบบ (สร้าง event ปฏิทิน ส่ง invite
สร้างโฟลเดอร์ Drive เขียนชีท และปลุก worker จองห้องให้ยิงเข้าระบบของทีมอื่น)
เวลามีคนถามว่า "ใครสั่งจองห้องนี้" เราตอบไม่ได้

### 2.0.1 🟠 ใบ routine ไม่มี `video@` / `Sound@` ในทีมงาน (พบ 2026-09-24)

ทุกใบของ `now-news-2026-08/09` มี `video@thestandard.co` + `Sound@thestandard.co`
อยู่ใน `assignedEmails` เสมอ (แล้วค่อยเติมคนรายวันทับ) แต่ใบที่ `RoutinePlanner`
สร้างมา `assignedEmails` **ว่างเปล่า**

ผลคือ `bookingCalendarAttendees()` คืนแค่ `producerEmail` ⇒ event ปฏิทินมีแขกคนเดียว
⇒ **ทีมวิดีโอและทีมเสียงไม่เห็นงานพวกนี้บนปฏิทินของตัวเอง** จนกว่าจะมีคนกด
"เพิ่มทีมงานทั้งชุด" (v1.230 — merge ไม่ใช่ทับ)

ยืนยันกับ event จริงบนปฏิทิน: ใบที่อนุมัติไว้ก่อนหน้า (1–2 ต.ค. ที่มี crew) มีแขก 3 คน
`sarut.a@ / video@ / sound@` ส่วนใบที่เพิ่งอนุมัติมีแขกคนเดียว
