# สถานะปัจจุบัน — 2026-09-25 · v1.238.0
...
## 1. ของที่เพิ่ง ship (v1.228 → v1.238.0)
...
### 2.0.1 ✅ แก้แล้วที่ v1.237 — `sheetRowIndex` ผิด 39 ใบ (มีสองสาเหตุ)

ตรวจของจริง (อ่านคอลัมน์ A ทั้งแท็บ 700 แถวเทียบกับ DB): **ชีทสะอาด** ทุกใบมีแถวครบ
ไม่มีแถวซ้ำ ไม่มีแถวหาย — ผิดแค่ตัวชี้ใน DB และแยกได้เป็นสองสาเหตุคนละแบบ:

| กลุ่ม | ลายเซ็น | สาเหตุ | แก้ได้ไหม |
|---|---|---|---|
| 34 ใบ | "แถวจริง" มากกว่า "ที่เก็บ" เสมอ (+1..+14) ทั้งหมดในชุดที่สร้าง 20 ใบใน 7 วินาที | `values.append` ที่ยิงพร้อมกันคืน `updatedRange` ไม่ตรงแถวที่เขียนจริง | ✅ ต่อคิว (`src/lib/serial-queue.ts`) |
| 5 ใบ | 4 ใบ +1 ที่แถวติดกัน 355–361 · 1 ใบ **−1** | มีคนแทรก/ลบแถวในชีทเอง เลขที่เคยถูกกลายเป็นผิดทีหลัง | ❌ โค้ดกันไม่ได้ จะเกิดอีก |

**กลุ่มที่สองไม่เป็นไร** เพราะไม่มีโค้ดตรงไหน dereference เลขนี้ — `updateBookingRow`
หาแถวจากคอลัมน์ A เสมอ (จงใจ กันคนแทรก/ลบ/เรียงแถวเอง) ค่านี้ทำหน้าที่แค่ธง
"ใบนี้มีแถวในชีทแล้ว" · **ถ้าวันหนึ่งจะมีใครเอาเลขนี้ไปเขียนทับแถวตรง ๆ ต้องเลิกคิด
แบบนั้น หรือเปลี่ยนคอลัมน์เป็น boolean ก่อน**

ตัวซ่อมประจำ: `POST /api/admin/backfill-bookings-sheet` — pass CLAIM ซ่อมทั้งค่าว่าง
และค่าที่ผิด (dry-run ก่อนเสมอ) · รันใหม่ได้ทุกครั้งที่สงสัยว่ามีคนไปขยับแถวในชีท

**บทเรียนจากการแก้รอบนี้** (รีวิวจับได้ก่อน deploy): การต่อคิวสร้างความเสี่ยงใหม่ที่
ไม่มีตอนยิงขนาน — งานที่ค้างหนึ่งชิ้นกันคิวได้ทั้งโปรเซส ⇒ `jobTimeoutMs` เป็น
พารามิเตอร์ **บังคับ** ของ `makeSerialQueue` · และคิวรอบแรกครอบไม่ถึง `ensureSheetTab`
ซึ่งเขียน A1 ทุกครั้งอยู่นอกคิว (1 append = 2 เขียน) ⇒ แพตช์เกือบอ้างว่าแก้ 429
ทั้งที่ยังไม่ได้แก้

### 2.0.2 ✅ แก้แล้วที่ v1.238 — log 96% เป็นขยะ · ลูป restart ไม่เคยรอดการแครช

worker ที่ถูกปิด 2 ตัวผลิต 6,654 จาก 6,898 บรรทัดใน 16 ชม. (ตัวละ 1,663 รอบ)
เพราะ supervisor ปลุกใหม่ทุก ~35 วินาที ทั้งที่ **env ของคอนเทนเนอร์ที่รันอยู่แก้ไม่ได้**
⇒ การปลุกซ้ำไม่เคยเปลี่ยนอะไรได้เลย · ตอนนี้ worker ที่ปิดออกด้วย exit 78 และ
supervisor หยุดปลุก เหลือ 2 บรรทัดต่ออายุคอนเทนเนอร์

**ของที่ใหญ่กว่าซึ่งเจอระหว่างทาง:** `start.sh` รันด้วย `set -e` และของเดิมเรียก
`node scripts/x.js` เปล่า ๆ ในตัวลูป ⇒ ลูป "restart" รอดเฉพาะ exit 0 · worker ที่
แครชจริงฆ่า supervisor ตัวเองทิ้งเงียบ ๆ ไม่มี log และไม่กลับมาอีก
แก้เป็น `code=0; node "$script" || code=$?` พร้อม backoff 5→…→300 วินาที

**กฎที่ได้มา:** ความหมายของ log บรรทัดหนึ่งเปลี่ยนได้เพราะโค้ดที่ผลิตมันเปลี่ยน —
