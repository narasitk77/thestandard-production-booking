# สถานะปัจจุบัน — 2026-09-24 · v1.236.0
...
## 1. ของที่เพิ่ง ship (v1.228 → v1.236.0)
...
### 2.0 ✅ ปิดแล้วที่ v1.235 — `approve` ไม่ทิ้งร่องรอยว่าใครอนุมัติ / ใบ routine ไม่มีทีมงาน

ทั้งสองข้อแก้แล้ว (`Booking.approvedByEmail` + audit `booking.approve` ·
`teamMailboxesForCrew()` + checkbox ในฟอร์ม routine) และ backfill ข้อมูลเดิมครบแล้ว:
61 ใบ ต.ค.–ธ.ค. 2026 + 63 ใบ ม.ค.–มี.ค. 2027 มี `video@` + `sound@` ครบ
รายละเอียดใน `docs/ops-log.md` รายการวันที่ 2026-09-24

**ใบ 59 ใบที่อนุมัติไปก่อน v1.235 มี `approvedByEmail` เป็น null** — จงใจไม่เติมย้อนหลัง
ค่าว่างแปลว่า "ไม่ได้บันทึกไว้" ซึ่งเป็นความจริง การเติมทีหลังจะทำให้คอลัมน์นี้
เชื่อไม่ได้ตั้งแต่แถวแรก

### 2.0.1 🟠 `sheetRowIndex` ซ้ำกันข้ามใบ — 32 ใบเก็บเลขแถวผิด (พบ 2026-09-24)

9 แถวในชีทถูกอ้างโดยหลายใบ (แถว 638 ถูกอ้างโดย 12 ใบ) ทั้งหมดคือใบ Now ปี 2027
ที่สร้างรวดเดียว — `appendBookingRow` ถูกยิงแบบไม่ await หลายตัวพร้อมกัน
แต่ละตัวอ่านความยาวชีทแล้วได้เลขเดียวกัน

**ยังไม่ทำอันตราย**: `grep sheetRowIndex` ทั้งรีโปแล้ว ทุกจุดใช้มันเป็นแค่ธง
`if (booking.sheetRowIndex)` = "ใบนี้มีแถวในชีทแล้ว" ส่วนแถวจริง `updateBookingRow`
หาโดยสแกนคอลัมน์ A เสมอ (จงใจ — กันคนแทรก/ลบ/เรียงแถวเองในชีท)

เป็นกับดักสำหรับคนที่มาเชื่อตัวเลขนี้ทีหลัง · ทางแก้ที่ถูกคือให้ `appendBookingRow`
อ่านเลขแถวจาก response ของ append (`updates.updatedRange`) แทนการอ่านความยาวก่อน
