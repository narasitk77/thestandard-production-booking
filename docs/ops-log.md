...
จาก `evaluateWorkers()` เอง) แต่ระหว่างอ่าน log เจอว่า **6,654 จาก 6,898 บรรทัด
(96%) ใน 16 ชั่วโมงเป็นเสียงรบกวนจาก worker 2 ตัวที่ถูกปิดไว้** ตัวละ 1,663 รอบ
ทุก ~35 วินาที (worker พิมพ์ว่าปิดอยู่ → ค้าง 30 วินาทีเพื่อ "ชะลอ" → supervisor
sleep 5 → ปลุกใหม่) · ผลคือ log เป็นที่ที่หา error จริงไม่เจอ — ตอนไล่เคสจริง
grep คำว่า `429` ได้ 46 ครั้งซึ่งเป็นเลขในไทม์สแตมป์นาโนวินาทีล้วน ๆ

**เหตุผลที่คอมเมนต์ให้ไว้ว่า "ปลุกซ้ำเพื่อให้สลับ env ใน Portainer แล้วติดเลย"
เป็นไปไม่ได้** — env ของคอนเทนเนอร์ที่รันอยู่แก้ไม่ได้ (Docker ไม่มี API) และการ
อัปเดต stack คือการสร้างคอนเทนเนอร์ใหม่ ซึ่ง worker อ่านค่าใหม่ตอน launch แรก
อยู่แล้ว ⇒ การปลุกซ้ำ 1,663 ครั้งไม่เคยเปลี่ยนอะไรได้เลยสักครั้ง
→ `exitDisabled()` (exit 78) + supervisor หยุดปลุก ⇒ เหลือ 2 บรรทัดต่ออายุคอนเทนเนอร์

### ของที่ใหญ่กว่าที่เจอระหว่างทาง: ลูป restart ไม่เคยรอดการแครช

`start.sh` รันด้วย `set -e` · คำสั่งเปล่า ๆ ในตัวลูปที่ออกไม่เป็นศูนย์จะฆ่า subshell
ทันที · ของเดิมเขียน `node scripts/x.js` เปล่า ๆ แปลว่า **ลูปที่ทั้งไฟล์เรียกว่า
"restart loop" รอดเฉพาะการ exit 0** worker ที่แครชจริงจะฆ่า supervisor ตัวเองทิ้ง
**เงียบสนิท ไม่มี log สักบรรทัด** และไม่กลับมาอีกตลอดอายุคอนเทนเนอร์
พิสูจน์ด้วยการรันจริงเทียบสองแบบใต้ `set -e` → แก้เป็น `code=0; node "$script" || code=$?`

### ผลพวงที่ต้องกันต่อ (สองชั้น)
1. พอลูป restart ทำงานจริงเป็นครั้งแรก worker ที่พังซ้ำจะพ่น 12 บรรทัด/นาที =
   ปัญหาเดิมในรูปใหม่ → backoff ไต่ 5→10→20…→300 วินาที รีเซ็ตเมื่ออยู่ได้เกิน 1 นาที
2. ตัวตรวจ Hermes มี `supervisor: worker exited` อยู่ใน SKIP_RE ซึ่ง **เดิมถูก**
   (บรรทัดนั้นมีแต่ตอน worker ที่ปิด exit สะอาด) แต่พอลูปรอดการแครชแล้ว บรรทัด
   เดียวกันเปลี่ยนความหมายเป็น "แครชวน" ซึ่งต้องมีคนเห็น → ย้ายรูปที่มี `(code N)`
   ไป BAD_RE · **ความหมายของ log บรรทัดหนึ่งเปลี่ยนได้เพราะโค้ดที่ผลิตมันเปลี่ยน —
   ตัวกรองที่เคยถูกจึงกลายเป็นตัวปิดหูได้**

### เกร็ดวิธีทำงาน
- เทสรอบแรกของ supervise() **ไม่มี `set -e`** จึงเห็น crashy restart ได้ทั้งที่ของจริง
  จะตาย · จับได้ตอนกลับไปอ่าน Dockerfile ว่า entrypoint รันยังไง
- ตรวจ entrypoint ต้องใช้เชลล์จริง: `docker run --rm node:20-alpine sh -n start.sh`
  และรัน `supervise()` จริงในคอนเทนเนอร์ ไม่ใช่แค่ bash บนแมค
- ยุบบล็อก supervisor 15 ชุดที่ก๊อปกันมาเหลือฟังก์ชันเดียว (-39 บรรทัด)

---

## 2026-09-24 · v1.237 — `sheetRowIndex` ผิด 39 ใบ: สองสาเหตุ และแพตช์แรกที่เป็นการถดถอย

นัทสั่ง "แก้ sheetRowIndex ซ้ำด้วย" · สมมติฐานแรกของผมผิด — `appendBookingRow`
อ่านเลขแถวจาก `updates.updatedRange` ของ response อยู่แล้ว ไม่ได้อ่านความยาวชีทก่อน
จึงไปเทียบของจริง: อ่านคอลัมน์ A ทั้งแท็บ (700 แถว) ชนกับ DB

**ชีทสะอาด** ทุกใบมีแถวครบ ไม่ซ้ำ ไม่หาย · ผิดแค่ตัวชี้ 39 ใบ และมี **สองสาเหตุ**:
34 ใบเป็นลายเซ็นของ append ที่ยิงพร้อมกัน (แถวจริง > ที่เก็บ เสมอ +1..+14 ทั้งหมด
อยู่ในชุดที่สร้าง 20 ใบใน 7 วินาที) · อีก 5 ใบเลขเคยถูกแล้วกลายเป็นผิด (4 ใบ +1
ที่แถวติดกัน · 1 ใบ **−1**) = มีคนแทรก/ลบแถวในชีทเอง ซึ่งโค้ดกันไม่ได้

**แพตช์รุ่นแรกของผมเป็นการถดถอย — รีวิวจับได้ก่อน deploy**

1. คิวที่ไม่มี timeout อันตรายกว่าไม่มีคิว: request ค้างหนึ่งตัวกันคิวได้ตลอดอายุ
   คอนเทนเนอร์ ⇒ ใบจองทุกใบหลังจากนั้นไม่ลงชีทและเงียบสนิท (create-booking ปิดท้าย
   ด้วย `.catch(() => {})`) · เดิมยิงขนาน ค้างหนึ่งตัวเสียหนึ่งใบ — blast radius
   ต่างกันคนละเรื่อง → `jobTimeoutMs` เป็นพารามิเตอร์บังคับ (type ฟ้องถ้าไม่ใส่)
2. คิวครอบไม่ถึงตัวที่ระเบิดจริง: `ensureSheetTab` ถูก await **นอกคิว** และเขียน A1
   ทุกครั้งแบบไม่มีเงื่อนไข ⇒ 1 append = 1 อ่าน + 2 เขียน และ 2 ใน 3 ไม่ผ่านคิว
   เลข 1100ms คิดผิดครึ่งหนึ่ง และ burst ที่ทำแถวหาย 12 แถวเมื่อวานยังอยู่ครบ —
   **แพตช์กำลังจะอ้างว่าแก้สิ่งที่ยังไม่ได้แก้** → memoize ต่อโปรเซส + ย้ายเข้าคิว

ทำเพิ่มอีก 6 ข้อ ข้อละไม่กี่บรรทัด: append ที่ล้มต้องดัง (เดิม `.catch(() => {})`
และไม่เช็ก null) · backfill ที่คืน `errors: 0` ทั้งที่ล้มทุกใบ · re-entrancy guard
(504 แล้วยิงซ้ำ = แถวซ้ำ) · throttle pass 3 ให้เท่า pass 4 · รายงาน `duplicateCodes` ·
audit ของการซ่อม · และข้ามใบที่เพิ่งสร้าง <5 นาที เพราะคิวของผมเองทำให้
"ยังไม่มีแถว" ไม่ได้แปลว่า "จะไม่มี"

**บทเรียน: การเปลี่ยนจากขนานเป็นอนุกรมไม่ใช่การแก้ที่ปลอดภัยโดยอัตโนมัติ —
มันย้ายความเสี่ยงจาก "เสียทีละชิ้น" ไปเป็น "เสียทั้งสาย" ต้องมีทางปล่อยเสมอ**

---

## 2026-09-24 · v1.235–v1.236 — อนุมัติ Now ต.ค.–ธ.ค. 59 ใบ แล้วเจอสี่อย่างระหว่างทาง

นัทสั่ง "อนุมัติชุด ต.ค. พ.ย. ธ.ค." · **สำเร็จ 59/59 ไม่มีอะไรหลุด** ที่จังหวะ 5 วินาที/ใบ
(ปุ่มใน UI ใช้ 3) ยืนยันครบทุกปลายทาง: DB · ปฏิทิน (อ่าน events.get จริง ไม่มี event ซ้ำ) ·
ชีท (แถวที่ไม่ขึ้น CONFIRMED = ใบ CANCELLED พอดีทั้ง 5 ใบ)

**สิ่งที่การอนุมัติรอบนี้เปิดโปง**

1. **ไม่มีใครรู้ว่าใครกดอนุมัติ** — `audit_logs` ในชั่วโมงนั้นไม่มีแถวไหนบอกเลย
   approve เขียน audit เฉพาะตอนพัง และ `bookings` มีแค่ `approvedAt`
   → v1.235 เพิ่ม `approvedByEmail` + audit `booking.approve` · ต้องมีทั้งคู่เพราะ
   audit ถูกล้างที่ 90 วัน ส่วนใบจองอยู่ล่วงหน้าได้ 3 เดือน
   → เส้นอื่นที่พาใบไป CONFIRMED (PATCH, force-status) เซ็ตคู่นี้ด้วย และ restore ล้างคู่กัน

2. **ใบ routine เกิดมาไม่มีทีมงาน** — ใบ Now ส.ค./ก.ย. (migrate มา) มี video@ + Sound@
   ครบทุกใบ แต่ใบที่ `/admin/routine` สร้างมี `assignedEmails` ว่าง ⇒ แขกปฏิทินเหลือ
   producer คนเดียว ทีมวิดีโอ/เสียงไม่เห็นงานบนปฏิทินตัวเอง
   → v1.235 เพิ่ม `teamMailboxesForCrew()` + checkbox ในฟอร์ม (เซิร์ฟเวอร์ derive เอง
   เพราะตัวกรอง `SHARED_MAILBOXES` อ่าน env ซึ่งไม่มีใน bundle ฝั่ง client)
   → backfill: 61 ใบ ต.ค.–ธ.ค. ผ่าน assign (สถานะไม่ขยับ) · 63 ใบปี 2027 เขียน DB ตรง
   **ไม่ผ่าน assign** เพราะเราต์นั้นพลิก REQUESTED→ASSIGNED แล้ว `producerEditMode()`
   คืน `'none'` = โปรดิวเซอร์แก้ใบตัวเองไม่ได้ (เคสเดียวกับ v1.193)

3. **ปุ่ม "เพิ่มทีมงานทั้งชุด" ล้างข้อมูล** — เราต์ assign เขียน `adminNotes` /
   `freelancers` / `mainVideographerEmail` ทุกครั้งจาก body ตรง ๆ แต่ปุ่มส่งมาแค่
   `{assignedEmails, sendEmail}` ⇒ กดครั้งเดียวล้างสามฟิลด์ทั้งชุด
   พรอด ณ วันนั้น: ใบ routine มี adminNotes 38 ใบ · freelancers 27 ใบ
   ที่หนักกว่าคือ `emailRecipients` ประกอบจากอีเมลฟรีแลนซ์ด้วย พอว่าง ฟรีแลนซ์หลุด
   จากลิสต์แขกแล้ว Google ส่งใบยกเลิกให้ → v1.235 เปลี่ยนเป็น
   "คีย์ที่ไม่ได้ส่งมา = ไม่เปลี่ยน" แยกเป็น `resolveAssignPatch()` ที่เทสได้

4. **กระดิ่ง "งานได้รับอนุมัติแล้ว" ไม่เคยดังเลย** — label อยู่ใน
   `OWNER_OUTCOME_LABELS` ใต้คีย์ `approve` แต่ไม่เคยมีใครเขียน audit ชื่อนั้น
   และการแมตช์เป็นแบบตรงตัวไม่ใช่ prefix ⇒ ฟีเจอร์เขียนเสร็จแล้วตายมาตลอด
   → เปลี่ยนคีย์เป็น `booking.approve` ให้ตรงกับ audit ที่ข้อ 1 เพิ่งเริ่มเขียน

**v1.236 ลดจำนวนตอนได้** (นัทขอต่อ) — กันเฉพาะที่กระทบจริง: ตอนที่มีไฟล์อัปโหลด
ห้ามลบ (FK `uploads.episodeId` เป็น SET NULL ไม่ใช่ cascade — ลบแล้วไฟล์กำพร้าถาวร ·
พรอด: 7 ตอน 1,861 ไฟล์) · ต้องเหลืออย่างน้อย 1 ตอน · ไม่เลื่อน `sequence` ให้ชิด
(add-episodes มินต์จาก max+1 เลื่อนชิด = ID ชน) · ไม่ลบโฟลเดอร์ Drive

**เจอเพิ่มโดยยังไม่ได้แก้:** `sheetRowIndex` ซ้ำ — 9 แถวถูกอ้างโดย 32 ใบ
(ใบ Now 2027 ที่สร้างรวดเดียว `appendBookingRow` ยิงพร้อมกันแล้วอ่านความยาวชีท
ได้เลขเดียวกัน) · **ยังไม่ทำอันตราย** เพราะทุกจุดใช้ค่านี้เป็นแค่ธง "มีแถวไหม"
ส่วนแถวจริงหาโดยคอลัมน์ A เสมอ — แต่เป็นกับดักสำหรับคนที่มาเชื่อตัวเลขนี้ทีหลัง

**บทเรียนเครื่องมือ:** `parallel()` ของ workflow ต้องรับ thunk ไม่ใช่ promise —
ส่งผิดแล้วด่านคัดค้านล้มทั้ง 34 ข้อ และคืน `{survived: []}` ซึ่งหน้าตาเหมือน
"ตรวจแล้วไม่พบความเสี่ยง" เป๊ะ · อ่าน `<failures>` เสมอ อย่าอ่านแต่ `result`

---

## 2026-09-23 · v1.234.0–.2 — ต่อชุด Now ถึง มี.ค. 2027 · แถวชีทหายไป 12 โดยไม่มีอะไรฟ้อง

นัทสั่ง "ต่ออายุชุด routine ให้ Now ถึงมีนาคม" ชุดเดิม (`now-news-2026-12`) จบ 31 ธ.ค. 2026

**เจอก่อนสร้าง: `skipHolidays` ปี 2027 ไม่ได้ข้ามอะไรเลย** — `thai-holidays.ts` มีแต่ปี 2026
`isThaiHoliday()` คืน `false` เหมือนกันทั้งกับ *ไม่ใช่วันหยุด* และ *ไม่มีข้อมูลทั้งปี*
สร้างชุดข้ามปีจึงดู "ทำงานอยู่" ทั้งที่เงียบสนิท → v1.234.0 เพิ่ม `holidayYearCovered()` /
`uncoveredHolidayYears()` + คำเตือนสีเหลืองในฟอร์ม

**แล้วคำเตือนนั้นก็เงียบเอง** — `COVERED_YEARS` derive มาจาก `THAI_HOLIDAYS.map(...)`
พอใส่ `2027-01-01` เข้าไปวันเดียว ทั้งปี 2027 ถูกนับว่าครบทันที
`uncoveredHolidayYears('2027-01-01','2027-03-31')` คืน `[]` · v1.234.1 เปลี่ยนเป็นลิสต์
เขียนมือ และแก้เทสที่ล็อกพฤติกรรมผิดเอาไว้เอง (คาด `['2028']` ทั้งที่ควรเป็น `['2027','2028']`)

**เจอตอนเทียบค่าก่อนสร้าง: ฟอร์ม routine ไม่เคยเก็บ `producerEmail`** และ
`create-booking.ts:389` ไม่มี fallback ⇒ ทุกใบจากหน้านี้อีเมลเป็น null แล้วหลุด
`scope=mine` ของ `/my-bookings` = โปรดิวเซอร์มองไม่เห็นงานตัวเอง (คลาสเดิมกับ v1.196)
ชุด 2026 รอดเพราะ migrate มา ไม่ได้เกิดจากฟอร์มนี้ → v1.234.2 เพิ่มช่อง

**สร้างจริง 63 ใบ** `now-news-2027-01/02/03` (20/20/23) ข้ามแค่ 1 ม.ค.
รหัสออกมาเป็น `NWS-TSN-27MMDD-01` ถูกทั้ง 63 ใบ ค่าทุกช่องตรงชุดเดิม

**แล้วก็เจอของจริง: ชีทได้แค่ 51 จาก 63** — endpoint ตอบ `200 {created: 23, failed: []}`
ทั้งสามก้อน แต่ `count("sheetRowIndex")` ของ มี.ค. ค้างที่ **11/23** นิ่งอยู่หลายนาที
`create-booking.ts:474` ยิง `appendBookingRow(...)` แบบไม่ await ปิดท้าย `.catch(() => {})`
โควตา Sheets 60 เขียน/นาที (service account ตัวเดียวกับ worker ทุกตัว) เต็มกลางก้อนที่สาม
429 ถูกกลืน `created` ยังนับเต็มเพราะมันนับแถวใน **DB** ไม่ใช่ในชีท
→ บันทึกเป็น bug class ที่ 16 `fire-and-forget hides the failure`

**บทเรียน: ค่าที่ endpoint คืนกลับมาวัดเฉพาะสิ่งที่มัน await** ผลข้างเคียงที่ fire-and-forget
อยู่นอกสายตาของทั้ง success และ error path พร้อมกัน — ต้องไปอ่านของจริงฝั่งปลายทางเสมอ

---

## 2026-09-12 · v1.221.1 — `SONYCARD.IND` 0 ไบต์คือเรื่องปกติ ไม่ใช่ไฟล์เสีย

ต่อ NAS ได้เป็นครั้งแรกในรอบ 25 วัน (VPN + mount `production team`) แล้วอ่านต้นทางจริง
ไฟล์ 0 ไบต์ทั้ง **9 ตัว** ในกองค้างคือ `SONY/SONYCARD.IND` — ไฟล์ index ของการ์ด Sony
ที่ **ว่างเป็นปกติตั้งแต่ในกล้อง**

v1.221 ที่เพิ่ง deploy ไปเมื่อวานจึงจะฟ้อง "0 ไบต์" ทุกใบที่ถ่ายด้วย Sony = false positive ประจำ
บนช่องทางที่ทั้งจุดประสงค์คือ "ยังมีคนอ่านอยู่" — เตือนหมาป่าทุกวันจะฝึกให้คนข้ามพอดีวันที่
ไฟล์เสียจริง เพิ่ม `LEGIT_EMPTY_FILES` (match ที่ *ชื่อไฟล์* เพราะมันว่างโดยดีไซน์ไม่ว่าอยู่ที่ไหน)
+ เทสต์กันถอยหลัง 7 ข้อใน `footage-integrity.test.ts`

**บทเรียน: เขียนตัวตรวจจากภาพฝั่งปลายทางอย่างเดียวไม่พอ — ต้องเห็นต้นทางก่อนถึงจะรู้ว่า
อะไรคือ "ปกติ"** ถ้าไม่ได้เปิด NAS วันนี้ก็คงรู้ตอนที่คนเลิกอ่านแจ้งเตือนไปแล้ว

---

## 2026-09-12 · v1.221 — ถามคำถามที่ไม่มีใครถาม: "ไฟล์ใช้ได้ไหม"

AGN-260911-01/02 ผ่านทุกด่านแบบสะอาด — landing ว่าง, `video-merge` dry-run คืน
`seen=0 moved=0 dup=0`, folder-integrity เขียว, ไม่มีใครเตือนอะไรเลย แต่ของจริงคือ:

* `C004C002_260911WP.MXF` ลงมาเป็น **0 ไบต์**
* `A004R001_260911TJ.MXF` มี **2 ไฟล์ชื่อเดียวกันในโฟลเดอร์เดียวกัน** 9.87 GB กับ 47.9 GB
* AGN-260911-02 มี AUDIO ครบแต่ **CAM-A/B/C ว่างทั้งหมด** — ไม่มีวิดีโอเลยทั้งใบ

เพราะเครื่องมือเดิมทุกตัวถามว่า *"ไฟล์อยู่ถูกที่ไหม"* — `video-merge` นับว่าย้ายอะไรไป,
landing นับว่าลบอะไรไป, folder-integrity ตรวจ *รูปทรง* ของโฟลเดอร์
**ไฟล์ที่อัปไม่จบผ่านทั้งสามด่านสบาย ๆ**

### สิ่งที่เพิ่ม
`src/lib/footage-integrity.ts` + `/api/internal/footage-integrity/run` +
`scripts/footage-integrity-worker.js` (13:00 BKK ทุกวัน หลังรอบ prune เที่ยง) ตรวจ 3 อย่าง:
0 ไบต์ · ชื่อซ้ำในโฟลเดอร์เดียวกัน · ตอนที่มีเสียงแต่ไม่มีวิดีโอ

**ไม่มี apply path เลยโดยตั้งใจ** — ทางแก้ทุกทางคือลบหรือทับฟุตเทจจริง และเคส
TSS-WYS-260824-01 พิสูจน์แล้วว่าทางที่ "ดูชัด" อาจผิดตรงข้าม (ที่นั่นชื่อ+ขนาดตรงกัน
แต่ตัวที่เสียคือตัวใน **box**) การตัดสินว่าตัวไหนจริงต้องดูการ์ดต้นทาง ซึ่ง process นี้มองไม่เห็น
ทุกรายงานจึงปิดท้ายว่า *อย่าเพิ่งลบจนกว่าจะเทียบกับการ์ด*

### รายละเอียดที่ตั้งใจ
* auth ใช้ `internalSecretAllowed` (รับ secret ตัวไหนก็ได้) **ไม่ใช่** chain `A || B || C`
  ที่ทำให้ cron เงียบ 401 ไป 13 วัน · และ **ไม่สร้าง** `FOOTAGE_INTEGRITY_SECRET` ใหม่
  เพราะ secret ใหม่ = ของอีกชิ้นที่ตั้งบน stack แล้วลืมใส่ compose ได้
* `size === 0` เท่านั้น — ไฟล์ Google-native เป็น `null` ไม่ใช่ 0
* ชนเพดาน 60 ใบเมื่อไร เขียนบอกในรายงาน ไม่เงียบแล้วทำเหมือนตรวจครบ

### สองเทสต์ที่จับผมได้ตอนเขียน
`compose-env-coverage` จับว่า `FOOTAGE_INTEGRITY_URL/_SECRET` โค้ดอ่านแต่ compose ไม่ส่งเข้า
และ `heartbeat-specs` จับว่า worker ใหม่ยังไม่มี key ใน `SCRIPT_TO_KEY` (= worker ตายแล้วไม่มีใครรู้)
ทั้งคู่คือด่านที่กันความผิดพลาดแบบเดียวกับที่ไล่กันมาทั้งอาทิตย์ — ทำงานถูกต้อง

---

## 2026-09-11 · v1.220.1 — ถอนคำแนะนำที่ชี้ให้ลบไฟล์ตัวจริง

v1.220 ส่งข้อความเข้า Discord ทุกเที่ยงว่า "ถ้า `moved=0 dup>0` คือซ้ำ ลบตัวใน landing ได้"
ซึ่ง **ผิดแบบทำฟุตเทจหาย**: `dup` ตัดสินจาก *ชื่อ + ขนาด* เท่านั้น (video-merge.ts mirrorMove)

เคสจริง TSS-WYS-260824-01 — ทั้ง 585 ไฟล์ตรงกันชื่อ+ขนาด แต่ `DSC04216.ARW` เนื้อไม่เหมือน:
ตัวใน landing เป็น Sony RAW จริง (TIFF `49492A00`) ตัวใน box เป็นไฟล์หัว JPEG (`FFD8FFE1`)
= เสีย และ md5 ของตัวดีไม่มีอยู่ที่อื่นเลยทั้งสองไดรฟ์ โฟลเดอร์นี้คืน `moved=0 dup=585` พอดี
⇒ ข้อความเดิมชี้ให้คนเดินไปลบสำเนาที่ดีที่สุดเพียงตัวเดียว

เปลี่ยนเป็นเตือนกลับด้าน: dup>0 ไม่ได้แปลว่าลบได้ ต้องเทียบ md5 ก่อนเสมอ
(service account ของแอปคืน `md5Checksum` ได้ ส่วน Drive MCP connector ไม่คืน — ความเข้าใจผิด
ว่า "เช็ค checksum ไม่ได้" คือต้นเหตุที่รอบแรกสรุปผิด)

**บทเรียน: ชื่อ + ขนาด ตรงกัน ไม่ใช่ไฟล์เดียวกัน · `dup` ที่นิ่ง คือหน้าตาของ false duplicate**

---

## 2026-09-10 · v1.220 — ดึงรอบเที่ยงกลับเข้าคอนเทนเนอร์ + โฟลเดอร์ค้างที่ไม่มีใครรู้

### อาการที่ตั้งต้น
"โฟลเดอร์เก่าเยอะมาก และ worker ที่ยกไปให้ Hermes เหมือนไม่ได้ทำอะไร"

### สิ่งที่ตรวจแล้วพบว่า *ไม่* จริง
worker 19:00 ในคอนเทนเนอร์ทำงานปกติมาตลอด — audit `drive.landing_lifecycle` มี 19 จาก 20 วัน
(20 ส.ค.–8 ก.ย.) ลบโฟลเดอร์ว่างไป 51 ใบ รวมทุกวันในช่วงที่คิดว่าระบบตาย

### ที่ตายจริงคือ cron ฝั่ง Hermes (บนโน้ตบุ๊ก)
prod หมุน `NEXTAUTH_SECRET` วันที่ 25 ส.ค. แต่ `~/.hermes/scripts/probook.env` ถือค่าเก่า →
401 ติดกัน 13 รอบ (26 ส.ค.–8 ก.ย.) route ใช้ chain first-set-wins
`PREP_FOLDERS_SECRET || NEXTAUTH_SECRET || AUTH_SECRET` ⇒ v1.213 ที่ส่ง `AUTH_SECRET`
เข้าคอนเทนเนอร์จึงช่วยเส้นนี้ไม่ได้ ส่วน `probook-worker-check` รอดเพราะยิง endpoint ที่ใช้
`internalSecretAllowed()` (รับ secret ตัวไหนก็ได้). สคริปต์ Hermes `exit 0` เสมอ →
`failure_streak` ไม่ขยับ กลไกเตือนซ้ำของ Hermes (threshold 3) จึงไม่เคยทำงาน

### แต่ต่อให้ไม่ 401 มันก็ลบ 10 โฟลเดอร์นั้นไม่ได้อยู่ดี — ลูปปิดที่ไม่มีทางออก
`video-merge` เจอไฟล์ที่กล่องงานมี **ชื่อ + ขนาดตรงกัน** จะ `stats.dup++; continue`
= *ปล่อยไว้ใน landing* → โฟลเดอร์ไม่ว่าง → นโยบายห้ามลบโฟลเดอร์ที่มีไฟล์ → ค้างตลอดกาล
4 ใน 5 โฟลเดอร์ merge สำเร็จไปแล้ว **การกดปุ่ม merge ซ้ำเป็น no-op** — บอกให้ไปกดปุ่มคือสิ่งที่ทำให้ลูปปิดต่อไป
เช็คด้วย `video-merge/run?dryRun=1&code=<ID>`: `moved=0 dup>0` = ซ้ำจริง

ชื่อโฟลเดอร์ที่ค้างเคยอยู่แค่ใน response body เท่านั้น — เมลสรุปพิมพ์ `keptRecent` เป็นตัวเลขเปล่า,
audit เก็บแค่ `.length`, ไม่มีหน้าจอไหนอ่าน `kept*` เลย ⇒ 10 ใบ (เก่าสุด 16 วัน) ไม่มีใครเคยรู้

### สิ่งที่แก้
1. `pruneLandingToToday` เป็น **past-only** ผ่าน `landingMayBeTrashed()` — predicate ที่เขียน
   + มีเทสต์ไว้แล้วแต่ **ไม่เคยถูก import** (v1 ถามแค่ "ใช่วันนี้ไหม" ซึ่งตอบ "ไม่ใช่" ให้โฟลเดอร์
   *พรุ่งนี้* ที่เพิ่งสร้างตอน 19:00 ด้วย). เดิมกันไว้ฝั่ง client ในสคริปต์ python ที่กำลังจะเลิกใช้
2. รอบเที่ยงย้ายเข้า `scripts/landing-worker.js` (timer ที่สอง, `LANDING_PRUNE_HOUR` ค่าเริ่ม 12)
   — resolve secret จาก process env เดียวกับรอบเย็น จึง drift ไม่ได้ และไม่ง้อว่าโน้ตบุ๊กตื่นไหม
3. โฟลเดอร์ค้างส่งเข้า `notifyChat(…, 'footage')` **พร้อมชื่อ** + เขียน audit แม้ลบ 0 ใบ
4. heartbeat `landing-prune` แยก key จาก `landing` โดยตั้งใจ (รอบเที่ยงที่ปกติต้องไม่บังรอบเย็นที่ตาย)
5. `footage-ready` latch boolean → timestamp 15 นาที (แบบเดียวกับที่ landing แก้ไปตั้งแต่ v1.149)
   ตัวนี้ค้างจริงตั้งแต่ 4 ก.ย. ตอบ 409 วันละ 38 ครั้ง งานถ่ายจบ 16 ใบไม่เคยถูกเดินดู
6. `LANDING_PRUNE_*` ใส่ passthrough ใน compose **ทั้งสอง service** (ค่าบน stack ที่ compose
   ไม่อ้างถึง = ไม่มีอยู่จริง)

### ยังไม่ได้ทำ (ตัดสินใจแล้ว)
ไม่ให้ video-merge ลบไฟล์ซ้ำเอง — รายงานอย่างเดียว ให้คนตัดสินใจลบ
...
**v1.156 → v1.177.1 ขึ้น prod ไปแล้ว** · ฝั่งฟีเจอร์ CHANGELOG ครบแล้ว แต่**บันทึกการ deploy
(commit sha, รอบ redeploy, ผลตรวจหลัง deploy) ของช่วงนั้นหายไปและกู้ไม่ได้** — เขียนไว้ตรงนี้
ว่าหายจริง ดีกว่าประกอบขึ้นใหม่จากความจำ

ด้านล่างคือเหตุการณ์ปฏิบัติการในช่วงท้ายที่**มีหลักฐานอ้างอิงได้** (commit / log จริง):

### 2026-08-18 · งานตั้งเวลา 3 ตัวย้ายจาก Claude Code → Hermes cron

worker-check · id-first monitor · landing-cleanup เขียนใหม่เป็น Python stdlib
เก็บต้นฉบับที่ `scripts/hermes/` (`9c5315a`) · ตัวที่รันจริงคือสำเนาใน `~/.hermes/scripts/`
ซึ่ง**ไม่มีระบบสำรอง** — แก้ที่ repo แล้วต้องคัดลอกไปเอง

### 2026-08-19 ~11:03–12:00 · DNS ของเครื่อง operator ล่ม → รอบ 12:00 ล้ม **และ alert ก็ส่งไม่ออก**

`urlopen` Errno 8 ตอน landing-cleanup แล้ว Discord ก็ resolve ไม่ได้ด้วยเหตุเดียวกัน
รอบนั้นจึงเงียบสนิทจนไล่ดูเอง · แก้ด้วย retry 20/60/120 วิ + `net_down()` แยก
"เน็ตเราล่ม" ออกจาก "prod ล่ม" และให้ nightly-check อ่านไฟล์ผล + grep `delivery error`
ย้อน 24 ชม. (`a38076d`)

> **บทเรียน:** ช่องทางแจ้งเตือนที่วิ่งผ่านเน็ตเส้นเดียวกับสิ่งที่มันเฝ้า จะเงียบพร้อมกันเสมอ

### 2026-08-18/19 · Google auth flap — `unauthorized_client` เป็นครั้งคราว

`[sound-merge] 500 unauthorized_client` 18 ส.ค. 20:19 · `folderLiveness indeterminate`
อีก 2 ครั้ง 19 ส.ค. 13:19 — **รอบถัดไปกลับมา errors=0 เอง** (`02d6ff5`)
เป็นอาการฝั่ง Google ไม่ใช่สัญญาณให้เปลี่ยนคน impersonate · tri-state guard รับไว้ได้

### 2026-08-19 · launchd NAS agent — เด้ง 205 รอบติดกัน (~56 ชม.) โดยไม่มีใครรู้

agent เด้งทุก 10 นาทีแล้วขึ้น `not mounted — skip` เพราะ NAS ไม่ได้ mount
ซึ่ง**เป็นพฤติกรรมที่ออกแบบไว้** แต่ไม่มีใครตรวจกลุ่มงานนี้เลย จึงไม่มีใครรู้ว่าเงียบมานานแค่ไหน
· เพิ่ม step เฝ้าใน worker-check (เตือนที่ 48 ชม.) + ก็อป log ออกจาก `/tmp` ซึ่งหายทุกครั้งที่รีบูต (`e1f95fb`)

### 🔴 ค้างอยู่ · `DRIVE_FOOTAGE_ROOT` ชี้ไดรฟ์ที่หมุนด้วยมือ

ค่าปัจจุบันชี้ **"VIDEO 2026 [JUL–DEC]"** ซึ่งเปลี่ยนงวดด้วยมือ **ไม่มีโค้ดเทียบกับปฏิทิน**
ขึ้นงวดใหม่แล้วลืมหมุน = เขียนกล่องงานลงไดรฟ์งวดเก่าเงียบ ๆ ไม่มีอะไรฟ้อง
· worker-check เตือนล่วงหน้าเป็นช่วง (≤14/7/3/1 วัน) แล้ว แต่**การหมุนยังต้องทำเอง**
...
the event behave like On Location (Map location + van + the offsite validation) —
