 ...
 * and joins them with episode-status counts from the PD tabs + legacy "_EPs"
 * (see fetchAllEpisodeRows) + booking counts from DB.
 *
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [projectsRes, episodes] = await Promise.all([
      sheets.spreadsheets.values.get({ spreadsheetId: sheetId, range: `${projectsTab}!A2:J` }),
      fetchAllEpisodeRows(sheets, sheetId),
    ])
